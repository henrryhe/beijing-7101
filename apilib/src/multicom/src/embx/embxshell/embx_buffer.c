...
EMBX_ERROR res = EMBX_INVALID_TRANSPORT;
    ...
    EMBX_Info(EMBX_INFO_BUFFER, (">>>>EMBX_Alloc(htp=0x%08lx, size=%u)\n",(unsigned long)htp,(unsigned)size));
...
EMBX_Buffer_t *buf;
    ...
    EMBX_Info(EMBX_INFO_BUFFER, (">>>>EMBX_GetBufferSize(0x%08lx)\n",(unsigned long)buffer));
    EMBX_DebugOn(EMBX_INFO_BUFFER);
