                                      ...
                                      EMBXSHM_PipeElement_t *element)
{
...
#ifdef __ST20__
    /* since the ST20 has only 16-byte cache lines there is a risk
     * the control word could be written before the data so we must
     * prevent that happending
     */
    ...
    EMBXSHM_WROTE(&pipe->elements[index]);
#else
    __optasm { ldab &pipe->elements[index].control, element->control; devsw; }
#endif
