...
MODULE_PARM_DESC (mailbox0, "Configuration string of the form "
                            "name:id:participants:warp:maxports:maxobjects:freelist:shared:size[:warp:size]");
...
static int parseMailboxString(char *str, EMBXSHM_MailboxConfig_t *mailbox)
{
	...
	EMBX_Info(EMBX_INFO_FACTORY, ("<<<parseMailboxString sharred addr 0x%08x\n", mailbox->sharedAddr));
	EMBX_Info(EMBX_INFO_FACTORY, ("<<<parseMailboxString sharred size 0x%08x\n", mailbox->sharedSize));
	EMBX_Info(EMBX_INFO_FACTORY, ("<<<parseMailboxString warp addr 0x%08x\n", mailbox->warpRangeAddr));
