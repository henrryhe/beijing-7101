                                  ...
                                  EMBX_HANDLE      *userHandle)
{
    ...
    if (found)
    {
         ...
         */
        if (((size_t)object >= (size_t)tpshm->warpRangeAddr) &&
            (((size_t)object+size) <= ((size_t)tpshm->warpRangeAddr+tpshm->warpRangeSize)))
        {
            obj[index].owningCPU  = tpshm->cpuID;
            obj[index].size       = size;
            obj[index].sharedData = (EMBX_VOID *)LOCAL_TO_BUS (object, tpshm->pointerWarp);
    ...
    EMBX_Info (EMBX_INFO_TRANSPORT, ("<<<registerObject(..., *userHandle = 0%08x) = %d\n",
...
{
    EMBX_Info (EMBX_INFO_TRANSPORT, (">>>testState(0x%08x, 0x%08x)\n", 
