...
#define ST20_C2_TICKS_PER_SEC 15625
#define EMBX_OS_Delay(_a) task_delay( (_a)*(ST20_C2_TICKS_PER_SEC/1000) )
...
#if defined(__ST20__) && (__CORE__ == 4)
/* Setup use of the device instructions on the ST20 C2 */
#pragma ST_device(EMBX_vuint32_t)
#pragma ST_device(EMBX_vuint16_t)
#pragma ST_device(EMBX_vuint8_t)
#endif /* __ST20__ */
 ...
 *
 * On OS21/ST210 you must use an address controlled by a DPU
 * region which is set to uncached.
 *
 * On OS20/ST20-C2 these will use the device instructions and
...
/*
 * - Physical Memory mapping for WinCE
 */
EMBX_VOID *EMBX_OS_PhysMemMap   (EMBX_VOID *pMem, EMBX_INT size);
...
semaphore_t * EMBX_OS_SemaphoreCreate(EMBX_UINT);
void EMBX_OS_SemaphoreDelete(semaphore_t *);

#define EMBX_OS_MUTEX_INIT(pMutex)      ((*(pMutex) = EMBX_OS_SemaphoreCreate(1)) != NULL)
#define EMBX_OS_MUTEX_DESTROY(pMutex)   EMBX_OS_SemaphoreDelete(*(pMutex))
#define EMBX_OS_MUTEX_TAKE(pMutex)      semaphore_wait(*(pMutex))
#define EMBX_OS_MUTEX_RELEASE(pMutex)   semaphore_signal(*(pMutex))

#define EMBX_OS_EVENT_INIT(pMutex)      ((*(pMutex) = EMBX_OS_SemaphoreCreate(0)) != NULL)
#define EMBX_OS_EVENT_DESTROY(pMutex)   EMBX_OS_SemaphoreDelete(*(pMutex))
...
#define EMBX_OS_EVENT_INIT(pEvent)      ((*(pEvent) = CreateSemaphore(NULL,0,0xffff,NULL)) != NULL)
#define EMBX_OS_EVENT_DESTROY(pEvent)   CloseHandle((HANDLE)*(pEvent))
#define EMBX_OS_EVENT_WAIT(pEvent)      (WaitForSingleObject((HANDLE)*(pEvent),INFINITE) != WAIT_OBJECT_0)
#define EMBX_OS_EVENT_POST(pEvent)      ReleaseSemaphore((HANDLE)*(pEvent),1,NULL)
...
#define EMBX_CACHE_SCRATCH_ADJUSTMENT 16
extern char *embx_cache_scratch_area;
...
#define EMBX_OS_PurgeCacheWithPrecomputableLength(ptr, sz) \
        do { \
                volatile int * x; \
                if (sz <= 16) { \
                        EMBX_Assert((16 - (((unsigned) (ptr)) & 15)) <= 16); \
                        x = (void *) (embx_cache_scratch_area + ((unsigned) (ptr) & 0x1ff0)); \
                        (void) x[(EMBX_CACHE_SCRATCH_ADJUSTMENT/4)]; \
                } else if (sz <= 32) { \
                        EMBX_Assert((32 - (((unsigned) (ptr)) & 31)) <= 32); \
                        x = (void *) (embx_cache_scratch_area + ((unsigned) (ptr) & 0x1ff0)); \
                        (void) x[(EMBX_CACHE_SCRATCH_ADJUSTMENT/4)]; \
                        (void) x[(EMBX_CACHE_SCRATCH_ADJUSTMENT/4) + 4]; \
                } else { \
...
#if defined EMBX_SUPPORT_ST220_CUT1
#define GNBvd27351() __asm__ __volatile__ ("sync; ;; ; sync" ::)
#else
#define GNBvd27351() ((void) 0)
#endif
