		...
		*dataBuffer_p = &(buf->buffer);
		...
		if (0 != driverResult) {
			...
			return MME_NOMEM;
                ...
                if (flags & MME_ALLOCATION_UNCACHED) {
                        /* SH4 hack - set returned phys address as uncached */
                        /* This will be used by mmap() */
                        allocInfo.offset = (allocInfo.offset & 0x1fffffff) | 0xa0000000;
                }

		MME_Info(MME_INFO_LINUX_MODULE, (DEBUG_NOTIFY_STR
						 "Calling mmap(0, size 0x%08x, prot READ|WRITE, PRIVATE, devioce %d, offset 0x%08x)\n",
