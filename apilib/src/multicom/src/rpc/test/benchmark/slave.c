/*
 * shakedown/roundtrip/st40.c
 *
 * Copyright (C) STMicroelectronics Ltd. 2001
 *
 * ST40 side of simple primative types shakedown test
 */

#include "benchmark.h"

int main(void)
{
	/* ensure the machine and RPC are booted and initialised */
	harness_boot();

	/* we have nothing to do to just wait around */
	harness_waitForShutdown();

	return 0;
}

void st40_no_data          (void)                 {}
void st40_int_1_word       (int d)                {}
void st40_struct_4_word    (struct_4_word_t d)    {}
void st40_struct_16_word   (struct_16_word_t d)   {}
void st40_struct_64_word   (struct_64_word_t d)   {}
void st40_struct_256_word  (struct_256_word_t d)  {}
void st40_struct_1024_word (struct_1024_word_t d) {}
void st40_array_4_word     (array_4_word_t d)     {}
void st40_array_16_word    (array_16_word_t d)    {}
void st40_array_64_word    (array_64_word_t d)    {}
void st40_array_256_word   (array_256_word_t d)   {}
void st40_array_1024_word  (array_1024_word_t d)  {}
void st40_struct_10_word   (inout struct_10_word_t *p) {}
void st40_func_10_param    (
	inout int *a, inout int *b, inout int *c, inout int *d, inout int *e,
	inout int *f, inout int *g, inout int *h, inout int *i, inout int *j)
	{}

void st40_dotest(test_type_t test) {
	int i;

	VERBOSE(printf("st40_dotest(%d) ... ", test));
	VERBOSE(fflush(stdout));

	switch (test) {
		case no_data:
		{
			for (i=0; i<NUM_ITERATIONS; i++) { st20_no_data(); }
			break;
		}

		case int_1_word:
		{
			int i1;
			for (i=0; i<NUM_ITERATIONS; i++) { st20_int_1_word(i1); }
			break;
		}

		case struct_4_word:
		{
			struct_4_word_t s;
			for (i=0; i<NUM_ITERATIONS; i++) { st20_struct_4_word(s); }
			break;
		}

		case struct_16_word:
		{
			struct_16_word_t s;
			for (i=0; i<NUM_ITERATIONS; i++) { st20_struct_16_word(s); }
			break;
		}

		case struct_64_word:
		{
			struct_64_word_t s;
			for (i=0; i<NUM_ITERATIONS; i++) { st20_struct_64_word(s); }
			break;
		}

#ifndef REDUCE_COMPLEXITY
		case struct_256_word:
		{
			struct_256_word_t s;
			for (i=0; i<NUM_ITERATIONS; i++) { st20_struct_256_word(s); }
			break;
		}

#ifdef EXTRA_COMPLEXITY
		case struct_1024_word:
		{
			struct_1024_word_t s;
			for (i=0; i<NUM_ITERATIONS; i++) { st20_struct_1024_word(s); }
			break;
		}
#endif
#endif /* REDUCE_COMPLEXITY */

		case array_4_word:
		{
			array_4_word_t a;
			for (i=0; i<NUM_ITERATIONS; i++) { st20_array_4_word(a); }
			break;
		}

		case array_16_word:
		{
			array_16_word_t a;
			for (i=0; i<NUM_ITERATIONS; i++) { st20_array_16_word(a); }
			break;
		}

		case array_64_word:
		{
			array_64_word_t a;
			for (i=0; i<NUM_ITERATIONS; i++) { st20_array_64_word(a); }
			break;
		}

		case array_256_word:
		{
			array_256_word_t a;
			for (i=0; i<NUM_ITERATIONS; i++) { st20_array_256_word(a); }
			break;
		}

		case array_1024_word:
		{
			array_1024_word_t a;
			for (i=0; i<NUM_ITERATIONS; i++) { st20_array_1024_word(a); }
			break;
		}

		case struct_10_word:
		{
			struct_10_word_t a;
			for (i=0; i<NUM_ITERATIONS; i++) { st20_struct_10_word(&a); }
			break;
		}

		case func_10_param:
		{
			int a, b, c, d, e, f, g, h, i, j;
			for (i=0; i<NUM_ITERATIONS; i++) { st20_func_10_param(&a, &b, &c, &d, &e, &f, &g, &h, &i, &j); }
			break;
		}

		default:
			assert(0);
			break;
	}

	VERBOSE(printf("done\n"));
}
