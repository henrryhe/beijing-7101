...
#define LX_AUDIO_CODE_MEMORY_SIZE   (2*1024*1024)    /* 2 MBytes */
        ...
        #define LX_VIDEO_CODE_MEMORY_SIZE_MB    2   /* Audio firmware with LX Copy must be loaded at address 0xA4200000 */
    #else
...
#define STAVMEM_SYS_MEMORY_SIZE         ((45-LX_VIDEO_CODE_MEMORY_SIZE_MB)*1024*1024 + 512*1024) /* 512kB = NCACHE PARTITION */
...
#define MPEG_SDRAM_SIZE                     VIRTUAL_SIZE
     ...
     *              | EMI (64MB)                  |
     *  0xA4000000  -------------------------------
     *              | LX_VIDEO_CODE_MEMORY  (2MB) | <- SECURE
     *  0xA4200000  -------------------------------
     *              | LX_AUDIO_CODE_MEMORY  (2MB) | <- SECURE
     *  0xA4400000  -------------------------------
     *              | SH4_CODE_MEMORY      (16MB) | <- Stack and heap space included: NON-SECURE
     *  0xA5400000  -------------------------------
     *              | NCACHE PARTITION    (0.5MB) | <- NON-SECURE
     *  0xA5480000  -------------------------------
     *              | SECURE_PARTITION     (10MB) | <- PARTITION2: SECURE partition for PES + ES buffers
     *  0xA5E80000  -------------------------------
     *              | RESERVED_PARTITION   (24kB) | <- PARTITION3: PARTITION FOR STAPI DRIVERS INTERNAL USAGE (NON-SECURE at startup)
     *  0xA5E86000  -------------------------------
     *              | STAVMEM_SYS_MEMORY   (33MB) | <- PARTITION0 (rest of LMI_SYS): NON-SECURE
     *  0xA7F80000  -------------------------------
     *              ...............................
     *  0xB0000000  -------------------------------
     *              | STAVMEM_VID_MEMORY   (64MB) | <- PARTITION1 (LMI_VID): SECURE
     *  0xB3FF1000  -------------------------------
     */

    #define MAX_PARTITIONS                     4
    #define SECURE_PARTITION_SIZE              (10*1024*1024)
    #define RESERVED_PARTITION_SIZE            (24*1024)

    #define PARTITION0_START                   (NCACHE_PARTITION_BASE + NCACHE_PARTITION_SIZE + SECURE_PARTITION_SIZE + RESERVED_PARTITION_SIZE)
    #define PARTITION0_STOP                    (NCACHE_PARTITION_BASE + STAVMEM_SYS_MEMORY_SIZE - 1)
    ...
    /* Reserve a partition to set in secure memory to allocate PES and ES buffers */
    #define PARTITION2_START                   (NCACHE_PARTITION_BASE + NCACHE_PARTITION_SIZE)
    #define PARTITION2_STOP                    (PARTITION2_START + SECURE_PARTITION_SIZE - 1)

    /* Reserve a partition to set in secure memory to allocate Preproc and display filter work arounds */
    #define PARTITION3_START                   (PARTITION2_STOP + 1)
    #define PARTITION3_STOP                    (PARTITION3_START + RESERVED_PARTITION_SIZE - 1)
...
#endif /* defined (ST_7109) */
#define STAVMEM_VID_MEMORY_SIZE     (64*1024*1024)  /* 64 MBytes */
#define SHARED_AUDIO_WINCE			   (6*1024*1024)   /* 6 MBytes shared memory for audio */
#define LMI_SYS_BASE                0xA4000000
#define LMI_SYS_SIZE                (64*1024*1024)  /* 64 MBytes of LMI_SYS */
#define LMI_VID_BASE                0xB0000000
#define LMI_VID_SIZE                (64*1024*1024) /* 64 MBytes of LMI_VID */
 ...
 *              | LX_AUDIO_CODE_MEMORY (2MB) | <- SECURE
 *  0xA4400000  ------------------------------
 *              | SH4_CODE_MEMORY   (31.5MB) | <- Stack and heap space included (starts at 0xA53ffffc): NON SECURE
 *  0xA6380000  ------------------------------
 *              | NCACHE PARTITION   (0.5MB) | <- NON SECURE
 *  0xA6400000  ------------------------------
 *              | SECURE_PARTITION    (14MB) | <- PARTITION2: SECURE partition for PES + ES buffers
 *  0xA7200000  ------------------------------
 *              | RESERVED_PARTITION  (24kB) | <- PARTITION3: FOR STAPI DRIVERS INTERNAL USAGE
 *  0xA7206000  ------------------------------
 *              | STAVMEM_SYS_MEMORY (~14MB) | <- PARTITION0: NON-SECURE (LMI_SYS)
 *  0xA8000000  ------------------------------
 *              ..............................
 *  0xB0000000  ------------------------------
 *              | STAVMEM_VID_MEMORY (64MB)  | <- PARTITION1: SECURE (LMI_VID)
 *  0xB4000000  ------------------------------
 *
*/
...
#define SECURE_PARTITION_SIZE              (14*1024*1024)
#define RESERVED_PARTITION_SIZE            (24*1024)
#define STAVMEM_SYS_MEMORY_SIZE            (LMI_SYS_SIZE - SECURE_PARTITION_SIZE - NCACHE_PARTITION_SIZE - SH4_CODE_MEMORY_SIZE - LX_AUDIO_CODE_MEMORY_SIZE - LX_VIDEO_CODE_MEMORY_SIZE - RESERVED_PARTITION_SIZE)

#define PARTITION0_START                   (LMI_SYS_BASE + LX_VIDEO_CODE_MEMORY_SIZE + LX_AUDIO_CODE_MEMORY_SIZE + SH4_CODE_MEMORY_SIZE + NCACHE_PARTITION_SIZE + SECURE_PARTITION_SIZE + RESERVED_PARTITION_SIZE)
#define PARTITION0_STOP                    (PARTITION0_START + STAVMEM_SYS_MEMORY_SIZE - 1)
...
#define PARTITION3_START                   (LMI_SYS_BASE + LX_VIDEO_CODE_MEMORY_SIZE + LX_AUDIO_CODE_MEMORY_SIZE + SH4_CODE_MEMORY_SIZE + NCACHE_PARTITION_SIZE + SECURE_PARTITION_SIZE)
#define PARTITION3_STOP                    (PARTITION3_START + RESERVED_PARTITION_SIZE - 1)

#else /* Not Secured 7109 */
#define MAX_PARTITIONS                      2

#define STAVMEM_SYS_MEMORY_SIZE            (LMI_SYS_SIZE - SHARED_AUDIO_WINCE - NCACHE_PARTITION_SIZE - SH4_CODE_MEMORY_SIZE - LX_AUDIO_CODE_MEMORY_SIZE - LX_VIDEO_CODE_MEMORY_SIZE)
