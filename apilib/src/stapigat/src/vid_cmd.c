...
static BOOL VID_Close(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if (API_TestMode == TEST_MODE_MULTI_HANDLES)
    {
        ...
        if (!found)
        {
            STTBX_Print(("Error: impossible to find stream injection context associated to this video handle !!!\n"));
        }
    }
    else
    {
            ...
            for (i = 0; i < VID_MAX_STATISTICS_PB_LIST; i++)
            {
                ...
                if (StrFound_p != NULL)
                {
                    ...
                    switch(i)
                    {
                        ...
                        case 2:
                        case 3:
                            STTBX_Print(("   Hardware error not available.\n"));
                            break;
#endif /* STVID_HARDWARE_ERROR_EVENT */
...
#if defined (mb295) || defined (mb290) || defined (db573) || defined (ST_5528) || defined (ST_5100) || defined (ST_5105) || defined (ST_5107) || defined (ST_5188) || defined (ST_5301) || defined (ST_7710) || defined (ST_7100) || defined (ST_7109) || defined (ST_7200) || defined (ST_5525)
      ...
      if (!(RetErr))
      {
        ...
        switch (ErrCode)
        {
            case ST_NO_ERROR :
                ...
                {
                    STTBX_Print(("Failed to subscribe workarounds injection !!\n" ));
                    API_ErrorCount++;
                    RetErr = TRUE;
                }
#endif /* STVID_INJECTION_BREAK_WORKAROUND */
                ...
                if (SubscribeErrorCode != ST_NO_ERROR)
                {
                    STTBX_Print(("Failed to subscribe PROVIDED_DATA_TO_BE_INJECTED  !!\n" ));
                    API_ErrorCount++;
                    RetErr = TRUE;
                }
...
#ifdef VIDEO_DEBUG
/*-------------------------------------------------------------------------
 * Function : VID_RequestDeinterlacingMode
 *            Set requested Deinterlacing mode
 * Input    : *pars_p, *result_sym_p
 * Output   :
 * Return   : TRUE if ok, else FALSE
 * ----------------------------------------------------------------------*/
static BOOL VID_RequestDeinterlacingMode(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ST_ErrorCode_t          ErrCode;
    BOOL                    RetErr;
    STVID_ViewPortHandle_t  CurrentViewportHndl;
    ...
    {
        ErrCode = STVID_RequestDeinterlacingMode(CurrentViewportHndl, RequestedMode);
    } /* end if */
    STTST_AssignInteger(result_sym_p, ErrCode, FALSE);

    return (API_EnableError ? RetErr : FALSE );

} /* end VID_RequestDeinterlacingMode() */
#endif /* VIDEO_DEBUG */
...
static BOOL VID_SetMemoryProfile(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if (!(RetErr))
    {
        ...
        if (RetErr)
        {
            STTBX_Print(("Error : variable MEM_PROF_BUFFER_NB undefined !\n"));
        }
        if (!(RetErr))
        {
    ...
    if (!(RetErr)) /* if parameters are ok */
    {
        ...
        switch(ErrCode)
        {
            case ST_NO_ERROR :
                ...
                {
                    STTBX_Print(("Failed to unsubscribe workarounds injection !!\n" ));
                    API_ErrorCount++;
                    RetErr = TRUE;
                }
#endif /* STVID_INJECTION_BREAK_WORKAROUND */
...
BOOL VID_RegisterCmd (void)
{
    ...
    RetErr |= STTST_RegisterCommand("VID_CLEAR",     VID_Clear,                 "[<Handle>] <Luma_pattern> <Chroma_pattern> <Chroma_pattern> Clears/colors the display");
    ...
    RetErr |= STTST_RegisterCommand("VID_INIT",      VID_Init,                  "<DeviceName><\"V|MPEG\"><VIDDev#><EvtDevName><ClkDevName> Initialization for video Mpeg\n\t\t<DeviceName><\"H264\"><VIDDev#><EvtDevName><ClkDevName> Initialization for H264\n\t\t<DeviceName><\"VC1\"><VIDDev#><EvtDevName><ClkDevName> Initialization for VC1\n\t\t<DeviceName><\"MPEG4P2\"><VIDDev#><EvtDevName><ClkDevName> Initialization for MPEG4P2\n\t\t<DeviceName><\"D\"><1=SD1|2=SD2|3=HD0><EvtDevName><ClkDevName> Initialization for video with D1");
    #else
    RetErr |= STTST_RegisterCommand("VID_INIT",      VID_Init,                  "<DeviceName><\"V|MPEG\"><VIDDev#><EvtDevName><ClkDevName> Initialization for video Mpeg\n\t\t<DeviceName><\"D\"><1=SD1|2=SD2|3=HD0><EvtDevName><ClkDevName> Initialization for video with D1");
    #endif
#else
    RetErr |= STTST_RegisterCommand("VID_INIT",      VID_Init,                  "<DeviceName><\"V|MPEG\"><VIDDev#><EvtDevName><ClkDevName><BB-allocated><BB-address><BB-size> Initialization for video\n\t\t<DeviceName><\"D\"><1=SD1|2=SD2|3=HD0><VINDevName><EvtDevName><ClkDevName> Initialization for video with D1");
#endif /* ST_MasterDigInput */
...
#ifdef VIDEO_DEBUG
    RetErr |= STTST_RegisterCommand("VID_RQSTDEIMODE",  VID_RequestDeinterlacingMode,      "<ViewportHndl> <Mode> Set requested Deinterlacing mode");
#endif
    ...
    {
        STTBX_Print(("Failed register conditional traces !!\n"));
        RetErr =TRUE;
    }
#endif /* VALID_TOOLS */
