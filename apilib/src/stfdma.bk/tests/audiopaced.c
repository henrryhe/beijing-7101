...
ST_ErrorCode_t FDMALinkedListAllocate(StreamType_t StreamType, void **Node_p, U32 SampleCount)
{
    ...
    if(StreamType == SPDIF_COMPRESSED)
    {
        ...
        for(i = 0, j = 0; i < SPDIF_NODE_COUNT; i++)
        {
    		...
    		temp_p->SourceAddress_p			= (void *)(((U32)SPDIF_PLAYER_BUFFER_ADDRESS + i*(FrameSize/4)*4) & (0x1FFFFFFF));
#if defined (ST_7200)
  		    temp_p->DestinationAddress_p	= (void *)(SPDIF_FIFO_DATA_REG);
#else
    		temp_p->DestinationAddress_p	= (void *)((SPDIF_FIFO_DATA_REG) & (0x1FFFFFFF));
#endif
    ...
    else
    {
        ...
        for(i = 0; i < 4; i++)
        {
    		...
    		if(StreamType == PCM_TEN_CHANNEL)
    		{
    		    ...
    		    temp_p->SourceAddress_p					    = (void *)((PCM_PLAYER_0_BUFFER_ADDRESS + i*temp_p->NumberBytes) & 0x1FFFFFFF);
#if !defined (ST_7200)
    		    temp_p->DestinationAddress_p				= (void *)(PCMPLAYER0_FIFO_DATA_REG & 0x1FFFFFFF);
#else
            ...
            else
            {
    		    ...
    		    temp_p->SourceAddress_p					    = (void *)((PCM_PLAYER_1_BUFFER_ADDRESS + i*temp_p->NumberBytes) & 0x1FFFFFFF);
#if !defined (ST_7200)
    		    temp_p->DestinationAddress_p				= (void *)(PCMPLAYER1_FIFO_DATA_REG & 0x1FFFFFFF);
#else
...
void PlayerStart(StreamType_t StreamType)
{
    ...
    if(StreamType == SPDIF_COMPRESSED)
    {
    	...
    	STOS_TaskDelay(ST_GetClocksPerSecond()/30000);
...
#endif
{
    ...
    ST_ErrorCode_t ErrorCode = ST_NO_ERROR;
    U32 SampleCount = 0;
    StreamType_t StreamType = (StreamType_t)pData;
