...
int fdmatest_readdir(struct file *filp,  void *other, filldir_t othermore)
{
    return (0);
}


unsigned int fdmatest_poll   (struct file *filp,  struct poll_table_struct *pollp)
{
    int err = 0;    
    unsigned int mask = 0;
...
int fdmatest_mmap   (struct file *filp,  struct vm_area_struct *memp)
{
    return (-EBUSY);
}


int fdmatest_fsync  (struct inode *node, struct dentry *entry, int other)
{
    return (-EBUSY);
}
