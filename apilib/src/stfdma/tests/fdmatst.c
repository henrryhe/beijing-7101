...
defined(ST_7100) || defined(ST_5700) || defined(ST_7109) || defined (ST_5525) || defined (ST_5107) || defined (ST_5162)
...
static const ST_Revision_t stfdmatst_Rev = "2.3.0";
...
static U32 intCounter = 0;
static U32 totalInterrupts = 0;
extern U32 fdma_int_timeTaken;
semaphore_t * interrupt_profile_sem_p = NULL;
BOOL FDMAIntPrfTaskContinue = TRUE;
static tdesc_t *FDMAInterruptProfileTaskDescriptor = NULL;
static U32     *FDMAInterruptProfileTaskStack = NULL;
#if defined (ST_OSLINUX)
static struct task_struct* FDMAInterruptProfileTask;
static int  FDMAIntPrfTskFn(void * Param);
#else
static task_t* FDMAInterruptProfileTask;
static void FDMAIntPrfTskFn(void * Param);
#endif
#endif
...
/* Functions -------------------------------------------------------------- */
#if defined (ARCHITECTURE_ST40)
void * BasePhyAddrSet = NULL;
void * BaseVirtAddrSet = NULL;

void * fdmatst_PeriphAddrTranslate(void * Addr)
{
	void * PhyAddr;
#if 0 /*Non 32-bit support*/	
	return ((void*)((U32)(Addr) & 0x1FFFFFFF));
#else
#if 1 
	/*Addresses reset everytime, to make test-harness fullt 32 bit compatible need to 
      allocate a partition from existing NChache partition and use global nodes across all 
      test functions and use the base addresses so obtained for all subsequent phy addr 
      access. Right now  STOS_VirtToPhys() called everytime !*/	
	fdmatst_ResetPeriphAddrTranslate();	
#endif
	if(BaseVirtAddrSet == NULL)
		BaseVirtAddrSet = Addr;
	if(BasePhyAddrSet == NULL)
		BasePhyAddrSet = STOS_VirtToPhys(Addr);
	
	PhyAddr = STOS_AddressTranslate(BasePhyAddrSet, BaseVirtAddrSet, Addr);
	/*When No STOS required*/
	/*PhyAddr = ((void *)((U32)(BasePhyAddrSet) + ((U32)(Addr) - (U32)(BaseVirtAddrSet))));*/	
	
	/*STTBX_Print(("BV: %x, BP: %x, Tv: %x: TP %x\n", (U32)BaseVirtAddrSet, (U32)BasePhyAddrSet, (U32)Addr, (U32)PhyAddr));*/
	return (PhyAddr);
#endif	
}
...

{
...
#elif defined (ST_5105) || defined (ST_5188) || defined (ST_5107) || defined (ST_7200) || defined (ST_5162)
    ...
    if((Node_p->NodeControl.PaceSignal = PaceSignal) != STFDMA_REQUEST_SIGNAL_NONE)
    {
...
#endif
    	{
            ...
            Dest_p = (U32 *)ST5162_SSC1_BASE_ADDRESS;
...
defined(ST_7100) || defined(ST_5700) || defined(ST_7109) || defined (ST_5525) || defined (ST_5107) || defined (ST_5162)
    ...
    */
    ...
    {
#if defined (PROFILE_FDMA_INTERRUPT)	
	if(interrupt_profile_sem_p == NULL)
		interrupt_profile_sem_p = STOS_SemaphoreCreateFifo(system_partition, 0);
	if(FDMAInterruptProfileTaskDescriptor == NULL)
		FDMAInterruptProfileTaskDescriptor = (tdesc_t *)STOS_MemoryAllocate(system_partition, sizeof(tdesc_t));
       STOS_TaskCreate((void(*)(void *))FDMAIntPrfTskFn,
                        (void *)NULL,
                        system_partition,
                        (size_t)(16 * 1024),
                        (void **)&FDMAInterruptProfileTaskStack,
                        system_partition,
                        &FDMAInterruptProfileTask,
                        FDMAInterruptProfileTaskDescriptor,
                        MIN_USER_PRIORITY,
                        "FDMAInterruptProfileTask",
                        (task_flags_t)0);
#endif       
...
#endif
#if defined (PROFILE_FDMA_INTERRUPT)		
		FDMAIntPrfTaskContinue = FALSE;
		 STTBX_Print(("Before Task wait...\n"));
		 STOS_SemaphoreSignal(interrupt_profile_sem_p);
        STOS_TaskWait(&FDMAInterruptProfileTask, (STOS_Clock_t *)TIMEOUT_INFINITY);
		 STTBX_Print(("After Task wait...\n"));		
        STOS_TaskDelete(FDMAInterruptProfileTask,
                        system_partition,
                        FDMAInterruptProfileTaskStack,
                        system_partition);	
		STOS_MemoryDeallocate(system_partition, FDMAInterruptProfileTaskDescriptor);		
		if(interrupt_profile_sem_p)
			STOS_SemaphoreDelete(system_partition, interrupt_profile_sem_p);
#endif
    }
...
/****************************************************************************
Name         : FDMAIntPrfTskFn
Description  : Task for profiling Interrupt Latency
Parameters   :
Return Value :
****************************************************************************/
#if defined (PROFILE_FDMA_INTERRUPT)	
...
{	
	int i =0;
    STOS_TaskEnter(NULL);
	while(FDMAIntPrfTaskContinue)
	{			
		STOS_SemaphoreWaitTimeOut(interrupt_profile_sem_p, TIMEOUT_INFINITY);
		int_ticks[intCounter] = 	fdma_int_timeTaken;
		average += int_ticks[intCounter];
		intCounter = (intCounter + 1)%10;
		totalInterrupts++;		
		/*STTBX_Print(("ProfileTask: INT-TICK: %u\n", fdma_int_timeTaken));*/
		/*STOS_TaskDelay(ST_GetClocksPerSecond()/10);*/
	}
	STTBX_Print(("Last 10 Interrupt Profile\n"));
	for(i = 0; i < 10; i++)
		STTBX_Print(("INT_PROF[%d] = %u\n", i, int_ticks[i]));
	STTBX_Print(("FDMA Interrupt Profiling Results>>Ticks Pers second = %u, Average Ticks = %u Total Interrupts = %u\n", ST_GetClocksPerSecond(), (average/totalInterrupts), totalInterrupts));	
    STOS_TaskExit(NULL);
#if defined (ST_OSLINUX)
    return ST_NO_ERROR;
#endif
