...
/*=============================================================================
  ...
  ===========================================================================*/
int stlayer_ioctl_open(struct inode *node, struct file *filp)
{
 ...
 /* int major = imajor(node);         *//* The major number */
    ...
    if (NULL == filp->private_data) {
        ...
        /* stlayer_ioctl_NewNode() may deshedule - call must be reentrant */
        filp->private_data = stlayer_ioctl_NewNode(minor);
...
/*=============================================================================
   ...
   stlayer_ioctl_release   (close)

   This function can deschedule and MUST be reentrant.

  ===========================================================================*/
int stlayer_ioctl_release(struct inode *node, struct file *filp)  /* close */
{
...
static stlayer_ioctl_dev_t *table[16];


/*=============================================================================

   stlayer_ioctl_NewNode
  ...
  ===========================================================================*/
stlayer_ioctl_dev_t *stlayer_ioctl_NewNode(unsigned int id)
{
    /* Check valid id */
    ...
    down(&(stlayer_ioctl_lock)); /* We can deshedule here !!! */
    ...
    if (NULL == table[id]) {
        ...
        table[id] = (stlayer_ioctl_dev_t*)kmalloc(sizeof(stlayer_ioctl_dev_t), GFP_KERNEL);
    ...
    up(&(stlayer_ioctl_lock));  /* Release the global lock */
...
/*=============================================================================
   ...
   stlayer_ioctl_GetNode

   Get an existing data structure for this 'device' (minor number)

  ===========================================================================*/
stlayer_ioctl_dev_t *stlayer_ioctl_GetNode(unsigned int id)
{
    /* Check valid id */
