                            ...
                            const  BOOL Apply)
{
    ...
    else
    {
        ...
        ScanValue = (U16)(Scan >> 3) ;
        HAL_Write8((U8 *)((HALLAYER_LayerProperties_t *)LayerHandle)->
                VideoDisplayBaseAddress_p + VID_SCN_8, ScanValue & VID_SCN_MASK);

        SaveBits = HAL_Read8((U8 *)((HALLAYER_LayerProperties_t *)LayerHandle)->
                VideoDisplayBaseAddress_p + VID_SCN_YDO_8);
        tmp = ~(SaveBits & 0x30);
        ScanValue =  (SaveBits & 0xC1) | (tmp & 0x30) | ( (Scan & 0x07)<<1 ) ;
        HAL_Write8((U8 *)((HALLAYER_LayerProperties_t *)LayerHandle)->
                VideoDisplayBaseAddress_p + VID_SCN_YDO_8, (U8)ScanValue );
    }
}
