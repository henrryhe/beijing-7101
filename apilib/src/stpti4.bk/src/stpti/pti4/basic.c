...
ST_ErrorCode_t stptiHAL_FlushSignalsForBuffer(FullHandle_t FullSignalHandle, FullHandle_t FullBufferHandle)
{
    ...
    */
    if (Queue_p != NULL)
    {
        ...
        while ((buffer_p = message_receive_timeout(Queue_p, TIMEOUT_IMMEDIATE)) != NULL)
        {
            TmpBufferHandle[MessageIndex].word = buffer_p[0];     /* get handle */
            message_release(Queue_p, buffer_p);     /* before we throw the message away */
...
ST_ErrorCode_t stptiHAL_FilterAllocateSection(FullHandle_t DeviceHandle, STPTI_FilterType_t FilterType, U32 *SFIdent)
{
    ...
    if (filter == TC_Params_p->TC_NumberSectionFilters)
    {
...
ST_ErrorCode_t stptiHAL_SlotUnLinkGeneral(FullHandle_t SlotHandle, FullHandle_t BufferHandle)
{
    ...
    if (SlotListHandle.word != STPTI_NullHandle())
    {
        ...
        if (UsedHandles == 1)
        {
            ...
            STSYS_WriteTCReg16LE((void*)&DMAConfig_p->SignalModeFlags, TC_DMA_CONFIG_SIGNAL_MODE_FLAGS_SIGNAL_DISABLE | TC_DMA_CONFIG_SIGNAL_MODE_TYPE_NO_SIGNAL);
        }
    }
    return Error;
}
...
ST_ErrorCode_t stptiHAL_SlotLinkToBuffer(FullHandle_t FullSlotHandle, FullHandle_t FullBufferHandle, BOOL DMAExists)
{
    ...
    else
    {
        ...
        if (dma < TC_Params_p->TC_NumberDMAs)
        {
            ...
            {
                if(SlotStruct_p->Flags.SoftwareCDFifo) /*HARDWIRE AS IF SWCDFIFO FLAG EXISTS AND IS ON!*/
                {
...
ST_ErrorCode_t stptiHAL_SlotClearPid(FullHandle_t SlotHandle, BOOL WindbackDMA)
{
...
#if !defined (STPTI_BSL_SUPPORT)
    if ( stptiMemGet_Slot(SlotHandle)->Pid == STPTI_WildCardPid() )
    {
        STSYS_ClearTCMask16LE((void*)&TC_Global_p->GlobalModeFlags, TC_GLOBAL_DATA_WILDCARD_PID_SET );
    }

    if ( stptiMemGet_Slot(SlotHandle)->Pid == STPTI_NegativePid() )
    {
...
ST_ErrorCode_t stptiHAL_SlotSetPid(FullHandle_t SlotHandle, U16 Pid, U16 MapPid)
{
    ...
    if ( Pid == STPTI_WildCardPid() )
    {
        ...
        STSYS_SetTCMask16LE((void*)&TC_Global_p->GlobalModeFlags, (TC_GLOBAL_DATA_WILDCARD_PID_SET) );
    }
...
void stptiHAL_SignalSetup(FullHandle_t FullBufferHandle)
{
    ...
    if (BufferStruct_p->SlotListHandle.word != STPTI_NullHandle() )
    {
        ...
        if (Error == ST_NO_ERROR)
        {
            ...
            /* this clears the signalling type but leaves the TC_DMA_CONFIG_SIGNAL_MODE_FLAGS_SIGNAL_DISABLE unchanged */
    ...
    {
        /* Signalling is not enabled until a Signal is associated */
        STSYS_WriteTCReg16LE((void*)&DMAConfig_p->SignalModeFlags, TC_DMA_CONFIG_SIGNAL_MODE_TYPE_NO_SIGNAL);
    }
    ...
    if ((STSYS_ReadRegDev16LE((void*)&DMAConfig_p->SignalModeFlags) & TC_DMA_CONFIG_SIGNAL_MODE_FLAGS_SIGNAL_DISABLE) &&
        (STSYS_ReadRegDev16LE((void*)&DMAConfig_p->DMAQWrite_p) != STSYS_ReadRegDev16LE((void*)&DMAConfig_p->DMARead_p)))
    {
        stptiHelper_SignalBuffer(FullBufferHandle, 0XBBBBBBBB);
    }
}
...
#ifdef STPTI_BUFFER_PTR_DEBUG
    printk(KERN_ALERT"Read_p        0x%08x\n",Read_p);
    printk(KERN_ALERT"Write_p       0x%08x\n",Write_p);
    printk(KERN_ALERT"Top_p         0x%08x\n",Top_p);
    printk(KERN_ALERT"Base_p        0x%08x\n",Base_p);
    printk(KERN_ALERT"BytesInBuffer 0x%08x\n",BytesInBuffer);
#endif
...
ST_ErrorCode_t stptiHAL_SlotQuery(FullHandle_t SlotHandle, BOOL *PacketsSeen, BOOL *TransportScrambledPacketsSeen, BOOL *PESScrambledPacketsSeen)
{
    ...
    STPTI_CRITICAL_SECTION_BEGIN {
        ...
        *PacketsSeen = FALSE;
        ...
        if (STSYS_ReadRegDev16LE((void*)&MainInfo_p->SlotState) == TC_MAIN_INFO_SLOT_STATE_SCRAMBLED)
        {
            STTBX_Print(("\n\nstptiHAL_DVBSlotQuery->We have a scrambled state in MainInfo\n\n"));
            if (STSYS_ReadRegDev16LE((void*)&MainInfo_p->SlotState) == TC_MAIN_INFO_SLOT_STATE_TRANSPORT_SCRAMBLED)
            {
