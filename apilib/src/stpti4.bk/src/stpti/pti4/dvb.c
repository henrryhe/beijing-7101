    ...
    *DataSize_p = 0;
    ...
    for (Packet = 0; Packet < ((stptiMemGet_Buffer(FullBufferHandle)->MultiPacketSize) & 0xffff); ++Packet)
    {
        ...
        else
        {
            ...
            {
                /* PMC 09/04/02 Changed from U16 to U32 as indeterminate lenght PES can exceed 16 bit sizes. */
                U32 PacketSize;
                ...
                if (PacketSize == 0)
                {
                    ...
                    while (End_p != Read_p)
                    {
                        ...
                        {                            
                            End_p = TRANSLATE_LOG_ADD(( (((U8 *) Base_p)[(EndIndex + 3) % BufferSize] << 24) |
                                                        (((U8 *) Base_p)[(EndIndex + 2) % BufferSize] << 16) |
                                                        (((U8 *) Base_p)[(EndIndex + 1) % BufferSize] <<  8) |
                                                        (((U8 *) Base_p)[ EndIndex      % BufferSize])),
                                                        stptiMemGet_Buffer(FullBufferHandle)->Start_p );
#if defined(ST_OSLINUX)
                            /* Need to calculate as a linux kernel virtual address */
...
ST_ErrorCode_t stptiHAL_DVBFiltersFlush(FullHandle_t FullBufferHandle, STPTI_Filter_t FilterHandles[], U32 NumberOfFilters)
{
    ...
    U32 Read_p   = TRANSLATE_LOG_ADD(DMAConfig_p->DMARead_p, stptiMemGet_Buffer(FullBufferHandle)->Start_p);
    U32 Write_p  = TRANSLATE_LOG_ADD(DMAConfig_p->DMAWrite_p, stptiMemGet_Buffer(FullBufferHandle)->Start_p);
    U32 QWrite_p = TRANSLATE_LOG_ADD(DMAConfig_p->DMAQWrite_p, stptiMemGet_Buffer(FullBufferHandle)->Start_p);
    U32 Top_p    = TRANSLATE_LOG_ADD((DMAConfig_p->DMATop_p & ~0X0F)+ 16, stptiMemGet_Buffer(FullBufferHandle)->Start_p);
    U32 Base_p   = TRANSLATE_LOG_ADD(DMAConfig_p->DMABase_p, stptiMemGet_Buffer(FullBufferHandle)->Start_p);
    ...
    while (Read_p != Write_p)
    {
        ...
        if (TC_Params_p->TC_AutomaticSectionFiltering != TRUE)
        {
            ((U8 *) Base_p)[(ReadBufferIndex + 0) % BufferSize] &= ~FilterMask[0];
            ((U8 *) Base_p)[(ReadBufferIndex + 1) % BufferSize] &= ~FilterMask[1];
            ((U8 *) Base_p)[(ReadBufferIndex + 2) % BufferSize] &= ~FilterMask[2];
            ((U8 *) Base_p)[(ReadBufferIndex + 3) % BufferSize] &= ~FilterMask[3];
            ((U8 *) Base_p)[(ReadBufferIndex + 4) % BufferSize] &= ~FilterMask[4];
            ((U8 *) Base_p)[(ReadBufferIndex + 5) % BufferSize] &= ~FilterMask[5];
            ((U8 *) Base_p)[(ReadBufferIndex + 6) % BufferSize] &= ~FilterMask[6];
            ((U8 *) Base_p)[(ReadBufferIndex + 7) % BufferSize] &= ~FilterMask[7];
        }
