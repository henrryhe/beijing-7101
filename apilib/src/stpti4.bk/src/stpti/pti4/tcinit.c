...
void stptiHelper_TCInit_SessionInfoModeFlags( FullHandle_t DeviceHandle )
{
       ...
       )
    {
        ...
        STSYS_SetTCMask16LE ((void*)&SessionInfo_p->SessionModeFlags,(TC_GLOBAL_DATA_DISCARD_SYNC_BYTE_SET));
    ...
    {
        STSYS_ClearTCMask16LE((void*)&SessionInfo_p->SessionModeFlags,(TC_GLOBAL_DATA_DISCARD_SYNC_BYTE_SET));
...
ST_ErrorCode_t stptiHelper_TCInit_InterruptDMA( FullHandle_t DeviceHandle ) /* Initialise 'Interrupt' DMA structure */
{
...
#if !defined(ST_OSLINUX)
    ST_Partition_t         *NcachePartition_p      =  Device_p->NonCachedPartition_p;
#endif /* #endif !defined(ST_OSLINUX) */
    TCPrivateData_t        *PrivateData_p          = &Device_p->TCPrivateData;
    STPTI_TCParameters_t   *TC_Params_p            =   (STPTI_TCParameters_t *) &PrivateData_p->TC_Params;
    ...
    {
        DMABufferSize = stptiHelper_BufferSizeAdjust(sizeof(TCStatus_t) * NO_OF_DVB_STATUS_BLOCKS);
    }

    /* Ensure the buffersize is a multiple of 16 */
    if (DMABufferSize & 0X0F)
        DMABufferSize = (DMABufferSize & ~0X0F) + 0X0010;
        ...
        base_p = (U8 *)dma_alloc_coherent(NULL,
                                          ...
                                          GFP_KERNEL);
#else
    base_p = (U8 *) memory_allocate(NcachePartition_p, DMABufferSize + DMAScratchAreaSize);
#endif /* #endif defined(ST_OSLINUX) */
    if (base_p == NULL)
    {
    ...
    memset (base_p, 0, DMABufferSize + DMAScratchAreaSize);
    ...
    base_p = (U8*)TRANSLATE_PHYS_ADD(stptiHelper_BufferBaseAdjust(base_p));
...
ST_ErrorCode_t stptiHelper_TCInit_AllocPrivateData( FullHandle_t DeviceHandle )
{
    ...
    {
        PrivateData_p->IndexHandles_p = memory_allocate(Partition_p, sizeof(STPTI_Index_t) * TC_Params_p->TC_NumberIndexs);
    ...
    {
        PrivateData_p->BufferHandles_p = memory_allocate(Partition_p, sizeof(STPTI_Buffer_t) * TC_Params_p->TC_NumberDMAs);
    ...
    if (TC_Params_p->TC_NumberSlots > 0)
    {
        PrivateData_p->SlotHandles_p = memory_allocate(Partition_p, sizeof(STPTI_Slot_t) * TC_Params_p->TC_NumberSlots);
    ...
    if (TC_Params_p->TC_NumberSlots > 0)
    {
        PrivateData_p->PCRSlotHandles_p = memory_allocate(Partition_p, sizeof(STPTI_Slot_t) * TC_Params_p->TC_NumberSlots);
    ...
    {
        PrivateData_p->SectionFilterHandles_p = memory_allocate(Partition_p, sizeof(STPTI_Filter_t) * TC_Params_p->TC_NumberSectionFilters);
    ...
    if (TC_Params_p->TC_NumberSectionFilters > 0)
    {
        PrivateData_p->SectionFilterStatus_p = memory_allocate(Partition_p, sizeof(STPTI_Slot_t) * TC_Params_p->TC_NumberSectionFilters);
    ...
    {
        PrivateData_p->PesFilterHandles_p = memory_allocate(Partition_p, sizeof(STPTI_Filter_t) * TC_Params_p->TC_NumberPesFilters);
    ...
    {
        PrivateData_p->TransportFilterHandles_p = memory_allocate(Partition_p, sizeof(STPTI_Filter_t) * TC_Params_p->TC_NumberTransportFilters);
    ...
    if (TC_Params_p->TC_NumberSlots > 0)
    {
        PrivateData_p->PesStreamIdFilterHandles_p = memory_allocate(Partition_p, sizeof(STPTI_Filter_t) * TC_Params_p->TC_NumberSlots);
    ...
    {
        PrivateData_p->DescramblerHandles_p = memory_allocate(Partition_p, TC_Params_p->TC_SizeOfDescramblerKeys * TC_Params_p->TC_NumberDescramblerKeys);
    ...
    if (TC_Params_p->TC_NumberSlots > 0)
    {
        PrivateData_p->SeenStatus_p = memory_allocate(Partition_p, sizeof(U8) * TC_Params_p->TC_NumberSlots);
    ...
    PrivateData_p->TCUserDma_p = memory_allocate(Partition_p, sizeof( TCUserDma_t ) );
