...
void stpti_BufferCacheInvalidate(STPTI_Buffer_t BufferHandle, void *WritePointer)
{
    ST_ErrorCode_t Error = ST_NO_ERROR;
    FullHandle_t FullBufferHandle;
    ...
    if ( Error == ST_NO_ERROR )
    {
        ...
        U32 Write_p    =  (U32)WritePointer;
        U32 Read_p     =  TRANSLATE_LOG_ADD(DMAConfig_p->DMARead_p, Buffer_p->Start_p);
        U32 Top_p      =  TRANSLATE_LOG_ADD((DMAConfig_p->DMATop_p & ~0xf) + 16, Buffer_p->Start_p);
        U32 Base_p     =  TRANSLATE_LOG_ADD(DMAConfig_p->DMABase_p, Buffer_p->Start_p);

        if (Read_p > Write_p) /* Wrap around case */
        {
            dma_cache_inv(phys_to_virt(Read_p), Top_p - Read_p);
            dma_cache_inv(phys_to_virt(Base_p), Write_p - Base_p);
        }
        else
        {
            dma_cache_inv(phys_to_virt(Read_p), Write_p - Read_p);
        }
    }
    else
        STTBX_Print(( "STPTI: invalid buffer handle 0x%08x\n", BufferHandle));
