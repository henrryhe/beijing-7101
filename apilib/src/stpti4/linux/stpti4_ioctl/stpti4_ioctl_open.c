...
/*=============================================================================
  ...
  ===========================================================================*/
int stpti4_ioctl_open(struct inode *node, struct file *filp)
{
 ...
 /* int major = imajor(node);         *//* The major number */
    ...
    if (NULL == filp->private_data) {
        ...
        /* stpti4_ioctl_NewNode() may deshedule - call must be reentrant */
        filp->private_data = stpti4_ioctl_NewNode(minor);
...
#if defined(STPTI4_PRINT_OPEN_ACCMODE)    
...
/*=============================================================================
   ...
   stpti4_ioctl_release   (close)
   
   This function can deschedule and MUST be reentrant.
   
  ===========================================================================*/
int stpti4_ioctl_release(struct inode *node, struct file *filp)  /* close */
{
...
static stpti4_ioctl_dev_t *table[16];


/*=============================================================================

   stpti4_ioctl_NewNode
  ...
  ===========================================================================*/
stpti4_ioctl_dev_t *stpti4_ioctl_NewNode(unsigned int id)
{
    /* Check valid id */
    ...
    down(&(stpti4_ioctl_lock)); /* We can deshedule here !!! */
    ...
    if (NULL == table[id]) {
        ...
        table[id] = (stpti4_ioctl_dev_t*)kmalloc(sizeof(stpti4_ioctl_dev_t), GFP_KERNEL);
    ...
    up(&(stpti4_ioctl_lock));  /* Release the global lock */
...
/*=============================================================================
   ...
   stpti4_ioctl_GetNode
   
   Get an existing data structure for this 'device' (minor number)
   
  ===========================================================================*/
stpti4_ioctl_dev_t *stpti4_ioctl_GetNode(unsigned int id)
{
    /* Check valid id */
