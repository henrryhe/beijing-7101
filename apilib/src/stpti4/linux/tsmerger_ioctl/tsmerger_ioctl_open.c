...
/*=============================================================================
  ...
  ===========================================================================*/
int tsmerger_ioctl_open(struct inode *node, struct file *filp)
{
 ...
 /* int major = imajor(node);         *//* The major number */
    ...
    if (NULL == filp->private_data) {
        ...
        /* tsmerger_ioctl_NewNode() may deshedule - call must be reentrant */
        filp->private_data = tsmerger_ioctl_NewNode(minor);
...
#if defined(TSMERGER_PRINT_OPEN_ACCMODE)
...
/*=============================================================================
   ...
   tsmerger_ioctl_release   (close)
   
   This function can deschedule and MUST be reentrant.
   
  ===========================================================================*/
int tsmerger_ioctl_release(struct inode *node, struct file *filp)  /* close */
{
...
static tsmerger_ioctl_dev_t *table[16];


/*=============================================================================

   tsmerger_ioctl_NewNode
  ...
  ===========================================================================*/
tsmerger_ioctl_dev_t *tsmerger_ioctl_NewNode(unsigned int id)
{
    /* Check valid id */
    ...
    down(&(tsmerger_ioctl_lock)); /* We can deshedule here !!! */
    ...
    if (NULL == table[id]) {
        ...
        table[id] = (tsmerger_ioctl_dev_t*)kmalloc(sizeof(tsmerger_ioctl_dev_t), GFP_KERNEL);
    ...
    up(&(tsmerger_ioctl_lock));  /* Release the global lock */
...
/*=============================================================================
   ...
   tsmerger_ioctl_GetNode
   
   Get an existing data structure for this 'device' (minor number)
   
  ===========================================================================*/
tsmerger_ioctl_dev_t *tsmerger_ioctl_GetNode(unsigned int id)
{
    /* Check valid id */
