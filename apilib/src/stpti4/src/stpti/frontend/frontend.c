...
ST_ErrorCode_t stptiHAL_FrontendCloneInput( FullHandle_t FullFrontendHandle, FullHandle_t FullPrimaryFrontendHandle )
{
    ...
    TsgdmaHAL_WritePTILiveDestination(Device_p->StreamID, Device_p->TCDeviceAddressPhysical_p);
...
ST_ErrorCode_t stptiHAL_TSINLinkToPTI( FullHandle_t FullFrontendHandle, FullHandle_t FullSessionHandle )
{
    ...
    /* Clear the DREQ counter */
    StfeHal_ClearDREQLevel( Device_p->StreamID );
...
ST_ErrorCode_t stptiHAL_FrontendUnlink( FullHandle_t FullFrontendHandle, STPTI_StreamID_t StreamID )
{
    ...
    case ( STPTI_FRONTEND_TYPE_TSIN ):
        TsgdmaHAL_ClearPTILiveDestination(Device_p->StreamID, Device_p->TCDeviceAddressPhysical_p);
        StfeHal_Enable ( StreamID, FALSE);
        TsgdmaHAL_ClearPTILiveDestination(Device_p->StreamID, Device_p->TCDeviceAddressPhysical_p);
        break;
...
ST_ErrorCode_t stptiHAL_FrontendInjectData( FullHandle_t FullFrontendHandle, STPTI_FrontendSWTSNode_t *FrontendSWTSNode_p, U32 NumberOfSWTSNodes, STPTI_StreamID_t StreamID )
{
    ...
    {
        if(stptiMemGet_Session(FullFrontendHandle)->NonCachedPartition_p==NULL)
        {
            Partition_p = stptiMemGet_Session( FullFrontendHandle )->DriverPartition_p;
        }
        else
        {
            Partition_p = stptiMemGet_Session( FullFrontendHandle )->NonCachedPartition_p;
        }

        /* allocate some memory for the TSGDMA linked list */
        SWTS_NodeMemory_p = (U32 *) STOS_MemoryAllocate(Partition_p, (sizeof(TSGDMA_SWTSNode_t) * NumberOfSWTSNodes +32) );
        ...
        if(Error == ST_NO_ERROR)
        {
            ...
            else
            {
                ...
                {
                    /* DVB is always a fixed size */
                    PacketLength = DVB_TS_PACKET_LENGTH;
                }

                BaseAddress_p =(STPTI_DevicePtr_t) Device_p->TCDeviceAddressPhysical_p;
                ...
                if (BaseAddress_p != NULL)
                {
                    ...
                    for (i=0; i<NumberOfSWTSNodes; i++)
                    {
                        ...
                        stpti_FlushRegion((void*) &TSGSWTSNode_p[i], sizeof(STPTI_FrontendSWTSNode_t));              /* flush the cache for the node */
#if defined(STAPIREF_COMPAT)
                        stpti_FlushRegion((void*) FrontendSWTSNode_p[i].Data_p, FrontendSWTSNode_p[i].SizeOfData);   /* Flush ram for injected data */
#else
                        /* STFAE - We do not flush because it could create some crash                                           */
                        /* STFAE - It is the responsibility of the application to provide a buffer not dirty 'no pb with stprm) */
            ...
            STOS_MemoryDeallocate(Partition_p, SWTS_NodeMemory_p);
        }
    }
