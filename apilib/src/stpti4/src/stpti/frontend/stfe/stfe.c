...
#define INTERNAL_RAM_BASE_ADDRESS                   (STFE_INTERNAL_RAM_BASE_ADDRESS)
...
void StfeHal_SetPid( U32 PidTableVirtStartAddr, U16 Pid )
{
    ...
    /* invalidate the cache for the pid */
    stpti_InvalidateRegion((void*) (PidTableVirtStartAddr + AddressOffset), sizeof(U32));
    ...
    stpti_FlushRegion((void*) (PidTableVirtStartAddr + AddressOffset), sizeof(U32));
}
...
/******************************************************************************
Function Name : StfeHal_ClearDREQLevel
  Description : Clear the number of DREQ requests in the queue
                This is important when reconfiguring the STFE/TSGDMA interface
                so that pkt syncronisation can be maintained.
                
   Parameters : StreamID
****************************************************************************/
void StfeHal_ClearDREQLevel( STPTI_StreamID_t StreamID)
{
    U32 Channel = StreamID & 0x0F;

    STTBX_Print(("StfeHal_ClearDREQLevel Previous level 0x%08x\n",STSYS_ReadRegDev32LE(DMA_BLOCK_REG_BUS_LEVEL(Channel))));

    STSYS_WriteRegDev32LE( DMA_BLOCK_REG_BUS_LEVEL(Channel) ,0x00000000);
}
...
void StfeHAL_IBRegisterDump(STPTI_StreamID_t StreamID)
{
    ...
    /* Current Memory Model prevents this code from working as is. */
#if 0
