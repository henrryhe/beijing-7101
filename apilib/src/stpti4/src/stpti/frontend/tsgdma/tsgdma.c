...
extern Driver_t STPTI_Driver;           /* Needed for device mapping */
...
void TsgdmaHAL_SlimCoreInit(void)
{
    ...
    if (TRUE == First)
    {
        ...
        /* Only load if the firmware if this is the first time the board has been powered */
        if ( !(0x01 & STSYS_ReadRegDev32LE((void*)TSGDMA_SLIM_CPU_RUN)) )
        {
...
void TsgdmaHAL_WritePTILiveDestination(STPTI_StreamID_t StreamID, STPTI_DevicePtr_t DestinationPTI)
{
    ...
    STTBX_Print(("TsgdmaHAL_WritePTILiveDestination 0x%08x 0x%08x 0x%08x\n", (U32)DestinationPTI,PTIA_BASE_ADDRESS,PTIB_BASE_ADDRESS ));
...
{
    U32 BitsPerMilliSec = 0;
    U32 BytesPerMilliSec = 0x7FFFFFFF;
    

    /* The Time Frame is set to 1ms so divide by a thousand to correct.

       Also the value set in the threshold register is set in bytes so
       we have to convert this figure by dividing by 8 
    */

    /* If the pace is set to zero then set to the default max rate */
    if ( PaceBps != 0 )
    {
        BitsPerMilliSec = PaceBps/(U32)1000;
        BytesPerMilliSec = BitsPerMilliSec/8;
    }
    STTBX_Print(("TsgdmaHAL_WriteSWTSPace StreamID 0x%04x Rate %u BytesPerMilliSec\n",StreamID, BytesPerMilliSec));

    STSYS_WriteRegDev32LE((void*) TSGDMA_BYTES_THRESHOLD((StreamID & 0x3F)), BytesPerMilliSec);
}
