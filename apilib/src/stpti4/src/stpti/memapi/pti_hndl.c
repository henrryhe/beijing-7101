...
ST_ErrorCode_t stpti_DeallocateDevice(FullHandle_t DeviceHandle, BOOL Force)
{
    ...
    /* --- Kill off the event and interrupt tasks as we are about the deallocate the device. --- */
    if ( LastDevice )
    {
...
#if defined( STPTI_FDMA_SUPPORT )
    STOS_MemoryDeallocate(stptiMemGet_Device(SessionHandle)->DriverPartition_p, OriginalFDMANode_p[stptiMemGet_Device(SessionHandle)->Session]);
#endif
...
ST_ErrorCode_t stpti_DeallocateSignal(FullHandle_t SignalHandle, BOOL Force)
{
    ...
    if (Error == ST_NO_ERROR)
    {
        ...
        if (SignalQueue_p != NULL)
        {
            ...
            {
                STPTI_Buffer_t *buffer_p = (STPTI_Buffer_t *)STOS_MessageQueueClaim (SignalQueue_p);

                *buffer_p = STPTI_NullHandle ();
                STOS_MessageQueueSend (SignalQueue_p, buffer_p);
        ...
        while (TRUE)
        {
            ...
            buffer_p = STOS_MessageQueueReceiveTimeout(SignalQueue_p, TIMEOUT_IMMEDIATE);
            if (buffer_p != NULL)
                STOS_MessageQueueRelease(SignalQueue_p, buffer_p);
            else
                break;
        }

        /* now it's safe to delete the queue */
        STOS_MessageQueueDelete(SignalQueue_p);
...
ST_ErrorCode_t stpti_DeallocateFrontend(FullHandle_t FrontendHandle, BOOL Force)
{
    ...
    if ( (stptiMemGet_Frontend(FrontendHandle))->Type == STPTI_FRONTEND_TYPE_TSIN )
    {
        ...
        if ( NULL != (stptiMemGet_Frontend(FrontendHandle))->RAMRealStart_p)
        {
            ...
            ST_Partition_t *partition = Session_p->NonCachedPartition_p;
            if( partition == NULL )
            {
                partition = Session_p->DriverPartition_p;
            }

            STOS_MemoryDeallocate( partition, (stptiMemGet_Frontend(FrontendHandle))->RAMRealStart_p );
        ...
        {
            STOS_MemoryDeallocate( Session_p->DriverPartition_p, (stptiMemGet_Frontend(FrontendHandle))->PIDTableRealStart_p  );
...

    {
        ...
        if( Addr ){
            ...
            if(!stptiMemGet_Buffer(BufferHandle)->ManualAllocation)
            {
                ...
                iounmap((void*)(stptiMemGet_Buffer(BufferHandle))->MappedStart_p);
#endif
                ...
                /* For legacy we use the NonCachedPartition, but for if this is not defined then we use the more efficient Cached DriverPartition */
                /* We will invalidate the cache during reads, and flush during writes. */
                ST_Partition_t *partition = stptiMemGet_Session( BufferHandle )->NonCachedPartition_p;
                if( partition == NULL )
                {
                    partition = stptiMemGet_Session( BufferHandle )->DriverPartition_p;
                }

                STOS_MemoryDeallocate( partition, (void *)Addr);
#endif
            ...
            {
                /* manually allocated iomapped buffer so unmap the buffer */
