...
ST_ErrorCode_t stpti_MemInit(ST_Partition_t *Partition, size_t MemSize, size_t HndlSize, MemCtl_t *MemCtl)
{
    ...
    MemCtl->FreeList_p = (Cell_t *) STOS_MemoryAllocate(Partition, MemSize);
    if (MemCtl->FreeList_p == NULL)
    {
        return ST_ERROR_NO_MEMORY;
    }

    MemCtl->Handle_p = (Handle_t *) STOS_MemoryAllocate(Partition, HndlSize);
    
    if (MemCtl->Handle_p == NULL)
    {
        STOS_MemoryDeallocate(Partition, (void *) MemCtl->FreeList_p);
        return ST_ERROR_NO_MEMORY;
    }
...
#endif
    {
        STOS_MemoryDeallocate(MemCtl->Partition_p, (void *) MemCtl->Block_p);
        
        STOS_MemoryDeallocate(MemCtl->Partition_p, (void *) MemCtl->Handle_p);
