...
inline static int AddIntStatus( __u32 DeviceHandle, __u32 EventStatus, __u32 Status )
{
    ...
    U32 NextWriteIndex_plus_one = ( InterruptCtl.WriteIndex + 2 ) % INTERRUPT_QUEUE_SIZE;
    U32 NextWriteIndex = ( InterruptCtl.WriteIndex + 1 ) % INTERRUPT_QUEUE_SIZE;
    ...
    /* Check for over flow */
    if( NextWriteIndex == InterruptCtl.ReadIndex )
    {
        /* We have no space free (we will have already reported an INTERRUPT FAIL prior to this) */
        return -1;
    }
    else if( NextWriteIndex_plus_one == InterruptCtl.ReadIndex )
    {
        /* We only have one space free so let report an INTERRUPT FAIL */
        InterruptData_p->EventStatus = STPTI_EVENT_INTERRUPT_FAIL_EVT;
        InterruptData_p->Status = 0;
        InterruptCtl.WriteIndex = NextWriteIndex;
...
BackendInterruptData_t *stptiHAL_WaitForInterrupt( BackendInterruptData_t *BackendInterruptData_p )
{
    ...
    else
    {
        ...
        {
            case 0:     /* A valid interrupt */
                break;

            case STPTI_EVENT_INTERRUPT_FAIL_EVT:
            {                
                /* STPTI_EVENT_INTERRUPT_FAIL_EVT means many things!!  This STPTI_EVENT_INTERRUPT_FAIL_EVT is because  */
                /* AddIntStatus() couldn't add a interrupt to it's "queue".  This is *not* an INTERRUPT_FAIL caused by */
                /* a status block overflow - hence why we clear BackendInterruptData_p->InterruptStatus!               */
        ...
        /* Advance ReadIndex */
        InterruptCtl.ReadIndex  = (InterruptCtl.ReadIndex + 1) % INTERRUPT_QUEUE_SIZE ;
    }
...
STOS_INTERRUPT_DECLARE(stptiHAL_InterruptHandler, DeviceHandle)
{
    ...
    
    {
        ...
        for (channel=0; channel<NUMBER_OF_CHANNELS; channel++)
        {
            ...
            {
                /* STFAE - Add trace to detect ts merger ram overflow */
/*#define TRACE_PRINT*/
#if defined(STFAE)&&defined(TRACE_PRINT)
                static U32 NbRamOverflows=0;
                NbRamOverflows++;
                TRACE_Print("STPTI4: TS Merger ram overflow on channel %d - Nb Ram overflows = %d!\n",channel-1,NbRamOverflows);
#endif
    ...
    if ( ( !FoundTSMERGERInterrupt ) || ( InterruptStatus != 0 ) )
    {
        ...
        if (((InterruptStatus & INTERRUPT_STATUS_VALID) == InterruptStatus) ||  ReportInvalidInterrupts)
        {
            ...
            BackendInterruptData_p = STOS_MessageQueueClaimTimeout(STPTI_Driver.InterruptQueue_p, TIMEOUT_IMMEDIATE);
...
#if (MESSAGE_QUEUE_SUCCESS_THRESHOLD > 0)       /* avoid warning */
