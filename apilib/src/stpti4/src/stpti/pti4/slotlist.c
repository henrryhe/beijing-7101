...
ST_ErrorCode_t stptiHelper_SlotList_Init( FullHandle_t DeviceHandle )
{
    ...
    PrivateData_p->SlotList = STOS_MemoryAllocate( Partition_p, sizeof( SlotList_t ) * TC_Params_p->TC_NumberOfSessions );
...
ST_ErrorCode_t stptiHelper_SlotList_Term( FullHandle_t DeviceHandle )
{
    ...
    /* Prevent compiler warning with STOS_MemoryDeallocate() for platforms which ignore the first parameter */
    Partition_p = Partition_p;

    if ( PrivateData_p->SlotList != NULL )
        STOS_MemoryDeallocate( Partition_p, PrivateData_p->SlotList );
