...
#define CAM_CFG_128_FILTERS     (0x0028)
...
ST_ErrorCode_t TcCam_Initialize( FullHandle_t DeviceHandle )
{
    ...
    PrivateData_p->FilterList = STOS_MemoryAllocate( Partition_p, sizeof( FilterList_t ) );
    ...
    PrivateData_p->FilterList->SessionFilterList = STOS_MemoryAllocate( Partition_p, sizeof( SessionFilterList_t ) * TC_Params_p->TC_NumberOfSessions );
    if ( PrivateData_p->FilterList->SessionFilterList == NULL )
    {
        STTBX_Print(("FAIL!\n"));
        STOS_MemoryDeallocate( Partition_p, PrivateData_p->FilterList );
        return( ST_ERROR_NO_MEMORY );
    }
    STTBX_Print(("ok\n"));
...
ST_ErrorCode_t TcCam_Terminate( FullHandle_t DeviceHandle )
{
    ...
    if ( PrivateData_p->FilterList != NULL )    /* trust that this is the last session! */
    {
        if ( PrivateData_p->FilterList->SessionFilterList != NULL )
        {
            STOS_MemoryDeallocate( Partition_p, PrivateData_p->FilterList->SessionFilterList );
        }
...
ST_ErrorCode_t TcCam_FilterAllocateSession( FullHandle_t DeviceHandle )
{
    ...
    if ( SessionFilterList[session].State == FL_FREE )
    {
        ...
        else if ( numfilters <= 96  )  cam_Config = CAM_CFG_96_FILTERS;
        else if ( numfilters <= 128 )  cam_Config = CAM_CFG_128_FILTERS;
        else
        {
                                  ...
                                  BOOL DSS_MPT_Mode )
{
    ...
    /* RAMCAM */
#if defined ( STPTI_ARCHITECTURE_PTI4_LITE ) || (defined (STPTI_ARCHITECTURE_PTI4_SECURE_LITE ) && (STPTI_ARCHITECTURE_PTI4_SECURE_LITE == 1))
...
#endif
        {
            ...
            for (mask = 1; (mask != 0); mask <<= 1)
            {
                if (MatchedFilterMask[camstatus] & mask)
                {
                    ...
                    {
                        /* We need to convert the long mode filter number into a short mode filter convention  */
                        /* long mode filters occupy two short mode blocks, and filter number must be expressed */
                        /* short mode terms (remember there are 8 short mode filters per block).               */
                        /* e.g. 0-7 long -> 0-7 short,  8-15 long -> 16-23 short , 16-23 long -> 32-39 short   */
                        U32 block = (filter/SF_NUM_FILTERS_PER_BLOCK);
                        filter = block*SF_NUM_FILTERS_PER_BLOCK*2 + filter%SF_NUM_FILTERS_PER_BLOCK;
                    }

                    FilterHandle.word = PrivateData_p->SectionFilterHandles_p[FilterOffset+filter];

                    if ( ST_NO_ERROR == stpti_FilterHandleCheck( FilterHandle ) )
                    {
    ...
    else if(filtermode==TC_SESSION_INFO_FILTER_TYPE_NEG_MATCH)
    {
        ...
        for (mask = 1, filter = 0; (mask != 0); ++filter, mask <<= 1)
        {
            if (CombinedMatches & mask) /* found one that matched */
            {
                ...
                FilterHandle.word = PrivateData_p->SectionFilterHandles_p[FilterOffset+32+filter];
...
U32 TcCam_RelativeMetaBitMaskPosition(FullHandle_t FullFilterHandle)
{
    ...
    if (TC_FilterMode == TC_SESSION_INFO_FILTER_TYPE_SHORT)
    {
        if(RelativeFilterIdent%SF_NUM_FILTERS_PER_BLOCK < SF_CAMLINE_WIDTH)
        {
            /* CAMA */
            BitPosition=(RelativeFilterIdent/SF_NUM_FILTERS_PER_BLOCK)*4 + RelativeFilterIdent%SF_NUM_FILTERS_PER_BLOCK;
        }
        else
        {
            /* CAMB */
            RelativeFilterIdent=RelativeFilterIdent - SF_CAMLINE_WIDTH;
            BitPosition=(RelativeFilterIdent/SF_NUM_FILTERS_PER_BLOCK)*SF_CAMLINE_WIDTH + RelativeFilterIdent%SF_NUM_FILTERS_PER_BLOCK;
            BitPosition+=(TcCam_GetMetadataSizeInBytes()/2)*8;
        }
    }
    else if (TC_FilterMode == TC_SESSION_INFO_FILTER_TYPE_LONG)
    {
        RelativeFilterIdent = (RelativeFilterIdent/(SF_NUM_FILTERS_PER_BLOCK*2))*SF_NUM_FILTERS_PER_BLOCK + RelativeFilterIdent%SF_NUM_FILTERS_PER_BLOCK;
        if(RelativeFilterIdent%SF_NUM_FILTERS_PER_BLOCK < SF_CAMLINE_WIDTH)
        {
            /* CAMA */
            BitPosition=(RelativeFilterIdent/SF_NUM_FILTERS_PER_BLOCK)*4 + RelativeFilterIdent%SF_NUM_FILTERS_PER_BLOCK;
