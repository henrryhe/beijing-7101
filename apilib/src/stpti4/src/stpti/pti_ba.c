...
/* Private Variables ------------------------------------------------------- */
    ...
    STFDMA_GenericNode_t *FDMANode_p, *OriginalFDMANode_p[3];
...
Driver_t STPTI_Driver = {
                            ...
                            0,                                              /* Physical PTIs Initialised */
                            0, 0, 0,                                        /* Device Mapping */
                                    
#if defined(ST_OSLINUX)
                            NULL, NULL, EVENT_Q_STACK, 0,                   /* Event Q initialisations */
                            NULL, NULL, INT_Q_STACK, 0,                     /* Interrupt Q initialisations */
#else
                            NULL, NULL, EVENT_Q_STACK, MIN_USER_PRIORITY,   /* Event Q initialisations */
                            NULL, NULL, INT_Q_STACK,MIN_USER_PRIORITY,      /* Interrupt Q initialisations */
                                     ...
                                     STPTI_Buffer_t * BufferHandle )
{
    ...
    size_t ActualSize;
    Buffer_t TmpBuffer;
    U8 *BufferPhys_p = NULL;
    void *MappedBuffer_p = NULL, *Buffer_p = NULL;
    ...
    if ( Error == ST_NO_ERROR )
    {
        ...
        ActualSize = stptiHelper_BufferSizeAdjust( RequiredSize );
         ...
         */
        {
#if defined(ST_OSLINUX) && defined( USING_BIG_PHYSAREA )
            ...
            /* For legacy reasons we use the NonCachedPartition, but for if this is not defined then we use the more efficient Cached DriverPartition */
            /* We will invalidate the cache during reads, and flush during writes. */
            partition = stptiMemGet_Session( FullSessionHandle )->NonCachedPartition_p;
            if(partition == NULL )
            {
                partition = stptiMemGet_Session( FullSessionHandle )->DriverPartition_p;
            }

            Buffer_p = STOS_MemoryAllocate( partition, ActualSize + STPTI_BUFFER_ALIGN_MULTIPLE - 1 );
#endif /*  #if defined(ST_OSLINUX) && defined( USING_BIG_PHYSAREA ) ... #else ... #endif */
#endif
            ...
            {
                BufferPhys_p = stptiHelper_BufferBaseAdjust( STOS_VirtToPhys(Buffer_p) );
#if defined(ST_OSLINUX) && defined(STPTI_SUPPRESS_CACHING)
                MappedBuffer_p = ioremap_nocache( (U32)BufferPhys_p, ActualSize );
#else
                MappedBuffer_p = stptiHelper_BufferBaseAdjust(Buffer_p);
#endif
            }
        }
        ...
        if ( Buffer_p != NULL )
        {
            ...
            if ( Error == ST_NO_ERROR )
            {
                ...
                TmpBuffer.MappedStart_p           = (U8 *) MappedBuffer_p;      /* ioremapped buffer address. Used for CPU reads. */
                TmpBuffer.UserSpaceAddressDiff    = (U32)-1;
                TmpBuffer.Start_p                 = (U8 *) BufferPhys_p;        /* Physical Address */
                TmpBuffer.RealStart_p             = Buffer_p;                   /* (unmapped) Virtual Address from allocations */
            ...
            else        /* Must have been an error allocating the object */
            {
#if defined(ST_OSLINUX) && defined(USING_BIG_PHYSAREA)
                bigphysarea_free_pages( Buffer_p );
#else
                /* For legacy we use the NonCachedPartition, but for if this is not defined then we use the more efficient Cached DriverPartition */
                /* We will invalidate the cache during reads, and flush during writes. */
                ST_Partition_t *partition = stptiMemGet_Session( FullSessionHandle )->NonCachedPartition_p;
                if( partition == NULL )
                {
                    partition = stptiMemGet_Session( FullSessionHandle )->DriverPartition_p;
                }

                STOS_MemoryDeallocate( partition, Buffer_p);
#endif
#if defined(ST_OSLINUX) && defined(STPTI_SUPPRESS_CACHING)
                iounmap((void*)MappedBuffer_p);
#endif
            }
        }
        else
                          ...
                          const STPTI_InitParams_t *InitParams )
{
...
#if defined( ST_OSLINUX )
    /* We don't use partitions under linux, but this variable must be non null */
    Partition = (ST_Partition_t *)1;
#else
    Partition = InitParams->Partition_p;
#endif
	...
	STPTI_Driver.InterruptTaskPriority = InitParams->InterruptProcessPriority;
        ...
        Error = stpti_InterruptQueueInit( Partition );
...
#endif
        /* Partition passed for task_init GNBvd21068*/
         Error = stpti_EventQueueInit( Partition );
    ...
    if ( Error == ST_NO_ERROR )   /* Initialise memory for NSessions & Handles to them */
    {
            ...
            if ( Device_p != NULL )
            {
...
#endif
                STTBX_Print(("Device found on TCDevice 0x%08x requested TCDevice 0x%08x\n",(U32)Device_p->TCDeviceAddressPhysical_p, (U32)InitParams->TCDeviceAddress_p ));
                if ( Device_p->TCDeviceAddressPhysical_p == InitParams->TCDeviceAddress_p )
                {
                ...
                else
                {
                    STTBX_Print(( "STPTI: PTI cell address NO match ( current: 0x%08x ) ( InitParams: 0x%08x )\n",
                                       Device_p->TCDeviceAddressPhysical_p, InitParams->TCDeviceAddress_p ));
                }
#endif
            }
        ...
        if ( Error == ST_NO_ERROR )
        {
            Error = stpti_MemInit( Partition, ( sizeof( Session_t ) + sizeof( CellHeader_t )) * InitialSessions,
            ...
            if ( Error == ST_NO_ERROR )
            {
                ...
                else
                {
                    ...
                    TmpDevice.TCDeviceAddressPhysical_p  = InitParams->TCDeviceAddress_p;
                    ...
                    TmpDevice.DriverPartition_p          = Partition;
#if defined( ST_OSLINUX )
                    TmpDevice.NonCachedPartition_p       = NULL;
#else
                    ...
                    TmpDevice.IndexTimerTickCount        = 0;
#endif
                    TmpDevice.DiscardSyncByte            = FALSE;
                    TmpDevice.DiscardBytes               = 0;
                    ...
                    switch ( TmpDevice.SectionFilterOperatingMode )
                    {
                        ...
                        case STPTI_FILTER_OPERATING_MODE_ANYx16:
                    ...
                    else
                    {
                     ...
                     /* GPDMATransferParams.TimingModel              = STGPDMA_TIMING_FREE_RUNNING; */
                        ...
                        OriginalFDMANode_p[stptiMemGet_Device(TmpDevice.Handle)->Session] = STOS_MemoryAllocate( TmpDevice.DriverPartition_p,
                                                              sizeof( STFDMA_GenericNode_t ) + 31 );
                        FDMANode_p = (STFDMA_GenericNode_t *) ((((U32) OriginalFDMANode_p[stptiMemGet_Device(TmpDevice.Handle)->Session] ) +31 ) & ~31 );
                        FDMANode_p->Node.Next_p = NULL;
                        FDMANode_p->Node.SourceStride = 0;
                        FDMANode_p->Node.DestinationStride = 0;

                        memset((void *)(&FDMANode_p->Node.NodeControl), 0, sizeof( STFDMA_NodeControl_t ));

                        FDMANode_p->Node.NodeControl.PaceSignal = STFDMA_REQUEST_SIGNAL_NONE;
                        FDMANode_p->Node.NodeControl.SourceDirection = STFDMA_DIRECTION_INCREMENTING;
                        FDMANode_p->Node.NodeControl.DestinationDirection = STFDMA_DIRECTION_INCREMENTING;
                        FDMANode_p->Node.NodeControl.NodeCompleteNotify = FALSE;
                        FDMANode_p->Node.NodeControl.NodeCompletePause = FALSE;
                        FDMANode_p->Node.NodeControl.Reserved = 0;

                        FDMAGenericTransferParams.ChannelId = STFDMA_USE_ANY_CHANNEL;
                        FDMAGenericTransferParams.Pool = STFDMA_DEFAULT_POOL;
                        FDMAGenericTransferParams.NodeAddress_p = (STFDMA_GenericNode_t *) ((U32)STOS_VirtToPhys(FDMANode_p));
...
#endif
                        /* Update memory */
                        stpti_FlushRegion((void*) FDMANode_p, sizeof(STFDMA_GenericNode_t));
#endif
...
ST_ErrorCode_t STPTI_SlotLinkToBuffer( STPTI_Slot_t Slot, STPTI_Buffer_t Buffer )
{
    ...
    if ( Error == ST_NO_ERROR )
    {
        ...
        /* If Slot if already linked to a conventional (non-record) buffer then return an error to avoid running out of DMAs - 
           Note each slot can be linked to a max of one conventional buffer */
                           ...
                           STPTI_Handle_t * Handle )
{
    ...
    if ( Error == ST_NO_ERROR )
    {
        ...
        Partition = (ST_Partition_t *)1;
#else
        Partition = OpenParams->DriverPartition_p;
#endif
        ...
        if ( Error == ST_NO_ERROR )
        {
            ...
            if ( Error == ST_NO_ERROR )
            {
                ...
                TmpSession.DriverPartition_p    = Partition;
#if defined( ST_OSLINUX )
                TmpSession.NonCachedPartition_p = NULL;
#else
    ...
    if ( OldPid != TC_INVALID_PID )
    {
        ...
        if (( stptiMemGet_Slot( FullSlotHandle ))->Type == STPTI_SLOT_TYPE_PCR )
        {
            ...
            if ( ExistingSlot != STPTI_NullHandle() )
            {
                ...
                ( stptiMemGet_Slot( FullExistingSlotHandle ))->PCRReturnHandle =
                                                             STPTI_NullHandle();
                SlotIdentIfPCR = stptiMemGet_Slot( FullExistingSlotHandle )->TC_SlotIdent;
#if defined (STPTI_FRONTEND_HYBRID)
                PidStillInUse = TRUE;
#endif
            }
...
#endif /* #if !defined ( STPTI_BSL_SUPPORT ) */
        if ( ST_NO_ERROR != (Error = stptiHAL_SlotClearPid( FullSlotHandle, WindbackDMA, SlotIdentIfPCR )))
        {
...
static ST_ErrorCode_t stpti_Slot_ClearPCRSlot( FullHandle_t FullSlotHandle )
{
    ...
    if ( OldPid != TC_INVALID_PID )
    {
        if (( stptiMemGet_Slot( FullSlotHandle ))->Type == STPTI_SLOT_TYPE_PCR )
        {
            Error = stptiHAL_SlotClearPid( FullSlotHandle, TRUE, 0xFFFFFFFF );
            ( stptiMemGet_Slot( FullSlotHandle ))->PCRReturnHandle = STPTI_NullHandle();
        }
    }
...
{
    return( stptiHelper_BufferSizeAdjust(Size) );
}
...
{
    return( stptiHelper_BufferBaseAdjust(Base_p) );
}
