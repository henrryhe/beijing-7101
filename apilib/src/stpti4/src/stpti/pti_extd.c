...
/******************************************************************************
Function Name : STPTI_BufferPacketCount
  Description : Obtain DMA buffer packet count
...
ST_ErrorCode_t STPTI_BufferPacketCount(STPTI_Buffer_t BufferHandle, U32 *Count_p)
{
    ...
    Error = stptiValidate_BufferPacketCount( FullBufferHandle, Count_p);
    if (Error == ST_NO_ERROR)
    {
        Error = stptiHAL_BufferPacketCount(FullBufferHandle, Count_p);
    }
...
ST_ErrorCode_t STPTI_GetCurrentPTITimer(ST_DeviceName_t DeviceName, STPTI_TimeStamp_t * TimeStamp)
{
    ...
    /* To avoid deadlock when this function is called from an interrupt handler */
...
{  ST_ErrorCode_t Error = ST_NO_ERROR;
    ...
    /* If it isn't defined, then the user will be passing in a virtual address (kernel virtual if linux)*/
    MappedBuffer_p = (void *) Base_p;
    Base_p = STOS_VirtToPhys(Base_p);    /* convert pointer to a physical address */
#else 
    /* If STPTI_LEGACY_LINUX_SWCDFIFO then the user will be passing in a physical address for the buffer base, */
    /* and it will need Mapping below (after some checks) */
#endif
    ...
    if (Error == ST_NO_ERROR)
    {
        if(Base_p != NULL)
        {
            ...
            Temp_p = (U8 *) stpti_BufferBaseAdjust(Base_p);
            if((Base_p != Temp_p)||(ActualSize != RequiredSize))
            {
            ...
            {
#if defined(STPTI_LEGACY_LINUX_SWCDFIFO)
                /* A physical address was passed in so we map it to get a virtual one */
                /* STOS call avoided to keep backward compatibility with previous versions of STOS */
                MappedBuffer_p = ioremap_nocache((unsigned long)Base_p, ActualSize);
#endif
                ...
                if (Error == ST_NO_ERROR)
                {
                    ...
                    TmpBuffer.Start_p                 = Base_p;         /* Physical Address */
                    TmpBuffer.RealStart_p             = MappedBuffer_p;   /* Virtual Address (not actually used for Manual Alloc'd Buffers) */
...
/******************************************************************************
Function Name : STPTI_ObjectEnableFeature
  Description : Enable some feature for a given object.
   Parameters : ObjectHandle - A valid handle to a given object to have some feature enabled.
                FeatureInfo - A structure defining the feature to be enabled
******************************************************************************/
ST_ErrorCode_t STPTI_ObjectEnableFeature(STPTI_Handle_t ObjectHandle, STPTI_FeatureInfo_t FeatureInfo)
{
	ST_ErrorCode_t Error = ST_NO_ERROR;
	FullHandle_t FullObjectHandle;

	FullObjectHandle.word = ObjectHandle;

	STOS_SemaphoreWait(stpti_MemoryLock);

	Error = stptiValidate_ObjectEnableFeatures(FullObjectHandle, FeatureInfo);
	if (Error == ST_NO_ERROR)
	{
		Error = stptiHAL_ObjectEnableDisableFeatures(FullObjectHandle, FeatureInfo, TRUE);
	}
...
/******************************************************************************
Function Name : STPTI_ObjectDisableFeature
  Description : Disable some feature for a given object.
   Parameters : ObjectHandle - A valid handle to a given object to have some feature disabled.
...
ST_ErrorCode_t STPTI_ObjectDisableFeature(STPTI_Handle_t ObjectHandle, STPTI_Feature_t Feature)
{
	ST_ErrorCode_t Error = ST_NO_ERROR;
	FullHandle_t FullObjectHandle;
	STPTI_FeatureInfo_t FeatureInfo;

	FullObjectHandle.word 	= ObjectHandle;
	
	FeatureInfo.Feature 	= Feature;
	FeatureInfo.Params_p 	= NULL;
	...
	if (Error == ST_NO_ERROR)
	{
		Error = stptiHAL_ObjectEnableDisableFeatures(FullObjectHandle, FeatureInfo, FALSE);
	}
...
ST_ErrorCode_t STPTI_Debug(ST_DeviceName_t DeviceName, char *dbg_class, char *string, int string_size)
{
    ...
    else
    {
        ...
        /* Make sure that the string is a sensible size */
        if(string_size<2048)
        {
            Error = ST_ERROR_BAD_PARAMETER;
            if(string_size>=80)
            {
                chars_output = sprintf(string,"String size should be at least 2Kbytes (64Kbytes for TC Memory Dumps).\n");
            }
        }
    }
    ...
    {
        Error = stpti_GetDeviceHandleFromDeviceName(DeviceName, &DeviceHandle, TRUE);
        if (Error != ST_NO_ERROR) chars_output = sprintf(string,"Invalid Device Name %s\n", DeviceName);
    }
    
    if (Error == ST_NO_ERROR)
    {
        if(strcmp(dbg_class,"TCDevice_t")==0)
        {
            chars_output = stptiHAL_read_procregs( string, &string_start, 0, string_size, &eof, (void *)DeviceHandle.word );
        }
        else if(strcmp(dbg_class,"TCGlobalInfo_t")==0)
        {
            chars_output = stptiHAL_read_procglobal( string, &string_start, 0, string_size, &eof, (void *)DeviceHandle.word );
        }
        else if(strcmp(dbg_class,"Filters")==0)
        {
            chars_output = stptiHAL_read_procfilters( string, &string_start, 0, string_size, &eof, (void *)DeviceHandle.word );
        }
        else if(strcmp(dbg_class,"TCMainInfo_t")==0)
        {
            chars_output = stptiHAL_read_procmaininfo( string, &string_start, 0, string_size, &eof, (void *)DeviceHandle.word );
        }
        else if(strcmp(dbg_class,"TCSessionInfo_t")==0)
        {
            chars_output = stptiHAL_read_proc_sess( string, &string_start, 0, string_size, &eof, (void *)DeviceHandle.word );
        }
        else if(strcmp(dbg_class,"TCDMAConfig_t")==0)
        {
            chars_output = stptiHAL_read_proc_globaltcdmacfg( string, &string_start, 0, string_size, &eof, (void *)DeviceHandle.word );
        }
        else if(strcmp(dbg_class,"SlotLUT")==0)
        {
            chars_output = stptiHAL_read_proc_lut( string, &string_start, 0, string_size, &eof, (void *)DeviceHandle.word );
        }
        else if(strcmp(dbg_class,"TC_Data")==0)
        {
            chars_output = stptiHAL_read_proctcdata( string, &string_start, 0, string_size, &eof, (void *)DeviceHandle.word );
        }
        else if(strcmp(dbg_class,"STPTI_TCParameters_t")==0)
        {
            chars_output = stptiHAL_read_proc_tcparams( string, &string_start, 0, string_size, &eof, (void *)DeviceHandle.word );
        }
        else if(strcmp(dbg_class,"CAM")==0)
        {
            chars_output = stptiHAL_read_proc_cam( string, &string_start, 0, string_size, &eof, (void *)DeviceHandle.word );
        }
        else if(strcmp(dbg_class,"Objects")==0)
        {
            chars_output = stptiHAL_read_proc_objects( string, &string_start, 0, string_size, &eof, (void *)DeviceHandle.word );
        }
        else if(strcmp(dbg_class,"Misc")==0)
        {
            chars_output = stptiHAL_read_procmisc( string, &string_start, 0, string_size, &eof, (void *)DeviceHandle.word );
        }
        else if(strcmp(dbg_class,"IntHistory")==0)
        {
            U32 dummy;
            chars_output=0;
            Error = STPTI_DebugGetInterruptHistory ( DeviceName, &dummy, string, &chars_output);
        }
        else if(strcmp(dbg_class,"Statistics")==0)
        {
            STPTI_DebugStatistics_t dummy;
            chars_output=0;
            Error = STPTI_DebugGetStatistics( DeviceName, &dummy, string, &chars_output);
        }
        else
        {
            chars_output = sprintf(string,"Unrecognised class %s\n", dbg_class);
            Error = ST_ERROR_BAD_PARAMETER;
        }
    }
    ...
    if(string_size>=256)
    {
        (void) sprintf(string,"Recompile with STPTI_DEBUG_SUPPORT in STPTI_CONFIG environment variable\n");
    }
