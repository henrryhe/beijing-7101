...
/******************************************************************************
Function Name : STPTI_IndexChain
  Description :
   Parameters :
******************************************************************************/
...
{
    ST_ErrorCode_t Error = ST_NO_ERROR;
    FullHandle_t *IndexFullHandles = (FullHandle_t *) IndexHandles;
    
    Error = stptiValidate_IndexChain(IndexFullHandles, NumberOfHandles);
    if (Error != ST_NO_ERROR)
        return( Error );
    
    Error = stptiHAL_IndexChain(IndexFullHandles, NumberOfHandles);
...
ST_ErrorCode_t stpti_IndexQueueTerm(void)
{
    ...
    do
    {
        ...
        event_p = STOS_MessageQueueClaimTimeout(STPTI_Driver.IndexQueue_p, &time);
    }
    while (event_p == NULL);

    event_p->Source = FullNullHandle;
    STOS_MessageQueueSend(STPTI_Driver.IndexQueue_p, (void *) event_p);
    ...
    else
    {
        ...
        {
            /* delete the queue */
            STOS_MessageQueueDelete(STPTI_Driver.IndexQueue_p);
...
static void stpti_ProcessEvent(EventData_t *event_p)
{
    ...
    BOOL IndexEnable;
    STOS_MessageQueue_t *IndexQueue_p = STPTI_Driver.IndexQueue_p;
    ...
    {
        STOS_MessageQueueRelease(IndexQueue_p, (void *) event_p);
        return;
    }
...

    {
...

        {
            ...
            if (IndexListHandle.word != STPTI_NullHandle())
            {
                ...
                if (Index==MaxHandles)
                {
                    /* No indexes associated (any more) so lets not raise an event (probably a PCR handled elsewhere) */
                    IndexDefinition.word = 0;
                }
            }
            else
            {
                /* No indexes associated so lets not raise the event (probably a PCR handled elsewhere) */
                IndexDefinition.word = 0;
            }
        }
