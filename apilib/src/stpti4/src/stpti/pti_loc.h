...
/* In systems in which the host is tied up (context locked) for extended periods of time you may
   ...
   metadata between the tc and the  host.  NO_OF_DVB_STATUS_BLOCKS and NO_OF_DTV_STATUS_BLOCKS
   have been merged into one define NO_OF_STATUS_BLOCKS */
#define NO_OF_STATUS_BLOCKS  200
...
/* Maximum interrupt notifications is total number of status blocks (across multiple physical PTIs) 
   plus a few extra for interrupt fail events and transport stream errors */
 #define INTERRUPT_QUEUE_SIZE   ( NO_OF_STATUS_BLOCKS * STPTI_CORE_COUNT + 32 )
#endif
 ...
 #define SIGNAL_QUEUE_SIZE       64     /* some customers may need 64 */
#endif

/* BUFFER ALIGNMENT definitions (must be ST40/ST200 CACHE LINE aligned as well as DMA boundry aligned) */
#define STPTI_BUFFER_SIZE_MULTIPLE 0x20
#define STPTI_BUFFER_ALIGN_MULTIPLE 0x20
#define DMAScratchAreaSize       (STPTI_BUFFER_SIZE_MULTIPLE+STPTI_BUFFER_ALIGN_MULTIPLE)            /* Account for shift by stptiHelper_BufferBaseAdjust() and stptiHelper_BufferSizeAdjust() */
...
typedef volatile struct Driver_s
{
    ...
    U32                  Physical_PTIs_Mapped;
    
    U32                  STFE_Base_Address;
    U32                  STFE_Internal_RAM_Base_Address;
    U32                  TSGDMA_Base_Address;

    STOS_MessageQueue_t *EventQueue_p;
    task_t              *EventTaskId_p;
    size_t               EventTaskStackSize;
    S32                  EventTaskPriority;

    STOS_MessageQueue_t *InterruptQueue_p;
    task_t              *InterruptTaskId_p;
    size_t               InterruptTaskStackSize;
    S32                  InterruptTaskPriority;

#ifndef STPTI_NO_INDEX_SUPPORT
    STOS_MessageQueue_t *IndexQueue_p;
...
typedef volatile struct TCPrivateData_s
{
    ...
    void                 *InterruptBufferRealStart_p;
...
typedef volatile struct Device_s
{
    ...
    volatile U32                  *TCDeviceAddress_p;                       /* Virtual Address PTI (used for register writing) */
    volatile U32                  *TCDeviceAddressPhysical_p;               /* Physical Address PTI (used for comparisons) */
    ...
    BOOL                           IndexEnable;
    U8                             IndexTimerTickCount;                     /* Only used for STPTI_TIMER_TICK feature */
#endif
