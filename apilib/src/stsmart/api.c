...
#if 0
#if defined(ST_OSLINUX)
/* UART LINUX PIO Device Name */
static const ST_DeviceName_t TXDPioName = "UARTTXD_PIO";
static const ST_DeviceName_t RXDPioName = "UARTRXD_PIO";
static const ST_DeviceName_t RTSPioName = "UARTRTS_PIO";
static const ST_DeviceName_t CTSPioName = "UARTCTS_PIO";
#define STPIO_Close(Handle)    stpio_free_pin(Handle);
#endif
#endif
/* Driver revision number */
static const ST_Revision_t Revision = "STUART-REL_1.8.3";
                          ...
                          )
{
    ...
    UART_ControlBlock_t *Uart_p = NULL;
    BOOL TXRXInit = FALSE;
    BOOL RTSInit = FALSE;
    BOOL CTSInit = FALSE;
...
#pragma ST_device(ASCRegs_p)
            ...
            ST_ErrorCode_t PIOError = ST_NO_ERROR;
            volatile UART_ASCRegs_t *ASCRegs_p;
 ...
 /* Remap base address under Linux */
                Uart_p->MappingWidth = STUART_MAPPING_WIDTH;
                Uart_p->MappingBaseAddress = (unsigned long *)STOS_MapRegisters( (void *)Uart_p->InitParams.BaseAddress,
                                                                          Uart_p->MappingWidth,"UARTSMART" );
                if (Uart_p->MappingBaseAddress == NULL)
                {
                 ...
                 * for the buffer.
                 */
                if (error == ST_NO_ERROR && InitParams->SwFIFOEnable)
                {
                    Uart_p->ReceiveBufferBase =
...
#if defined(ST_OSLINUX)&&defined(API_DEBUG)
                        printk("KERNEL API: Request Uart IRQ ...  \n");
                        printk("KERNEL API: Interrupt Number %d \n", InitParams->InterruptNumber);
#endif
                        rc_irq = request_irq((int)(InitParams->InterruptNumber),
			   ...
			   if(STOS_InterruptInstall(InitParams->InterruptNumber,
					  ...
					  STOS_INTERRUPT_PROTOTYPE(UART_InterruptHandler),
					  NULL,
					  Uart_p) == 0)
...
#pragma ST_device(ASCRegs_p)
    ...
    UART_Operation_t        *ReadOperation = NULL;
            ...
            *NumberRead = 0;
            ...
            if (MinToRead > 0)
            {
                ...
                ReadOperation->Timeout = /* 1ms periods */
                   (TimeOut*ST_GetClocksPerSecond()) /1000;
                ...
                {
                    UART_GetDataFromSwFIFO(Uart_p, Buffer, MinToRead, MaxToRead,&Terminated, ReadOperation);
                }
                else
                {
...
#pragma ST_device(ASCRegs_p)
    ...
    if (Uart_p != NULL)
    {
        ...
        if (Uart_p->Handle == 0 || TermParams->ForceTerminate)
        {
            ...
            if (Uart_p->TXDHandle != (STPIO_Handle_t )NULL )
            {
                (void)STPIO_Close(Uart_p->TXDHandle);
                if ((!Uart_p->IsHalfDuplex) && (Uart_p->RXDHandle != (STPIO_Handle_t )NULL))
                {
            ...
            if (UART_IsRTSCTSSupported(Uart_p->InitParams.UARTType) && (Uart_p->InitParams.RTS.PortName[0] != 0) )
            {
                if (Uart_p->RTSHandle != (STPIO_Handle_t )NULL)
                {
                    (void)STPIO_Close(Uart_p->RTSHandle);
                }
                if (Uart_p->CTSHandle != (STPIO_Handle_t )NULL)
                {
                    (void)STPIO_Close(Uart_p->CTSHandle);
                }
            }
         ...
         /* Now uninstall the interrupt handler */
#if defined (ST_OSLINUX) && defined (LINUX_FULL_KERNEL_VERSION)
            free_irq(Uart_p->InitParams.InterruptNumber, (void *)Uart_p);
#else

            if (STOS_InterruptUninstall(Uart_p->InitParams.InterruptNumber,
                                    Uart_p->InitParams.InterruptLevel, 
                                    (void *)Uart_p) != 0)
            {
                                 ...
                                 &IntStatus) == 0)
            {
                ...
                {
                    /* interrupt_disable call will be redundant after change will be done in STBOOT */
...
#if defined(ST_OSLINUX)
            STOS_UnmapRegisters((void *)Uart_p->MappingBaseAddress, Uart_p->MappingWidth);
#endif
            STOS_MemoryDeallocate(DriverPartition,Uart_p);
        ...
        Protocol_p->DataBits <= STUART_9BITS_NO_PARITY)
    {
            ...
            Protocol_p->StopBits <= STUART_STOP_2_0)
        {
            ...
            switch (Protocol_p->FlowControl)
            {
                ...
                case STUART_SW_FLOW_CONTROL: /* Do nothing */
                    break;
...
#pragma ST_device(ASCRegs_p)
    ...
    for (Uart_p = UartQueueHead_p; Uart_p != NULL; Uart_p = Uart_p->Next_p)
    {
        if ((Uart_p->TXDHandle == Handle) && Uart_p->WriteAwaitingPioCompare)
        {
            ...
            Uart_p->WriteAwaitingPioCompare = FALSE;
#ifdef ST_OSLINUX
            if ((UART_Status(ASCRegs_p) & STATUS_TXHALFEMPTY) != 0 && NumberToWrite > 1)
#else
            if ((ASCRegs_p->ASCnStatus & STATUS_TXHALFEMPTY) != 0 && NumberToWrite > 1)
#endif
            {
