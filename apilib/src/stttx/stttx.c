...
Copyright (C) 2000, 2005 STMicroelectronics
...
#ifdef ST_OS20
#define STTTX_DMA_TIMEOUT_STACK_DEPTH 1024
#else
#define STTTX_DMA_TIMEOUT_STACK_DEPTH 17*1024
#endif
extern void TTXHandlerTask( void *Data );
extern void VBIOutputTask( void *Data );
extern void STBOutputTask( void *Data );
...
static const ST_Revision_t stttx_DriverRev = "STTTX-REL_3.1.4A2";
                           ...
                           const STTTX_InitParams_t *InitParams )
{
    ...
    if( stttx_Sentinel == NULL )                    /* first Init instance? */
    {
        stttx_Sentinel = ThisElem =                 /* keep a note of result */
            ( stttx_context_t* )memory_allocate_clear(
    ...
    else                                            /* already Init'ed */
    {
        ...
        PrevElem->Next = ThisElem =                 /* keep note of result */
            ( stttx_context_t* )memory_allocate(
    ...
    for( i = VBI_SLOT; i < j; i++ )                  /* loop on all Objects */
    {
        ...
        CurrPes->CircleBuff = (U8 *)
            memory_allocate((partition_t *)InitParams->DriverPartition,
    ...
    if( InitFailure )                               /* failure above? */
    {
        ...
        memory_deallocate(                          /* free the memory */
                           ...
                           STTTX_Handle_t            *Handle )
{
    ...
    *Handle = ThisElem->BaseHandle + i;     /* pass back base + index */
     ...
     */
    if ((error == ST_NO_ERROR) /*&& (OpenParams->Type != STTTX_STB)*//*wy 20061209 del for VBI*/)
    {
                           ...
                           const STTTX_TermParams_t *TermParams )
{
...
#endif
    stttx_context_t     *PrevElem = NULL, *ThisElem = NULL;
    ...
    memory_deallocate(ThisElem->InitParams.DriverPartition, ThisElem);
