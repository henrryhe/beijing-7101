...
void TTXHandlerTask( void *Data )
{
...
#ifdef  TELETEXT_BOTH_VBI_STB              /*wy 20061208 add*/
	extern U8	CurrentTXTMODE;
#endif
    ...
    LocalBuffer = (U8 *)memory_allocate( partition_p, MESSAGE_DATA_SIZE );
    if (LocalBuffer == NULL)
    {
    ...
    Page_p = (page_store_t *)dma_alloc_coherent(NULL,
                                    ...
                                    GFP_KERNEL);
#else
   Page_p = (page_store_t *) memory_allocate( partition_p,
                                                (U32)(sizeof(page_store_t) ) );
#endif
    if ( Page_p == NULL )
    {
...
#ifdef  TELETEXT_BOTH_VBI_STB              /* Only VBI type taken into account!*/
			if(CurrentTXTMODE==STB_TXT)
#endif				
					{
						/*zxg20050615 add*/	
					   extern int DecodeLine(/*Service_Text_info *TeletextInfo, */U8 *p);
					   extern     U8 ReverseBits(U8 Byte);
						U8  teletextPacket[45];
						int j;
                        memcpy(teletextPacket,&CurrPes->RxLinearBuff[(ExtractPoint + PACKET_ADDR_POS1-3)],45);

						for(j=2;j<45;j++)  
				                  teletextPacket[j]= ReverseBits(teletextPacket[j]);

                        DecodeLine(teletextPacket);
                        /****************/
					}
#endif
    ...
    memory_deallocate(partition_p, LocalBuffer);
#ifdef ST_OSLINUX
   dma_free_coherent(NULL,
                    ...
                    DMA_Handle);
#else
    memory_deallocate(partition_p, Page_p);
#endif
#if STTTX_PES_MODE == PES_CAPTURE_MODE
                                   ...
                                   U32              NumberOfLines )
{
    ...
    if (ValidLines > 0)
    {
        ...
        TtxBufferOrg_p = (U8 *)memory_allocate(ThisElem->InitParams.DriverPartition, (48*NumberOfLines)+15);
        /*Buffer must be 32 byte aligned...*/
        TtxBuffer_p = (U8*)((U32)(TtxBufferOrg_p+15)&(~15));
        ...
        for(j=0;j<NumberOfLines;j++)
        {
            memcpy( (void *)(TtxBuffer_p+(48*j)),&CurrPage->Lines[j][3], 43);
            LineMask = (LineMask << 1)|0x01;
        }
        LineMask = (LineMask << 1)|0x01;
        if (OddEvenFlag == STTTX_ODD_FIELD_FLAG)
        {
            ...
            VBIVPDisplayParamsOdd.LineMask      = LineMask;
        ...
        else
        {
            ...
            VBIVPDisplayParamsEven.LineMask     = LineMask;
...
#if defined(ST_5105) || defined(ST_5188) || defined(ST_5107)
        VBI.Type.TTX.LineMask = (LineMask<<7);
#else
...
#endif
        error = STVBI_SetDynamicParams( ThisElem->VBIHandle, &VBI);
...
#if !defined(ST_5100) && !defined(ST_5105) && !defined (ST_5301) && !defined(ST_5188) && !defined(ST_5107)
            reg = 0x22;
            bitmask = 0x04; /*0x08;*/ /* We start in line 7 because some teletext
                                         decoders can not use line 6 */
#else
            reg = 0x25;
            bitmask = 0x80;           /* On 5100 vysnc polarity is inverted */
#endif
        }
        else
        {
