...
#if defined(ST_5100) ||  defined(ST_5528) || defined(ST_5105) || defined(ST_7710) || defined(ST_5188) \
     || defined(ST_5107)|| defined(ST_7100) || defined(ST_7109)
#include "stvout.h"
#endif
...
#define GDMA_BASE_ADDRESS      (ST5188_GDMA1_BASE_ADDR)
        ...
        #if defined(ST_7109) || defined(ST_5301)
...
const U8  HarnessRev[] = "3.1.4A3";

#ifndef STTTX_PES_FILE
    #define STTTX_PES_FILE "ip_ttxpid414.pes"
#endif

#ifndef STTTX_PESBUFFER_SIZE
    #define STTTX_PESBUFFER_SIZE 6800000
#endif
...
#else
    #define                 SYSTEM_MEMORY_SIZE          0x0700000
#endif
#endif
...
/*------------------------------------------------------------------
                    Inits done specific to ST_7020
-------------------------------------------------------------------*/        
#if defined(ST_7020)
...
#endif /* End Inits Specific to ST_7020 */
...
/* ---GlobalInit_Dual() for dual test cases ---*/
#if defined(TTXDUAL_DISK_TEST) || defined(TTXDUAL_STPTI_TEST)
void GlobalInit_Dual(void)
{
    ST_ErrorCode_t  StoredError = ST_NO_ERROR;
...
ST_ErrorCode_t GlobalTerm(void)
{
    ...
    /* STVBI_Term() */
    {
        STVBI_TermParams_t TermParams;
        TermParams.ForceTerminate = TRUE;
        error = STVBI_Term(VBIDeviceName, (STVBI_TermParams_t *)&TermParams);
    }
#endif    
    /* STEVT_Term() */
    {
...
void NormalUseStpti( void )
{
    ...
    /* revision string available before STTTX Initialized */
    STTBX_Print(( "Calling STTTX_GetRevision() preInit\n" ));
    RevisionStr = STTTX_GetRevision();
    STTBX_Print(( "Software Revision reported as %s\n", RevisionStr ));
    STTBX_Print(( "----------------------------------------------------------\n" ));
...
void NormalUseDisk( void )
{
    ...
    if( STOS_TaskCreate((void(*)(void *))InsertMain,
                                ...
                                TEST_PARTITION_2,
                                STTTX_DISK_STACK_DEPTH,
                                &VBIInsertMainStack_p,
                                TEST_PARTITION_2,
                                &IMVBITask_p,
                                &IMVBItdesc,
                                (int)MIN_USER_PRIORITY,
                                "VBI packet insertion",
    ...
    /* And destroy. */
    STOS_TaskDelete(IMVBITask_p,TEST_PARTITION_2,VBIInsertMainStack_p,TEST_PARTITION_2);
...
#endif
#if defined(ST_OS20) || defined(ST_OS21) || defined(ST_OSLINUX)/* linux changes */
...
static void InsertMain( void *Data)
{
    ...
    {
        Params->PesSize =( Params->PesBuffer_p[BufferPosition + PCKT_LEN_MSB_POS] << 8 ) +
                           Params->PesBuffer_p[BufferPosition + PCKT_LEN_LSB_POS] +
                           PCKT_HDR_LENGTH;
...
/* -------------------------------------------------------------------------
                                VOUT_Setup()
--------------------------------------------------------------------------- */
#if defined (ST_5100) || defined (ST_5105)  || defined(ST_5188) || defined(ST_5107) || defined(ST_7100) ||\
         defined(ST_7109)
...
/*--------------------------------------------------------------------
                      ConfigureMerger() doing MERGE_Setup
----------------------------------------------------------------------*/
#if defined(TTXSTPTI_TEST)
...
void DataSpaceUsed()
{
    ...
    STTTX_InitParams_t InitParams_s;
    STTTX_TermParams_t TermParams;
    U32 address1, address2=0;
    U32 Used;
    U32 *temporary;

    /* Find first address */
    temporary = STOS_MemoryAllocate(TEST_PARTITION_1, sizeof(U32));
...
void StackUsage()
{
    ...
    ret = partition_status(TEST_PARTITION_2, &before, (partition_status_flags_t)0);
    if (ret != 0)
    ...
    partition_status(TEST_PARTITION_2, &after, (partition_status_flags_t)0);
    if (ret != 0)
        printf("error!! in partition_status \n");
...
#endif /* End of Stack usage tests*/
...
#else
    StackUsage();
    NormalUseDisk(); /* Used for Memory Injection*/
