...
#if defined(ST_7710) || defined(ST_5105) || defined(ST_5107)  || defined(ST_5162)/* || defined(ST_7100)  */
...
#define MAX_DMA_TRANSFER_SIZE_FOR_FAST_ACCESS_INJ   (512*1024);
#define MIN_DMA_TRANSFER_SIZE_FOR_FAST_ACCESS_INJ   (128*1024);
#ifdef ST_ZEUS
    #define MAX_DMA_TRANSFER_SIZE                   (512*1024);
    #define MIN_DMA_TRANSFER_SIZE                   (0);
#else /* ST_ZEUS */
    #define MAX_DMA_TRANSFER_SIZE                   (128*1024);
    #define MIN_DMA_TRANSFER_SIZE                   (0);
#endif /* not ST_ZEUS */
...
void VID_MemInject(STVID_Handle_t CurrentHandle, S32 InjectLoop, S32 FifoNb, S32 BufferNb);
...
void    VID_ReportInjectionAlreadyOccurs(U32 CDFifoNb)
{
    switch(VID_Injection[CDFifoNb-1].Type)
    {
        ...
        case VID_FAST_FILESYSTEM_INJECTION:
            STTBX_Print(("Injection from fast filesystem is already runnning on fifo %d!!\n", CDFifoNb));
            break;
...
static BOOL VID_StopMemInject(U32 VID_InjectionId)
{
    ...
    {
        STTBX_ErrorPrint(("Invalid CD fifo identifier\n"));
        return(TRUE);
    }
    if (VIDTid != NULL )
    {
        if ((VID_Injection[VID_InjectionId].Type == VID_MEMORY_INJECTION) ||
            (VID_Injection[VID_InjectionId].Type == VID_FAST_FILESYSTEM_INJECTION))
        {
            /* Lock access */
            ...
            STVID_UnmapPESBufferFromUserSpace(VID_Injection[VID_InjectionId].Driver.Handle);
#endif
#endif
...
static void DoVideoInject( void )
{
    ...
    {
        STTBX_ErrorPrint(("Error ! Unable to allocate FDMA node"));
        return;
    }
#else
	...
	while (1)
    {
...
#else
		    STOS_MemoryDeallocate (NCachePartition_p, NotRoundedNode_p);
#endif /*  mb411  mb519 */
#endif /* USE_FDMA_TRANSFER */
        ...
        for (VID_InjectionId=0; VID_InjectionId<VIDEO_MAX_DEVICE; VID_InjectionId++)
        {
...

            {
                ...
                {
                    STTBX_ErrorPrint(("Jobs list overflow for cd fifo %d !!! Stop memory injection.\n", VID_InjectionId+1));
            ...
            if(VID_Injection[VID_InjectionId].State == STATE_INJECTION_NORMAL)
            {
                    ...
                    /* Get the DMA transfert size */
                        ...
                        else
                        {
                            ...
                            if (ErrCode != ST_NO_ERROR)
                            {
                                STTBX_ErrorPrint(( "DoVideoInject() : error STVID_GetBitBufferFreeSize()=0x%x !!\n", ErrCode));
                                VID_StopMemInject(VID_InjectionId);
                            }
                            ...
                            if (DmaTransferSize > (S32)PtiMemInjectVID_Injection_p->MaxDmaTransferSize)
                            {
                                DmaTransferSize = PtiMemInjectVID_Injection_p->MaxDmaTransferSize;
                            }
                            ...
                            if (DmaTransferSize > 0)
                            {
...
#ifdef ST_ZEUS
                                if((DmaTransferSize > 0x40000) &&
                                   (VID_Injection[VID_InjectionId].Type != VID_FAST_FILESYSTEM_INJECTION))
                                {
                                    DmaTransferSize=0x40000;
                                }
#else /* ST_ZEUS */
...
#ifdef ST_OSLINUX
                                /* Under Linux, since mmaped the addresses are already CPU addresses */
                                /* no need then to convert anything ... */
                                CPUDest = (void *)SnapInputWrite;
                                CPUSrc  = (void *)PtiMemInjectVID_Injection_p->CurrentSrcPtr_p;
#else
                                ...
                                if(VID_Injection[VID_InjectionId].Type == VID_FAST_FILESYSTEM_INJECTION)
                                {
                                    StuffingToAdd = 0;
                                    if(PtiMemInjectVID_Injection_p->NeedToRealignTransfer)
                                    {
                                        if(DmaTransferSize >= (S32)PtiMemInjectVID_Injection_p->NeedToRealignTransfer)
                                        {
                                            StuffingToAdd = PtiMemInjectVID_Injection_p->NeedToRealignTransfer;
                                        }
                                        else
                                        {
                                            StuffingToAdd = DmaTransferSize;
                                        }
                                        DmaTransferSize -= StuffingToAdd;
                                        PtiMemInjectVID_Injection_p->NeedToRealignTransfer -= StuffingToAdd;

#ifdef ST_OSLINUX
                                        /* Under Linux, since mmaped the addresses are already CPU addresses */
                                        /* no need then to convert anything ... */
                                        CPUStuffingDest = (U8*)SnapInputWrite;
#else
                                        CPUStuffingDest = STAVMEM_VirtualToCPU(SnapInputWrite, &VirtualMapping);
#endif
                                        for(i = 0 ; i < StuffingToAdd ; i++)
                                        {
                                            *CPUStuffingDest = 0;
                                            CPUStuffingDest++;
                                            SnapInputWrite++;
                                    ...
                                    if((DmaTransferSize < (S32)PtiMemInjectVID_Injection_p->MinDmaTransferSize) &&
                                       (RemainingSourceDataSize >= PtiMemInjectVID_Injection_p->MinDmaTransferSize) &&
                                       (((U32)VID_Injection[VID_InjectionId].Top_p - SnapInputWrite) >= PtiMemInjectVID_Injection_p->MinDmaTransferSize))
                                    {   /* If transfert too small, and not at end of file nor at end of dest buffer, */
                                        /*   wait for next loop */
                                        DmaTransferSize = 0;
                                    }
                                    else
                                    {
                                        if(DmaTransferSize != 0)
                                        {
                                            FileReadPos = (U32)(PtiMemInjectVID_Injection_p->CurrentSrcPtr_p - PtiMemInjectVID_Injection_p->SrcBuffer_p);
#if defined(VALID_TOOLS) && defined(STVID_LOG_INJECT_PACKETS)
                                            TrsfBegTime = STOS_time_now();
#endif /* defined(VALID_TOOLS) && defined(STVID_LOG_INJECT_PACKETS) */
                                            fseek(PtiMemInjectVID_Injection_p->FileHandle, FileReadPos, SEEK_SET);
                                            fread(CPUDest, 1, DmaTransferSize, PtiMemInjectVID_Injection_p->FileHandle);
#if defined(VALID_TOOLS) && defined(STVID_LOG_INJECT_PACKETS)
                                            TrsfEndTime = STOS_time_now();
                                            VIDINJDBG_TrsfNumber++;
                                            if(VIDINJDBG_FirstUsb2Pes)
                                            {
                                                MSGLOG_LogPrint(VIDINJDBG_InjLogHandle, "0,USB2PES,,,%u,%u,,,%u,%u,,,,,\n",
                                                                PtiMemInjectVID_Injection_p->SrcBuffer_p,
                                                                (PtiMemInjectVID_Injection_p->SrcBuffer_p + (U32)PtiMemInjectVID_Injection_p->SrcSize),
                                                                VID_Injection[VID_InjectionId].Base_p,
                                                                VID_Injection[VID_InjectionId].Top_p);
                                                VIDINJDBG_FirstUsb2Pes = FALSE;
                                            }
                                            MSGLOG_LogPrint(VIDINJDBG_InjLogHandle, "%u,USB2PES,%u,%u,,,%u,%u,,,%u,%u,%llu,%llu,%llu\n",
                                                            VIDINJDBG_TrsfNumber,
                                                            DmaTransferSize,
                                                            StuffingToAdd,
                                                            ...
                                                            SnapInputRead,
                                                            SnapInputWrite,
                                                            (TrsfBegTime*1000000LLU)/TicksPerOneSecond,
                                                            (TrsfEndTime*1000000LLU)/TicksPerOneSecond,
                                                            ((TrsfEndTime-TrsfBegTime)*1000000LLU)/TicksPerOneSecond);
#endif /* defined(VALID_TOOLS) && defined(STVID_LOG_INJECT_PACKETS) */
                                        }
                                    }
                                }
                        ...
                        if(CorruptMemoryInjection && (ErrorModulo == 0))
                        {
                            ...
                            else
                            {
...
#else /* not ST_51xx_Device || ST_7100 || ST_7109 || ST_7710*/
                                /* Lock pti access */
                                ...
                                if ( ErrCode != ST_NO_ERROR )
                                {
                                    STTBX_ErrorPrint(("Task PtiMemInject : STPTI_UserDataWrite()=0x%x\n", ErrCode));
                                }
#else
                            ...
                            if (DmaTransferSize > 0)
                            {
                                ...
                                if ( ErrCode != ST_NO_ERROR )
                                {
                                    STTBX_ErrorPrint(("Task PtiMemInject : STPTI_UserDataSynchronize()=0x%x\n", ErrCode));
                                }
#else
                                     ...
                                     - 1)
                            {
                                if(VID_Injection[VID_InjectionId].Type == VID_FAST_FILESYSTEM_INJECTION)
                                {
                                if((PtiMemInjectVID_Injection_p->SrcSize & 0x03) != 0)
                                {
                                    PtiMemInjectVID_Injection_p->NeedToRealignTransfer = 4 - (PtiMemInjectVID_Injection_p->SrcSize & 0x03);
                                }
                                else
                                {
                                    PtiMemInjectVID_Injection_p->NeedToRealignTransfer = 0;
                                }
                                }
...
static BOOL VID_KillTask( STTST_Parse_t *pars_p, char *result_sym_p )
{
    ...
    if (WaitEnd == 1)
    {
        ...
        while (   ((VID_Injection[FifoNb - 1].Type == VID_MEMORY_INJECTION) ||
                   (VID_Injection[FifoNb - 1].Type == VID_FAST_FILESYSTEM_INJECTION))
    ...
    if ( VIDTid != NULL )
    {
        for (i = 0; (i < VIDEO_MAX_DEVICE) || (RetErr); i++)
        {
            ...
            if ((VID_Injection[i].Type == VID_MEMORY_INJECTION) ||
                (VID_Injection[i].Type == VID_FAST_FILESYSTEM_INJECTION))
            {
        ...
        if((!(StillFifoInjection)) && (!(RetErr)))
        {
            ...
            if ( ErrCode != ST_NO_ERROR )
            {
                STTBX_ErrorPrint(("VID_KillTask(): STOS_TaskWait() failure (error=0x%x buff=%d State=%d Type=%d) !\n",
            ...
            if ( ErrCode!= ST_NO_ERROR)
            {
                STTBX_ErrorPrint(("VID_KillTask(): STOS_TaskDelete() failure (error=0x%x buff=%d Type=%d) !\n",
...
/*-------------------------------------------------------------------------
 * Function : VID_MemInjectParameters
 *            Inject Video in Memory to DMA
 * Input    : *pars_p, *Result
 * Output   : FALSE if no error
 * Return   : TRUE if error, FALSE if success
 * ----------------------------------------------------------------------*/
static BOOL VID_MemInjectParameters( STTST_Parse_t *pars_p, char *result_sym_p )
{
    BOOL            RetErr = FALSE;
    ...
    if((VID_BufferLoad[BufferNb-1].AllocatedBuffer_p == NULL) &&
       (VID_BufferLoad[BufferNb-1].NbBytes == 0))
    {
        STTBX_ErrorPrint(( "No file loaded in memory buffer !!\n" ));
        return(TRUE);
    }

    VID_MemInject( CurrentHandle, InjectLoop, FifoNb, BufferNb);
#endif /* STUB_INJECT */
...
void VID_MemInject(STVID_Handle_t CurrentHandle, S32 InjectLoop, S32 FifoNb, S32 BufferNb)
{
    ...
    if(VID_Injection[FifoNb-1].BitBufferSize == 0)
    {
        ...
        if(ErrCode != ST_NO_ERROR)
        {
            ...
            STTBX_ErrorPrint(("Error occured while exec GetBitBufferFreeSize = %d\n", ErrCode));
            RetErr = TRUE;
        }
    }
    if (!(RetErr))
    {
        if(VID_Injection[FifoNb-1].Type == NO_INJECTION)
        {
            if (VIDTid == NULL)
            {
                ...
                if ( ErrCode != ST_NO_ERROR )
                {
                    STTBX_ErrorPrint(("VID_MemInject(): Unable to create video task (error=0x%x)!\n", ErrCode));
                    RetErr = TRUE;
                }
                else
                {
            ...
            if (!(RetErr))
            {
                if(VID_BufferLoad[BufferNb-1].AlignedBuffer_p == NULL)
                {
                    strcpy(VID_Injection[FifoNb-1].Config.Memory.FileName, VID_BufferLoad[BufferNb-1].StreamName);
                    VID_Injection[FifoNb-1].Config.Memory.FileHandle = VID_BufferLoad[BufferNb-1].FileHandle;
                }
...
#if defined ST_OSLINUX && !defined STVID_MMAP_PESBUFFERS_IN_USERWRAPPER
                ErrCode = STVID_MapPESBufferToUserSpace(VID_Injection[FifoNb-1].Driver.Handle, &VID_Injection[FifoNb-1].Base_p, InputBuffersize);
#endif
                ...
                STVID_SetDataInputInterface( VID_Injection[FifoNb-1].Driver.Handle,
                                             ...
                                             InformReadAddress,
                                             (void*)(FifoNb-1));
#else /* not ST_51xx_Device && not ST_7100 && not ST_7109 && not ST_7710*/
                ...
                {
                    VID_Injection[FifoNb-1].Type = VID_FAST_FILESYSTEM_INJECTION;
                    VID_Injection[FifoNb-1].Config.Memory.MaxDmaTransferSize = MAX_DMA_TRANSFER_SIZE_FOR_FAST_ACCESS_INJ;
                    VID_Injection[FifoNb-1].Config.Memory.MinDmaTransferSize = MIN_DMA_TRANSFER_SIZE_FOR_FAST_ACCESS_INJ;
                }
                else
                {
                    VID_Injection[FifoNb-1].Type = VID_MEMORY_INJECTION;
                    VID_Injection[FifoNb-1].Config.Memory.MaxDmaTransferSize = MAX_DMA_TRANSFER_SIZE;
                    VID_Injection[FifoNb-1].Config.Memory.MinDmaTransferSize = MIN_DMA_TRANSFER_SIZE;
                }
...
/*-------------------------------------------------------------------------
 * Function : VID_MemInjectForSeek
 *            Inject a first ratio of the stream, starting from beginning of the stream,
 *            then insert discontinuity and inject second ratio of the stream so that the
 *            end of second injection is also the end of the stream
 * Input    : *pars_p, *Result
 * Output   : FALSE if no error
 * Return   : TRUE if error, FALSE if success
 * ----------------------------------------------------------------------*/
static BOOL VID_MemInjectForSeek( STTST_Parse_t *pars_p, char *result_sym_p )
{
    BOOL            RetErr = FALSE;
    S32             FifoNb, BufferNb, InjectLoop;
    STTST_Parse_t * InjectionParameters_p;
    U32             SizeOfStream, SecondInjectionSize, ShiftToApply;
    void * StartOfStream_p;
    U8 *            BufferPointer_p;
    S32             PercentageForFirstInjection, PercentageForSecondInjection;

    STVID_Handle_t  CurrentHandle;

    InjectionParameters_p = pars_p;
#if !defined STUB_INJECT
    RetErr = STTST_GetInteger( InjectionParameters_p, DefaultVideoHandle, (S32*)&CurrentHandle);
    if ( RetErr )
    {
        STTST_TagCurrentLine(InjectionParameters_p, "expected video handle");
        return(TRUE);
    }

    RetErr = STTST_GetInteger( InjectionParameters_p, 1, &FifoNb);
    if ((RetErr ) || (FifoNb < 1) || (FifoNb > VIDEO_MAX_DEVICE))
    {
        sprintf(VID_Msg, "expected Fifo number (1 to %d)", VIDEO_MAX_DEVICE);
        STTST_TagCurrentLine( InjectionParameters_p, VID_Msg);
        return(TRUE);
    }

    RetErr = STTST_GetInteger( InjectionParameters_p, 1, &BufferNb);
    if (( RetErr ) || (BufferNb < 1) || (BufferNb > VID_MAX_BUFFER_LOAD))
    {
        sprintf(VID_Msg, "expected load buffer number (default is 1. max is %d)", VID_MAX_BUFFER_LOAD);
        STTST_TagCurrentLine( InjectionParameters_p, VID_Msg);
    ...
    RetErr = STTST_GetInteger( InjectionParameters_p, 1, &PercentageForFirstInjection );
    if ( RetErr )
    {
        STTST_TagCurrentLine(InjectionParameters_p, "expected number between 0 and 100, percentage of stream injected by first injection");
        return(TRUE);
    }

    RetErr = STTST_GetInteger( InjectionParameters_p, 1, &PercentageForSecondInjection );
    if ( RetErr )
    {
        STTST_TagCurrentLine(InjectionParameters_p, "expected number between 0 and 100, percentage of stream injected by second injection");
        return(TRUE);
    }
    ...
    SizeOfStream = VID_BufferLoad[BufferNb-1].NbBytes;
    StartOfStream_p = VID_BufferLoad[BufferNb-1].AlignedBuffer_p;
    InjectLoop = 1;

    VID_BufferLoad[BufferNb-1].NbBytes = (PercentageForFirstInjection * VID_BufferLoad[BufferNb-1].NbBytes) / 100;

    STTBX_Print(("First Injection size: %d, total size %d \n", VID_BufferLoad[BufferNb-1].NbBytes, SizeOfStream));

    VID_MemInject(CurrentHandle, InjectLoop, FifoNb, BufferNb);

    while (1)
    {
        /* Wait for the first injection completed to insert discontinuity and start second injection */
        if (VID_Injection[FifoNb-1].Type == NO_INJECTION)
        {
            STTBX_Print(("First Injection Completed \n"));
            STVID_InjectDiscontinuity(CurrentHandle );
            SecondInjectionSize = (PercentageForSecondInjection * SizeOfStream) / 100 ;
            STTBX_Print(("Second Injection size: %d\n", SecondInjectionSize));
            ShiftToApply = SizeOfStream - SecondInjectionSize;
            BufferPointer_p =  VID_BufferLoad[BufferNb-1].AlignedBuffer_p;
            BufferPointer_p +=   ShiftToApply;
            VID_BufferLoad[BufferNb-1].AlignedBuffer_p = BufferPointer_p;
            VID_BufferLoad[BufferNb-1].NbBytes = SecondInjectionSize;
        ...
        {
            STOS_TaskDelayUs(DELAY_1S); /* wait 1s */
            continue;
        }
    }
    while (1)
    {
        /* Wait for second injection to be finished before leaving */
        if (VID_Injection[FifoNb-1].Type == NO_INJECTION)
        {
            /* Put back the buffer used to its initial values */
            VID_BufferLoad[BufferNb-1].NbBytes = SizeOfStream;
            VID_BufferLoad[BufferNb-1].AlignedBuffer_p = StartOfStream_p;
            break;
        }
        else
        {
...
static BOOL VID_MemErrorsInject(STTST_Parse_t *pars_p, char *result_sym_p )
{
    ...
    RetErr = VID_MemInjectParameters(pars_p, result_sym_p);
    ...
    for (VID_InjectionId=0; VID_InjectionId<VIDEO_MAX_DEVICE; VID_InjectionId++)
    {
        ...
        switch(VID_Injection_Copy.Type)
        {
            ...
            case VID_MEMORY_INJECTION:
                ...
                break;
            case VID_FAST_FILESYSTEM_INJECTION:
                STTBX_Print(("Fast Filesystem Drive\n"));
                break;
            default:
        ...
        if ((VID_Injection_Copy.Type == VID_MEMORY_INJECTION) ||
            (VID_Injection_Copy.Type == VID_FAST_FILESYSTEM_INJECTION))
        {
...
static BOOL VIDDecodeFromMemory(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if((VID_BufferLoad[BuffNb-1].AllocatedBuffer_p == NULL) &&
       (VID_BufferLoad[BuffNb-1].NbBytes == 0))
    {
        STTBX_ErrorPrint(( "No file loaded in memory buffer !!\n" ));
        return(TRUE);
    }
    ...
    else
    {
        if(VID_Injection[FifoNb-1].BitBufferSize == 0)
        {
            ...
            if(ErrCode != ST_NO_ERROR)
            {
                STTBX_ErrorPrint(("Error occured : STVID_GetBitBufferFreeSize() = %d !!\n", ErrCode));
                RetErr = TRUE;
                VID_Injection[FifoNb-1].BitBufferSize = 0;
            }
        }
    ...
    {
        STTBX_ErrorPrint(("Cannot launch a decode until end of buffer while decoder is stopped !\n" ));
    }
    ...
    if (ErrCode == ST_NO_ERROR)
    {
        ...
        if ( ErrCode== ST_NO_ERROR )
        {
            ...
            {
                STTBX_ErrorPrint(("Error while subscribing STVID_NEW_PICTURE_TO_BE_DISPLAYED_EVT : %x\n", ErrCode ));
            }
            STEVT_Close(LocEvtHandle);
        }
        else
        {
            STTBX_ErrorPrint(("STEVT_Open error while subscribing STVID_NEW_PICTURE_TO_BE_DISPLAYED_EVT : %x\n", ErrCode ));
        }
    }
...
static BOOL VIDFlushVideoCDFIFO(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    * sending, just to make sure all relevant data are flushed.	*/
    ...
    STDMA_Params.Holdoff = 1;
    STDMA_Params.WriteLength = 4;
#elif defined(ST_51xx_Device) || defined(ST_7100) || defined(ST_7109) || defined(ST_7200) || defined(ST_7710)
...
#endif /* ST_51xx_Device || ST_7100 || ST_7109 || ST_7710 */
#else
...
static BOOL VID_StillDecode( STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    {
        STTBX_ErrorPrint(( "No file loaded in memory buffer %d !!\n", BufferNb1 ));
        return(TRUE);
    }
    ...
    if(VID_Injection[FifoNb-1].BitBufferSize == 0)
    {
        ...
        {
            VID_Injection[FifoNb-1].BitBufferSize = 0;
            STTBX_ErrorPrint(("VID_StillDecode(): STVID_GetBitBufferFreeSize() failure, error=0x%x !\n", ErrCode));
            RetErr = TRUE;
        }
    }
    if (!(RetErr))
    {
        if(VID_Injection[FifoNb-1].Type == NO_INJECTION)
        {
            ...
            STVID_SetDataInputInterface(
                                    VID_Injection[FifoNb-1].Driver.Handle,
                                    GetWriteAddress,InformReadAddress,(void*)(FifoNb-1));
#endif /* defined(ST_51xx_Device) || defined(ST_7100) || defined(ST_7109) || defined(ST_7710) */
            VID_Injection[FifoNb-1].Type = VID_MEMORY_INJECTION;
            /* Video Load Buffer in use */
            ...
            if(ErrCode != ST_NO_ERROR)
            {
                STTBX_ErrorPrint(("VID_StillDecode(): STVID_Stop() failure, error=0x%x ! \r\n", ErrCode));
...
#else /* ! (defined(ST_7100) || defined(ST_7109) || defined(ST_7200) || defined(ST_51xx_Device) || defined(ST_7710)) */
    U32 i;
...
#endif /* defined(ST_7100) || defined(ST_7109) || defined(ST_7200) || defined(ST_51xx_Device) || defined(ST_7710)*/
...
void VID_Inj_SignalAccess (U32 index)
{
    if ( index<VIDEO_MAX_DEVICE )
    {
        STOS_SemaphoreSignal(VID_Injection[index].Access_p);
    }
}
...
void VID_Inj_WaitAccess (U32 index)
{
    if ( index<VIDEO_MAX_DEVICE )
    {
        STOS_SemaphoreWait(VID_Injection[index].Access_p);
    }
}
...
BOOL VID_Inj_RegisterCmd (void)
{
...
#if !defined(ST_7015) && !defined(ST_7020) && !defined(ST_5528) && !defined(ST_51xx_Device)\
         && !defined(ST_7100) && !defined(ST_7109) && !defined(ST_7200) && !defined(ST_7710)
    ...
    RetErr |= STTST_RegisterCommand( "VID_INJECT",   VID_MemInjectParameters,
            "<loop(-1=infinite)><Handle><CDFifoNb><Load buffer nb> DMA Inject Video in memory (parallel task)");
    RetErr |= STTST_RegisterCommand( "VID_SEEK",   VID_MemInjectForSeek,
            "<Handle><CDFifoNb><Load buffer nb><PercentageForFirstInjection><PercentageForSecondInjection>, Percentagexxx must be between 0 and 100. DMA Inject first part then inject discontinuity and second part. First part starts at beginning of stream. Second part ends at end of stream");
