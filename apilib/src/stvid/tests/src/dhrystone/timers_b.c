...
double dtime( void );
    ...
    #pragma ST_section( dtime, "code_internal_section" )
#endif
...
#include <ostime.h>     /* time_now */
#include "stcommon.h"	/* ST_GetClocksPerSecond */

double dtime( void )
{
    clock_t time = time_now();

    return( (double)time / (double)ST_GetClocksPerSecond() );
}
#endif
