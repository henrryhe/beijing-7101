...
#if defined(ST_51xx_Device) || defined(ST_7100) || defined(ST_7109) || defined(ST_ZEUS) || defined(ST_7200)
...
/* Post-processing test related */
#define POSTPROCESSING_TEST_WHITE_COLOR 0xEB
/* --- Global variables ------------------------------------------------ */
...
/* Forced Decimation Test */
static semaphore_t*             ForcedDecimationSemaphore_p = NULL;
static STVID_DecimationFactor_t ForcedDecimationFactor;
...
/* Post-processing test related */
static STGXOBJ_Bitmap_t PostProcessTestBitmapParams;
static semaphore_t*     PostProcessTestAccessControlSemaphore_p;
...
static BOOL VID_Load(STTST_Parse_t *pars_p, char *result_sym_p )
{
    ...
    RetErr = STTST_GetInteger( pars_p, FALSE, &DirectInjection);
    if (( RetErr ) || ((DirectInjection != FALSE) && (DirectInjection != TRUE)))
    {
        sprintf(VID_Msg, "expected (default is FALSE. Possible value are %d|%d)", FALSE, TRUE);
        STTST_TagCurrentLine( pars_p, VID_Msg);
        STTST_AssignInteger(result_sym_p, TRUE, FALSE);
        return(TRUE);
    }
#if defined(ST_OSLINUX)
    /* Direct injection is not possible on linux since it uses filesystem API*/
    STTBX_Print(("*** warning: Fast injection from filesystem impossible. ***\n"));
    DirectInjection = FALSE;
#endif
#endif /* #if defined(USE_FILESYSTEM) ... */
    ...
    {
        STTBX_ErrorPrint(( "Buffer cannot be loaded because already in used !!\n" ));
        STTST_AssignInteger(result_sym_p, TRUE, FALSE);
        return(TRUE);
    }
...
#elif defined(ST_ZEUS) && defined(USE_ST20_DCU)
...
#ifdef USE_FILESYSTEM
    FileSize = SYS_FFilesize(FileDescriptor);
#else /* #ifdef USE_FILESYSTEM */
#ifdef ST_OS20
#if defined(ST_ZEUS) && !defined(USE_ST20_DCU)
...
#if defined(USE_FILESYSTEM) && !defined(STUB_INJECT)
    if(SYS_FIsFastAccessPath(FileName) && (DirectInjection == TRUE))
    {
        VID_BufferLoad[LVar-1].AllocatedBuffer_p = NULL;
        VID_BufferLoad[LVar-1].AlignedBuffer_p = NULL;
        VID_BufferLoad[LVar-1].Partition_p = NULL;
#if defined(mb411) || defined(mb519) || defined(mb618) || defined(mb636) || defined(ST_DELTAMU_HE)
        VID_BufferLoad[LVar-1].BlockHandle = 0;
#endif /* mb411 || mb519 || mb618 || mb636 || ST_DELTAMU_HE */
    ...
    {
#endif /* defined(USE_FILESYSTEM) && !defined(STUB_INJECT) */
...
#define MAX_FILE_SIZE	6*1024*1024
    ...
    while ((FileSize > 0) && (AllocatedBuffer_p == NULL ))
    {
        ...
        {
            STTBX_ErrorPrint(("VID_Load(): Error in vidutil_Allocate !!!\n"));
        }
...
#endif /* #if defined (mb411) || defined(mb519) ... #else ... */
    ...
    if (Truncated)
    {
        ...
        RetErr=TRUE;    /* Set to return an error status used for truncation detection */
        STTBX_ErrorPrint((VID_Msg));
    }
#else /* #ifndef ARCHITECTURE_SPARC */
...
#if defined(ST200_OS21)
{
    ...
    SYS_FRead(VID_BufferLoad[LVar-1].AlignedBuffer_p,  (size_t) FileSize, 1, FileDescriptor);
        ...
        Memfread(VID_BufferLoad[LVar-1].AlignedBuffer_p, 1, FileSize, FileDescriptor);
            ...
            {
                U8  i = 0;
                U32 RemainingDataToLoad = FileSize;

                STTBX_Print(("Loaded: 0%%"));
                for(i=0; i<99; i++)
                {
                    debugread(FileDescriptor, (void *)((U32)VID_BufferLoad[LVar-1].AlignedBuffer_p + (FileSize/100)*i), FileSize/100);
                    RemainingDataToLoad -= (FileSize/100);
                    STTBX_Print(("\rLoaded: %d%%", i+1));
                }
                debugread(FileDescriptor, (void *)((U32)VID_BufferLoad[LVar-1].AlignedBuffer_p + (FileSize/100)*99), RemainingDataToLoad);
                STTBX_Print(("\rLoaded: 100%%\n"));
            }
        #else
...
#endif /* #if defined(USE_FILESYSTEM) && !defined(STUB_INJECT) */
...
#ifdef USE_FILESYSTEM
    if((VID_BufferLoad[LVar-1].AlignedBuffer_p == NULL) && (DirectInjection == TRUE))
    {
        sprintf(VID_Msg, "file [%s] ready for injection from FILESYSTEM : size %ld\n",
            FileName, FileSize);
    }
    else
    {
...
static BOOL VID_LoadStill(STTST_Parse_t *pars_p, char *result_sym_p )
{
    ...
    if(VID_BufferLoad[LVar-1].UsedCounter != 0)
    {
        STTBX_ErrorPrint(( "Buffer cannot be load because already in used !!\n" ));
        return(TRUE);
    }
...
static BOOL VID_Copy(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if ( strcmp(Param,"FREE")==0 || strcmp(Param,"free")==0 )
    {
		...
		vidutil_Deallocate(AvmemPartitionHandle[0], &(CopiedPictureHandle[NbOfCopiedPictures-1]), CopiedPictureAddress[NbOfCopiedPictures-1]);
                ...
                {
                    /* Error handling */
                    STTBX_ErrorPrint(("Error freeing in VID_Copy()"));
                }
#endif	/* !LINUX || !FULL */
    ...
    {
        STTBX_ErrorPrint( ("VID_Copy() : too many pictures in memory !\n"));
        return(API_EnableError ? TRUE : FALSE );
    }
    ...
    else
    {
		...
		STAVMEM_PartitionHandle_t   AvmemPartitionHandle[1] = {0};
            ...
            {
                STTBX_ErrorPrint(("VID_Copy(): notice that AvmemAllocParams.Size=%d is lower than PictureParams.Size=%d\n",
            ...
            else
            {
                ...
                {
                    /* Error handling */
                    STTBX_ErrorPrint(("Error getting address in VID_Copy()"));
                }
            }
#endif	/* LINUX */
...
char FileName[80];
    ...
    if (FilePict == -1)
	{
            STTBX_ErrorPrint(("Failed to open %s in write mode.\n", FileName));
            return (FALSE);
	}
...
ST_ErrorCode_t Error;
    ...
    if ( NbOfCopiedPictures >= NB_MAX_OF_COPIED_PICTURES )
    {
        STTBX_ErrorPrint( ("VID_Restore : too many pictures in memory !\n"));
        return(API_EnableError ? TRUE : FALSE );
    }
    ...
    if (FilePict == -1)
	{
            STTBX_ErrorPrint(("Failed to open %s in read mode.\n", FileName));
            return(API_EnableError ? TRUE : FALSE );
	}
    ...
    else
    {
        ...
        {
            STTBX_ErrorPrint(("VID_Restore() : file inconsistency !\n"));
            STTBX_ErrorPrint(("              : file_size=%ld bytes but header_size+data_size=%d+%d=%d !\n",
                        FileSize, sizeof(PictParams), PictParams.Size, (PictParams.Size + sizeof(PictParams)) ));
            STTBX_ErrorPrint(("                command cancelled\n"));
            RetErr =  TRUE;
        }
        else
        {
            ...
            {
                STTBX_ErrorPrint(("VID_Restore(): notice that AvmemAllocParams.Size=%d is lower than PictParams.Size=%d\n",
            ...
            else
            {
                ...
                {
                    /* Error handling */
                    STTBX_ErrorPrint(("Error getting address in VID_RestorePicture()"));
                    RetErr =  TRUE;
                }
            }
#endif  /* LINUX */
...
BOOL VID_GetEventNumber(STTST_Parse_t *pars_p, STEVT_EventConstant_t *EventNumber)
{
    ...
    if ( strlen(ItemEvtNo)>0 )
    {
        ...
        if ( RetErr )
        {
            ...
            else if ( strcmp(ItemEvtNo,"STVID_NEW_FIELD_TO_BE_DISPLAYED_EVT")==0 )
            {
                EvtNo = STVID_NEW_FIELD_TO_BE_DISPLAYED_EVT;
            }
            else
            {
...
static BOOL VID_DeleteEvt(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    {
        STTBX_ErrorPrint(("VID_DeleteEvt() : events stack is in used (evt. not disabled) ! \n"));
        RetErr = TRUE;
    }
    STTST_AssignInteger(result_sym_p, RetErr, FALSE);
    return(API_EnableError ? RetErr : FALSE );
        ...
        switch(CallbackFunctionNb)
        {
            ...
            case 3 :
                ...
                if ( NbMaxOfVSyncToLog>0 )
                {
                    ...
                    if ( VSyncLog == NULL )
                    {
                        STTBX_ErrorPrint(("Unable to allocate memory, %d bytes for logging VSync !\n", (NbMaxOfVSyncToLog*sizeof(U32)) ));
                        RetErr = TRUE;
                    }
                }
                break;
...
static BOOL VID_TestEvtAspectRatio(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad DisplayAR %d (expected=%d)\n",
...
/*-------------------------------------------------------------------------
 * Function : VID_ForcedDecimationCallback
 *            picture params comparison for test VID_TestPictureParams()
 * Input    :
 * Output   :
 * Return   :
 * ----------------------------------------------------------------------*/
static void VID_ForcedDecimationCallback(STEVT_CallReason_t CC_Reason,
                            const ST_DeviceName_t RegistrantName,
                            STEVT_EventConstant_t Event,
                            void* EventData,
                            void *SubscriberData_p)
{
    ...
    PictureInfos_p = (STVID_PictureInfos_t *)(U32)EventData;
    if(PictureInfos_p->VideoParams.DecimationFactors == ForcedDecimationFactor)
    {
        semaphore_signal(ForcedDecimationSemaphore_p);
    }
}

/*-------------------------------------------------------------------------
 * Function : VID_ForceDecimationAndWait
 *            Force Decimation and wait 
 * Input    :
 * Output   : 
 * Return   : TRUE if error, FALSE if success
 * ----------------------------------------------------------------------*/
static BOOL VID_ForceDecimationAndWait(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ST_ErrorCode_t RetErr;
    S32 LVar;
    ...
    /* subscribe to the STVID_NEW_PICTURE_TO_BE_DISPLAYED_EVT event */
    switch(ForceDecimationFactorParams.ForceMode)
    {
        case STVID_FORCE_MODE_ENABLED:
            ForcedDecimationFactor = ForceDecimationFactorParams.DecimationFactor;
            break;
        ...
        default:
            ...
            if(ErrCode == ST_NO_ERROR)
            {
                ForcedDecimationFactor = MemProfile.DecimationFactor;
            }
            else
            {
                STTBX_ErrorPrint(("Error : Can't get current memory profile...\n"));
                return(TRUE);
            }
            break;
    }
    ForcedDecimationSemaphore_p = semaphore_create_fifo_timeout(0);
    SubscribeParams.NotifyCallback = (STEVT_DeviceCallbackProc_t)VID_ForcedDecimationCallback;
    ...
    if(ErrCode != ST_NO_ERROR)
    {
        STTBX_ErrorPrint(("Error : Can't subscribe to STVID_NEW_PICTURE_TO_BE_DISPLAYED_EVT...\n"));
        return(TRUE);
    }
    /* force decimation factor */
    ...
    if (!(RetErr))
    {
        /* Wait for the new decimation factor to happen */
        Time = STOS_time_plus(STOS_time_now(), ST_GetClocksPerSecond() * 6);
        if(semaphore_wait_timeout(ForcedDecimationSemaphore_p, &Time) == -1)
        {
            STTBX_ErrorPrint(("Error : Decimation didn't change within 6 seconds...\n"));
    ...
    if(ErrCode == ST_NO_ERROR)
    {
        semaphore_delete(ForcedDecimationSemaphore_p);
        ForcedDecimationSemaphore_p = NULL;
    }

    return ( RetErr );
} /* end of VID_ForceDecimationAndWait() */
...
static BOOL VID_TestEvtUnderflow(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitBufferFreeSize %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitRateValue %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad RequiredTimeJump %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad RequiredDuration %d (expected=%d)\n",
...
static BOOL VID_TestEvtDriftThreshold(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad DriftTime %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SpeedRatio %d (expected=%d)\n",
...
static BOOL VID_TestEvtPictInfo(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    {
        STTBX_ErrorPrint( ("   --> BitmapParams.Width %d is more than max allowed !\n",
    ...
    {
        STTBX_ErrorPrint( ("   --> BitmapParams.Height %d is more than max allowed !\n",
    ...
    {
        STTBX_ErrorPrint( ("   --> BitmapParams.Size1 %d is more than max allowed !\n",
    ...
    {
        STTBX_ErrorPrint( ("   --> BitmapParams.Size2 %d is more than max allowed !\n",
...
#endif
    {
        STTBX_ErrorPrint( ("   --> BitmapParams.Data1_p 0x%x is outside Shared Memory !\n",
...
#endif
    {
        STTBX_ErrorPrint( ("   --> BitmapParams.Data2_p 0x%x is outside Shared Memory !\n",
    ...
    {
        STTBX_ErrorPrint( ("   --> VideoParams.FrameRate %d is outside reasonable range !\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictInfo.BitmapParams.ColorType %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitmapParams.BitmapType %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitmapParams.PreMultipliedColor %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitmapParams.ColorSpaceConversion %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitmapParams.AspectRatio %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitmapParams.Width %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitmapParams.Height %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitmapParams.Pitch %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitmapParams.Offset %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitmapParams.Data1_p 0x%x (expected=0x%x)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitmapParams.Size1 %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitmapParams.Data2_p 0x%x (expected=0x%x)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitmapParams.Size2 %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad BitmapParams.SubByteFormat %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad VideoParams.FrameRate %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad VideoParams.ScanType %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad VideoParams.MPEGFrame %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad VideoParams.PictureStructure %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad VideoParams.TopFieldFirst %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad VideoParams.TimeCode.Hours %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad VideoParams.TimeCode.Minutes %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad VideoParams.TimeCode.Seconds %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad VideoParams.TimeCode.Frames %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad VideoParams.TimeCode.Interpolated %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad VideoParams.TimeCode.PTS.PTS %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad VideoParams.TimeCode.PTS.PTS33 %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad VideoParams.TimeCode.PTS.Interpolated %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictInfo.VideoParams.CompressionLevel %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictInfo.VideoParams.DecimationFactors %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictInfo.VideoParams.DoesNonDecimatedExist %d (expected=%d)\n",
    ...
    /* Picture quality descriptors */
    if ( strlen(ItemValue)>0 )
    {
        RetErr = STTST_EvaluateInteger( ItemValue, &SValue, 10);
        if ( !(RetErr) )
        {
            if ( (S32)RcvEventStack[LastRcvEvtIndex].EvtData.PictInfo.PictureDescriptors.PictureMeanQP != SValue )
            {
                STTBX_ErrorPrint( ("   --> bad PictInfo.PictureDescriptors.PictureMeanQP %d (expected=%d)\n",
                    RcvEventStack[LastRcvEvtIndex].EvtData.PictInfo.PictureDescriptors.PictureMeanQP, SValue ));
                CompareErr = TRUE;
            }
        }
         RetErr = STTST_GetItem( pars_p, "NC", ItemValue, sizeof(ItemValue));
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            if ( (S32)RcvEventStack[LastRcvEvtIndex].EvtData.PictInfo.PictureDescriptors.PictureVarianceQP != SValue )
            {
                STTBX_ErrorPrint( ("   --> bad PictInfo.PictureDescriptors.PictureVarianceQP %d (expected=%d)\n",
                    RcvEventStack[LastRcvEvtIndex].EvtData.PictInfo.PictureDescriptors.PictureVarianceQP, SValue ));
                CompareErr = TRUE;
            }
        }
        RetErr = STTST_GetItem( pars_p, "NC", ItemValue, sizeof(ItemValue));
...
static BOOL VID_TestEvtPictParam(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.CodingMode %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.ColorType %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.FrameRate %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.Data %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.Size %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.Height %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.Width %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.Aspect %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.ScanType %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.MPEGFrame %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.TopFieldFirst %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.TimeCode.Hours %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.TimeCode.Minutes %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.TimeCode.Seconds %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.TimeCode.Frames %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.TimeCode.Interpolated %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.TimeCode.PTS.PTS %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.TimeCode.PTS.PTS33 %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.TimeCode.PTS.Interpolated %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.TimeCode.pChromaOffset %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad PictParam.TimeCode.pTemporalReference %d (expected=%d)\n",
...
static BOOL VID_TestEvtSeqInfo(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.Height %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.Width %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.Aspect %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.ScanType %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.FrameRate %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.BitRate %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.MPEGStandard %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.IsLowDelay %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.VBVBufferSize %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.StreamID %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.ProfileAndLevelIndication %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.FrameRateExtensionN %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.FrameRateExtensionD %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SeqInfo.VideoFormat %d (expected=%d)\n",
...
static BOOL VID_TestEvtSynchro(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SyncInfo.Action %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SyncInfo.EffectiveTime %d (expected=%d)\n",
...
static BOOL VID_TestEvtUserData(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad UserData.BroadcastProfile %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad UserData.PositionInStream %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            if ( SPosition!=STVID_USER_DATA_AFTER_SEQUENCE)
            {
                ...
                {
                    STTBX_ErrorPrint( ("   --> bad UserData.pTemporalReference %d (expected=%d)\n",
            ...
            {
                STTBX_ErrorPrint(("   UserData.pTemporalReference value not meaningful here\n"));
            }
        }
        RetErr = STTST_GetItem( pars_p, "NC", ItemValue, sizeof(ItemValue));
    }
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad UserData.Length %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad UserData.BufferOverflow %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad UserData.Buff_p %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            if ( SPosition!=STVID_USER_DATA_AFTER_SEQUENCE)
            {
                ...
                {
                    STTBX_ErrorPrint( ("   --> bad UserData.PTS.PTS %d (expected=%d)\n",
            ...
            {
                STTBX_ErrorPrint(("   UserData.PTS.PTS value not meaningful here\n"));
            }
        }
        RetErr = STTST_GetItem( pars_p, "NC", ItemValue, sizeof(ItemValue));
    }
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            if ( SPosition!=STVID_USER_DATA_AFTER_SEQUENCE)
            {
                ...
                {
                    STTBX_ErrorPrint( ("   --> bad UserData.PTS.PTS33 %d (expected=%d)\n",
            ...
            {
                STTBX_ErrorPrint(("   UserData.PTS.PTS33 value not meaningful here\n"));
            }
        }
        RetErr = STTST_GetItem( pars_p, "NC", ItemValue, sizeof(ItemValue));
    }
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad UserData.PTS.Interpolated %d (expected=%d)\n",
    ...
    if ( !(RetErr) )
    {
        ...
        while ( (!(RetErr2) ) && (Count < SLength) && (SValue!= -1) )
        {
            ...
            {
                STTBX_ErrorPrint(("   --> bad user data 0x%02x in byte %d (expected=0x%02x)\n",
...
static BOOL VID_TestEvtImpossibleWithProfile(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad MemoryProfile.MaxWidth %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad MemoryProfile.MaxHeight %d (expected=%d)\n",
        ...
        if ( !(RetErr) )
        {
            ...
            switch (RcvEventStack[LastRcvEvtIndex].EvtData.MemoryProfile.NbFrameStore)
            {
                ...
                case STVID_OPTIMISED_NB_FRAME_STORE_ID :
                    ...
                    {
                        STTBX_ErrorPrint( ("   --> bad MemoryProfile.OptimisedNumber.Main %d (expected=%d)\n",
                ...
                default:
                    ...
                    {
                        STTBX_ErrorPrint( ("   --> bad MemoryProfile.NbFrameStore %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad MemoryProfile.CompressionLevel %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad MemoryProfile.DecimationFactor %d (expected=%d)\n",
...
static BOOL VID_TestEvtSynchronisationCheck(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SyncInfo.ClocksDifference %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SyncInfo.IsSynchronisationOk %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SyncInfo.IsLoosingSynchronisation %d (expected=%d)\n",
    ...
    if ( strlen(ItemValue)>0 )
    {
        ...
        if ( !(RetErr) )
        {
            ...
            {
                STTBX_ErrorPrint( ("   --> bad SyncInfo.IsBackToSynchronisation %d (expected=%d)\n",
...
static BOOL VID_TestCapability(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if ( ErrCode == ST_NO_ERROR )
    {
        ...
        {
            RetErr = TRUE;
            STTBX_ErrorPrint(("STVID_GetCapability() : unexpected Capability.SupportedXxxx data !\n" ));
        }
        ...
        {
            RetErr = TRUE;
            STTBX_ErrorPrint(("STVID_GetCapability() : unexpected Capability.XxxxCapable data !\n" ));
        }
        ...
        {
            RetErr = TRUE;
            STTBX_ErrorPrint(("STVID_GetCapability() : unexpected Capability.InputXxxx data !\n" ));
        }
        ...
        {
            RetErr = TRUE;
            STTBX_ErrorPrint(("STVID_GetCapability() : unexpected Capability.OutputXxxx data !\n" ));
        }
        ...
        {
            RetErr = TRUE;
            STTBX_ErrorPrint(("STVID_GetCapability() : unexpected Capability.SupportedHorizontalScaling.Xxxx data !\n" ));
        }
        ...
        {
            RetErr = TRUE;
            STTBX_ErrorPrint(("STVID_GetCapability() : unexpected Capability.SupportedVerticalScaling.Xxxx data !\n" ));
        }
        if (  (Capability.SupportedVerticalScaling.Continuous != FALSE)
           || (Capability.SupportedVerticalScaling.NbScalingFactors != 7)
    ...
    {
        STTBX_ErrorPrint(("STVID_GetCapability(%s,%ld) : data can't be tested due to error %d !\n",
    ...
    {
        STTBX_ErrorPrint(( "### VID_TestCapability() result : failed !\n"));
    }
    STTST_AssignInteger(result_sym_p, RetErr, FALSE);
    return (RetErr);
                            ...
                            void *SubscriberData_p)
{
    ...
    {
#if defined(ST_OS20) || defined(ST_OSWINCE)
        STTBX_ErrorPrint(("%s(%d): Preprocess event pb ! 0x%.8x \n", __FILE__, __LINE__, Event));
#else
        STTBX_ErrorPrint(("%s(): Preprocess event pb ! 0x%.8x \n", __FUNCTION__, Event));
#endif
    }
...
static BOOL VID_TestPictureParams(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    {
        semaphore_signal(PictParamTestFlag_p);
        STTBX_ErrorPrint(( "STVID_Stop(WHEN_END_OF_DATA) : failure=0x%x !\n",RetCode));
    }
    ...
    {
        STTBX_ErrorPrint(( "STVID_Stop(WHEN_END_OF_DATA) : timeout !\n"));
        RetErr = STVID_GetPictureParams(DefaultVideoHandle,STVID_PICTURE_LAST_DECODED,
    ...
    if ( ParamTestResult != 0 )
    {
        ...
        STTBX_ErrorPrint(("### PictureParams Test result : error=0x%x\n\n",
        ...
        STTBX_ErrorPrint(("### PictureInfos Test result  : error=0x%x\n",
                    ParamTestResult & 0xcccccccc));
        STTBX_ErrorPrint(("### VID_TestPictureParams() result : failed !\n"));
        RetErr = TRUE;
    }
    else
    {
...
static BOOL VID_TestSynchro(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    RetErr = TRUE;
    STTBX_ErrorPrint(("STCLKRV not in use, synchro test cant be run !\n"));
#endif /* not USE_CLKRV */
    ...
    if ( ErrorCode!= ST_NO_ERROR )
    {
        STTBX_ErrorPrint(( "STVTG-Open(%s) : failure=0x%x ! \n",
    ...
    if ( ErrorCode!= ST_NO_ERROR )
    {
        STTBX_ErrorPrint(( "STVTG_Getmode(Hndl=%d) : failure, can't retrieve VTG frequency !\n", (int)VTGHandle ));
    ...
    {
        STTBX_ErrorPrint(( "STVTG_Getmode(Hndl=%d) : unexpected FrameRate !\n", (int)VTGHandle ));
        STTBX_ErrorPrint(( "    TimingCode=%d FrameRate=%d \n", (int)VTGTimingMode, VTGModeParams.FrameRate ));
        return( FALSE );
    }
    ...
    if(DisynchroValue != 40)
    {
        ...
        {
            STTBX_ErrorPrint(("Not allowed to disynchronize more than 100 ms with speed > 100 !!\n"));
            return( FALSE );
        }
    }
    ...
    if(Tmp32 > 300000)
    {
        ...
        STTBX_ErrorPrint(("Test take too much time > 5 minutes. Abort test !!! \n"));
        API_ErrorCount++;
        return( FALSE );
    }
    ...
    if (ErrorCode != ST_NO_ERROR)
    {
        ...
        STTBX_ErrorPrint(("No STC found from STCLKRV driver (0x%x), can't proceed with test !\n", ErrorCode));
        STTBX_ErrorPrint(("Test Synchro failed: Check if PCR PID is set correctly...\n"));
        API_ErrorCount++;
        STCLKRV_Close(CLKRVHndl0);
        return(FALSE);
#else
        /* No PTI, no STC: can't proceed with test ! */
        STTBX_ErrorPrint(("No STC provided by PTI to STCLKRV driver !\n"));
        STTBX_ErrorPrint(("Test Synchro failed: cant proceed this test with no PTI\n"));
        API_ErrorCount++;
        STCLKRV_Close(CLKRVHndl0);
        return(FALSE);
    ...
    if(ErrorCode != ST_NO_ERROR)
    {
        STTBX_ErrorPrint(("Problem found while installing the callback !!!\n"));
        API_ErrorCount++;
    }
    ...
    {
        TestResult |= 0x0001;
        STTBX_ErrorPrint(("--> STVID_BACK_TO_SYNC_EVT occurrences > 0 !\n" ));
        BackSync = 0;
    }
    if(OutSync != 0)
    {
        TestResult |= 0x0001;
        STTBX_ErrorPrint(("--> STVID_OUT_OF_SYNC_EVT occurrences > 0 !\n"));
        OutSync = 0;
        LoosingSync = 0;
    }
    ...
    {
        TestResult |= 0x0001;
        STTBX_ErrorPrint(("-->  Derive occurrences > 0 !\n"));
        DeltaPos = 0;
        DeltaNeg = 0;
    }
    if (NbOfNewPict == 0) /* no pictures checked */
    {
        ...
        {
            STTBX_ErrorPrint(("-->  No check done because %d interpolated PTS received instead of 0 ! \n", NbOfPTSInterpolated));
        }
        else
        {
            STTBX_ErrorPrint(("-->  No check done because no NEW_PICTURE_TO_BE_DISPLAYED_EVT detected !\n"));
        }
    }
    if (StandardDeviationSquared > 10000 )
    {
        STTBX_ErrorPrint(("--> Standard Deviation Squared > 10000 !\n"));
        STTBX_ErrorPrint(("--> Standard Deviation > 1ms !\n"));
        StandardDeviationSquared = 0;
    }
...
if (ErrCod != ST_ERROR_FEATURE_NOT_SUPPORTED) /* No Trick Mode */
    ...
    {
        STTBX_ErrorPrint(("Error : Didn't got signal that synchro is lost...\n"));
        TestResult |= 0x10000;
    }
	...
	if (ErrCod != ST_ERROR_FEATURE_NOT_SUPPORTED) /* Trick Mode */
	{
		...
		{
			TestResult |= 0x0010;
			STTBX_ErrorPrint(("-->  Too many derive occurrences !\n"));
		}
	}
    ...
    else
    {
        ...
        {
            STTBX_ErrorPrint(( "Warning: a stream looping was detected, this may have caused the problem !\n" ));
        }
        API_ErrorCount++;
    }
    /* UnInstall callbacks */
    ...
    if(ErrorCode != ST_NO_ERROR)
    {
        STTBX_ErrorPrint(("Problem found while un-installing the callback !!!\n"));
        API_ErrorCount++;
    }
...
static BOOL VID_NullPointersTest(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    {
        STTBX_ErrorPrint(( "STVID_DataInjectionCompleted(...,NULL) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_DuplicatePicture(NULL, &PictParams);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_DuplicatePicture(NULL,...) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ...
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_GetCapability(...,NULL) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_GetDisplayAspectRatioConversion(&ViewPortHndl, NULL);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_GetDisplayAspectRatioConversion(...,NULL) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_GetHDPIPParams(DefaultVideoHandle, NULL);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_GetHDPIPParams(...,NULL,...) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_GetInputWindowMode(ViewPortHndl, NULL, &WinParams);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_GetInputWindowMode(...,NULL,...) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ...
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_GetOutputWindowMode(...,NULL,...) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ...
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_GetPictureAllocParams(...,NULL,...) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ...
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_GetPictureInfos(...,...,NULL) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_GetPictureParams(DefaultVideoHandle, STVID_PICTURE_DISPLAYED, NULL);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_GetPictureParams(...,...,NULL) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_GetSpeed(DefaultVideoHandle, NULL);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_GetSpeed(...,...,NULL) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_Init(STVID_DEVICE_NAME1, NULL);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_Init(...,NULL) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_Open(STVID_DEVICE_NAME1, NULL, &DefaultVideoHandle);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_Open(...,NULL,...) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_OpenViewPort(DefaultVideoHandle, NULL, ViewPortHndl);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_OpenViewPort(...,NULL,...) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ...
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_Pause(...,...,NULL) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_SetHDPIPParams(DefaultVideoHandle, NULL);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_SetHDPIPParams(...,NULL,...) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_SetInputWindowMode(ViewPortHndl, TRUE, NULL);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_SetInputWindowMode(...,...,NULL) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_SetMemoryProfile(DefaultVideoHandle, NULL);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_SetMemoryProfile(...,NULL) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_SetOutputWindowMode(ViewPortHndl, TRUE, NULL);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_SetOutputWindowMode(...,...,NULL) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_Setup(DefaultVideoHandle, NULL);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_Setup(...,NULL) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ErrCode = STVID_ShowPicture(ViewPortHndl, NULL, &Freeze);
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_ShowPicture(...,NULL,...) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ...
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_Clear(...,NULL) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ...
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_Init(...,(NULL,NULL,...)) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
    ...
    if ( ErrCode != ST_ERROR_BAD_PARAMETER )
    {
        STTBX_ErrorPrint(( "STVID_Clear(...,(PATTERN,NULL,...,NULL,...)) : unexpected return code=0x%x !\n", ErrCode ));
        NbErr++;
    }
...
static BOOL VID_StartTest(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if ( RetErr )
    {
        ...
        STTBX_ErrorPrint(("VID_StartTest() : STVID_SEQUENCE_INFO_EVT not found !\n"));
        API_ErrorCount++;
    }
    ...
    if ( RetErr )
    {
        STTBX_ErrorPrint(("VID_StartTest() : STVID_DISPLAY_NEW_FRAME_EVT not found !\n"));
        API_ErrorCount++;
    }
...
static BOOL VID_LengthTest(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    else
    {
        ...
        STTBX_ErrorPrint(("VID_LengthTest() : Feature not initialized, or no event occured \n"));
    }
   ...
   /* Parse Parameters ********************************** */
    ...
    if (!RetErr)
    {
        switch (StopStartMode)
        {
            ...
            default:
                  STTBX_ErrorPrint((VID_Msg, "VID_StopStart Mode must be between 0 and 3!\n" ));
                  RetErr = TRUE;
        }
    }
...
static BOOL AVMEM_Reinit( STTST_Parse_t *pars_p, char *result_sym_p )
{
    ...
    for (Count=0; Count < MAX_PARTITIONS; Count++)
    {
        ...
        {
            STTBX_ErrorPrint(("AVMEM_Reinit() : Warning STAVMEM_DeletePartition(%s) #%d ! Still allocated block\n",
    ...
    {
        STTBX_ErrorPrint(("AVMEM_Reinit() : Warning STAVMEM_Term(%s) ! Still created partition\n",
                 STAVMEM_DEVICE_NAME));
    ...
    if (ErrorCode != ST_NO_ERROR)
    {
        STTBX_ErrorPrint(("AVMEM_Term(%s): STAVMEM_Term() error 0x%0x !\n", STAVMEM_DEVICE_NAME, ErrorCode));
        RetErr = TRUE;
        return(API_EnableError ? RetErr : FALSE );
    }
    ...
    {
        STTBX_ErrorPrint(("AVMEM_Init() failed !\n"));
        RetErr = TRUE;
    }
    else
    {
        for (Count=0; (Count < MAX_PARTITIONS) && (!RetErr); Count++)
        {
            ...
            if (ErrorCode != ST_NO_ERROR)
            {
                STTBX_ErrorPrint(("AVMEM_Reinit() : AVMEM_CreatePartition() #%d failed !\n", Count));
                RetErr = TRUE;
            }
        }
        if (!(RetErr))
...
{
#if defined(ST_OSLINUX)
    STTBX_ErrorPrint(("ERROR: Not supported on linux\n"));
    return FALSE;
#else
    BOOL                                   RetErr = FALSE;
    S32                                     LVar;
    ...
    DefaultLX = 2;
#else
    STTBX_ErrorPrint((STTBX_REPORT_LEVEL_ERROR, "No Delta in this IC\n"));
#endif

    /* get LX Number */
    RetErr = STTST_GetInteger(pars_p, DefaultLX, &LVar);
    if (RetErr || ((U32)LVar > MaxNbLX))
    {
    ...
    {
        STTBX_ErrorPrint(("Not able to query DeltaTop informations for video device %d (tried %d times)\n", LVar, Trials));
    }
    else
    {
...
static BOOL VID_PingLX(STTST_Parse_t *pars_p, char *result_sym_p )
{
    ...
    if(ErrorCode != MME_SUCCESS)
    {
        STTBX_ErrorPrint(("Not able to Ping LX %d (tried %d times)\n", LVar, Trials));
    }
    else
    {
...
/* but Video LX code placed at the very beginning of the LMI_SYS */
    ...
    if(Found)
    {
        ...
        EndPos = strchr(CurrentPos, '\n');  /* find end of line */
        EndPos[0] = 0;
        strcpy(AVSFirmwareVersion, CurrentPos); /* Get AVS firmware identification */
        strcat(AVSFirmwareVersion, "\n");
        EndPos[0] = '\n';
        CurrentPos = EndPos + 1;
        EndPos = strchr(CurrentPos, '\n');  /* find end of line */
        EndPos[0] = 0;
        strcat(AVSFirmwareVersion, CurrentPos); /* Get AVS IPF firmware identification */
        EndPos[0] = '\n';
        CurrentPos = EndPos + 1;
        ...
        switch(DeviceType)
        {
            ...
            case    STVID_DEVICE_TYPE_7109_AVS :
                strcpy(FirmwareRelease, AVSFirmwareVersion);
                break;
...
#ifdef USE_FILESYSTEM
    if(SYS_FIsQVCFSFile(fp))
    {
        return(SYS_FGets(string, n, fp));
    }
    else
#endif /* USE_FILESYSTEM */
    {
...
static void VID_CRCReportFunc(const STVID_Handle_t VideoHandle)
{
    ...
    if(VID_CRC[InjectNumber].LogFileName[0] != 0)
    {
...
static BOOL VID_CRCStopCheck(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    if(RetErr == FALSE)
    {
        ...
        STTBX_Print((VID_Msg));
        ErrorCode = STVID_CRCGetCurrentResults(CurrentHandle, &CRCCheckResult);
        if(ErrorCode != ST_NO_ERROR)
        {
        ...
        STTST_AssignInteger("RET_VAL1", CRCCheckResult.NbCompCRC,0);
        STTST_AssignInteger("RET_VAL2", CRCCheckResult.NbErrors,0);
        STTST_AssignInteger("RET_VAL3", CRCCheckResult.NbRefCRCChecked, 0);
        STTST_AssignInteger("RET_VAL4", CRCCheckResult.NbErrorsAndWarnings, 0);
    }
    else
    {
        STTBX_ErrorPrint((VID_Msg));
...
/*******************************************************************************
Name        : CRCPrintValue
Description : Function subscribed to the VIDCRC event STVID_NEW_CRC_QUEUED_EVT
Parameters  : EVT API
Assumptions :
Limitations :
Returns     : EVT API
...
static BOOL VID_CRCStopTrace(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ...
    {
        RetErr = FALSE;
        STTBX_ErrorPrint(("Problem found while un-installing the CRC trace callback !!!\n"));
    }
 ...
 * Return   : FALSE if no error
 * ----------------------------------------------------------------------*/
static BOOL VID_OpenFile(STTST_Parse_t *pars_p, char *result_sym_p)
{
    FILE *inputfd;
    char FileName[VID_MAX_STREAM_NAME];
    BOOL RetErr;
    ...
    sprintf(VID_Msg, "Looking for %s ... \n", FileName);
    STTBX_Print((VID_Msg));

    inputfd = fopen(FileName, "r");
    if (inputfd != NULL)
    {
        STTBX_Print(("File %s loaded\n", FileName));
    }
    else
    {
        STTBX_Print(("unable to load file %q\n", FileName));
        return FALSE;
    }

    STTST_AssignInteger(result_sym_p, (int)inputfd, FALSE);

    return (API_EnableError ? RetErr : FALSE );
}/* End of VID_OpenFile function */

/*-------------------------------------------------------------------------
 * Function : VID_CloseFile
 * Input    :
 * Parameter:
 *
 * Output   :
 * Return   : FALSE if no error
 * ----------------------------------------------------------------------*/
static BOOL VID_CloseFile(STTST_Parse_t *pars_p, char *result_sym_p)
{
    FILE *inputfd;
    BOOL RetErr;
    S32 LVar;
    ...
    fclose(inputfd);
    STTBX_Print(("File closed\n"));

    return RetErr;
}/* End of VID_CloseFile function */

/*-------------------------------------------------------------------------
 * Function : VID_GetLine
 * Input    :
 * Parameter:
 *
 * Output   :
 * Return   : FALSE if no error
 * ----------------------------------------------------------------------*/
static BOOL VID_GetLine(STTST_Parse_t *pars_p, char *result_sym_p)
{
    FILE *inputfd;
    char Line[1024];
    BOOL RetErr;
    ...
    /* Read the newt stream file name */
    ReturnedValue = fscanf(inputfd, "%s", Line);
    if (ReturnedValue==EOF)
        STTST_AssignString("FILE_LINE","",0);
    else
        STTST_AssignString("FILE_LINE",Line,0);

    return (RetErr);
}/* End of VID_GetLine function */

#ifdef STVID_USE_FMD
/*-------------------------------------------------------------------------
 * Function : VID_FMDSetParams
 * Input    :
 * Parameter:
 *
 * Output   :
 * Return   : FALSE if no error
 * ----------------------------------------------------------------------*/
static BOOL VID_FMDSetParams(STTST_Parse_t *pars_p, char *result_sym_p)
{
    BOOL                 RetErr;
    STVID_ViewPortHandle_t  CurrentHandle;
    S32                    LVar;
    ST_ErrorCode_t  ErrorCode = ST_NO_ERROR;
    STLAYER_FMDParams_t FMDParams;
    STLAYER_FMDParams_t ReadFMDParams;

    UNUSED_PARAMETER(result_sym_p);

    sprintf(VID_Msg, "Setting FMD params: ");

    /* get Handle */

    RetErr = STTST_GetInteger(pars_p, (int)ViewPortHndl0, &LVar );
    CurrentHandle = (STVID_ViewPortHandle_t) LVar;
    if (RetErr)
    {
        STTST_TagCurrentLine(pars_p, "expected Handle" );
        return (TRUE);
    ...
    {
        ViewPortHndl0 = CurrentHandle;
    }

    RetErr = STTST_GetInteger(pars_p, 0, (S32 *)&FMDParams.CFDThreshold);
    CurrentHandle = (STVID_ViewPortHandle_t) LVar;
    if (RetErr)
    {
        STTST_TagCurrentLine(pars_p, "expected ConsecutiveFieldDifferenceThreshold" );
        return (TRUE);
    }

    RetErr = STTST_GetInteger(pars_p, 0, (S32 *)&FMDParams.BlockMoveDetectionThreshold);
    CurrentHandle = (STVID_ViewPortHandle_t) LVar;
    if (RetErr)
    {
        STTST_TagCurrentLine(pars_p, "expected BlockMoveDetectionThreshold" );
        return (TRUE);
    }

    RetErr = STTST_GetInteger(pars_p, 0, (S32 *)&FMDParams.PixelMoveDetectionThreshold);
    CurrentHandle = (STVID_ViewPortHandle_t) LVar;
    if (RetErr)
    {
        STTST_TagCurrentLine(pars_p, "expected PixelMoveDetectionThreshold" );
        return (TRUE);
    }

    RetErr = STTST_GetInteger(pars_p, 0, (S32 *)&FMDParams.RepeatFieldDetectionThreshold);
    CurrentHandle = (STVID_ViewPortHandle_t) LVar;
    if (RetErr)
    {
        STTST_TagCurrentLine(pars_p, "expected RepeatFieldDetectionThreshold" );
        return (TRUE);
    }

    RetErr = STTST_GetInteger(pars_p, 0, (S32 *)&FMDParams.SceneChangeDetectionThreshold);
    CurrentHandle = (STVID_ViewPortHandle_t) LVar;
    if (RetErr)
    {
        STTST_TagCurrentLine(pars_p, "expected SceneChangeDetectionThreshold" );
        return (TRUE);
    }

    /* setup the FMD */
    ErrorCode = STVID_SetFMDParams(CurrentHandle, &FMDParams);
    switch (ErrorCode)
    {
        case ST_NO_ERROR :
            RetErr = FALSE;
    ...
    /* read back values */
    ErrorCode = STVID_GetFMDParams(CurrentHandle, &ReadFMDParams);
    if(ErrorCode != ST_NO_ERROR || memcmp(&ReadFMDParams, &FMDParams, sizeof(STLAYER_FMDParams_t)) != 0 )
    {
            RetErr = TRUE;
            strcat(VID_Msg, "Error reading back FMD Params !\n" );
    }
    ...
    return(RetErr);
}/* End of VID_FMDSetParams function */

/*******************************************************************************
Name        : FMDPrintResults
Description : Function subscribed to the event STVID_NEW_FMD_QUEUED_EVT
Parameters  : EVT API
Assumptions :
Limitations :
Returns     : EVT API
*******************************************************************************/
static void FMDPrintResults(STEVT_CallReason_t Reason,
                                              const ST_DeviceName_t RegistrantName,
                                              STEVT_EventConstant_t Event,
                                              const void *EventData_p,
                                              const void *SubscriberData_p
...
{
    STLAYER_FMDResults_t *FMDResults_p;
    ...
    FMDResults_p = (STLAYER_FMDResults_t *)EventData_p;
    STTBX_Print(("FMD %03u (%d): FieldDiffSum=%08u  ConsecutiveFieldDiffSum=%08u SceneCount=%02d MoveStatus=%d RepeatStatus=%d\n",
                                                                                     FMDResults_p->PictureIndex,
                                                                                     FMDResults_p->FieldRepeatCount,
                                                                                     FMDResults_p->FieldSum,
                                                                                     FMDResults_p->CFDSum,
                                                                                     FMDResults_p->SceneCount,
                                                                                     FMDResults_p->MoveStatus,
                                                                                     FMDResults_p->RepeatStatus));
} /* End of FMDEndOfCheckCB() function */

/*-------------------------------------------------------------------------
 * Function : VID_FMDStartTrace
 * Input    :
 * Parameter:
 *
 * Output   :
 * Return   : FALSE if no error
 * ----------------------------------------------------------------------*/
static BOOL VID_FMDStartTrace(STTST_Parse_t *pars_p, char *result_sym_p)
{
    BOOL                 RetErr;
    STVID_ViewPortHandle_t  CurrentHandle;
    S32                    LVar;
    ...
    sprintf(VID_Msg, "Starting FMD traces: ");
    ...
    /* subscribe to new FMD reported event */
    STEVT_SubscribeParams.NotifyCallback = FMDPrintResults;
    STEVT_SubscribeParams.SubscriberData_p = (void *)CurrentHandle;
    ErrorCode = STEVT_SubscribeDeviceEvent(EvtSubHandle, NULL,
                       (STEVT_EventConstant_t)STLAYER_NEW_FMD_REPORTED_EVT, &STEVT_SubscribeParams);
    if ((ErrorCode != ST_NO_ERROR) &&
        (ErrorCode != STEVT_ERROR_ALREADY_SUBSCRIBED))
    {
        /* Error: subscribe failed, undo initialisations done */
        STTBX_ErrorPrint(( "Module FMD start: could not subscribe to STLAYER_NEW_FMD_REPORTED_EVT !"));
        return(TRUE);
    }

    /* start the FMD reporting */
    ErrorCode = STVID_EnableFMDReporting(CurrentHandle);
    switch (ErrorCode)
    {
        case ST_NO_ERROR :
            RetErr = FALSE;
    ...
    return(RetErr);
}/* End of VID_FMDStartTrace function */

/*-------------------------------------------------------------------------
 * Function : VID_FMDStopTrace
 * Input    :
 * Parameter:
 *
 * Output   :
 * Return   : FALSE if no error
 * ----------------------------------------------------------------------*/
static BOOL VID_FMDStopTrace(STTST_Parse_t *pars_p, char *result_sym_p)
{
    BOOL                 RetErr;
    STVID_ViewPortHandle_t  CurrentHandle;
    S32                    LVar;
    ...
    sprintf(VID_Msg, "Stopping FMD traces: ");
    ...
    /* stop the FMD reporting */
    ErrorCode = STVID_DisableFMDReporting(CurrentHandle);
    switch (ErrorCode)
    {
        case ST_NO_ERROR :
            RetErr = FALSE;
    ...
    ErrorCode = STEVT_UnsubscribeDeviceEvent(EvtSubHandle,
                                        NULL,
                                        (STEVT_EventConstant_t)STLAYER_NEW_FMD_REPORTED_EVT);
    if(ErrorCode != ST_NO_ERROR)
    {
        RetErr = FALSE;
        STTBX_ErrorPrint(("Problem found while un-installing the FMD trace callback !!!\n"));
    }

    return(RetErr);
}/* End of VID_FMDStopTrace function */
...
/*-------------------------------------------------------------------------
 * Function : VID_PostprocessTestPictureCallBack
 *            Callback used to store the new picture to be displayed bitmap params
 * Input    : *pars_p, *result_sym_p
 * Output   :
 * Return   : TRUE if error, FALSE if success
 * ----------------------------------------------------------------------*/
static void VID_PostprocessTestPictureCallBack(
    ...
    const void *SubscriberData_p)
{
    ...
    /* Retreive PictureInfos from EventData */
    PictureInfos_p = (STVID_PictureInfos_t *)(U32)EventData_p;

    if (PictureInfos_p != NULL)
    {

#if 0
        TraceState("NPTBD","0x%x",(U32)PictureInfos_p->VideoParams.DecimatedBitmapParams.Data1_p);
#endif

        /* Get AccessControl */
        STOS_SemaphoreWait(PostProcessTestAccessControlSemaphore_p);

        /* Get BitmapParams */
        memcpy(&PostProcessTestBitmapParams, &PictureInfos_p->BitmapParams, sizeof(STGXOBJ_Bitmap_t));

        /* Get BitmapParams. Data1_p, Data2_p, Size1, Size2 fields */
        PostProcessTestBitmapParams.Data1_p = PictureInfos_p->VideoParams.DecimatedBitmapParams.Data1_p;
        PostProcessTestBitmapParams.Size1   = PictureInfos_p->VideoParams.DecimatedBitmapParams.Size1;
        PostProcessTestBitmapParams.Data2_p = PictureInfos_p->VideoParams.DecimatedBitmapParams.Data2_p;
        PostProcessTestBitmapParams.Size2   = PictureInfos_p->VideoParams.DecimatedBitmapParams.Size2;


        /* Release AccessControl */
        STOS_SemaphoreSignal(PostProcessTestAccessControlSemaphore_p);
    }

    return;
}/* End of VID_PostprocessTestPictureCallBack function */



/*-------------------------------------------------------------------------
 * Function : VID_PostprocessTestFieldCallBack
 *            Callback used to peform pixel drawing on next displayed picture
 * Input    : *pars_p, *result_sym_p
 * Output   :
 * Return   : TRUE if error, FALSE if success
 * ----------------------------------------------------------------------*/
static void VID_PostprocessTestFieldCallBack(
    STEVT_CallReason_t Reason,
    const ST_DeviceName_t RegistrantName,
    STEVT_EventConstant_t Event,
    ...
    const void *SubscriberData_p)
{
...
#if 0
    TraceState("NFTBD","0x%x",(U32)PostProcessTestBitmapParams.Data1_p);
#endif

    /* ---------------------------------------- */
    /* Now draw an horizontal line on video frame buffer to be displayed on next Vsync */
    /* ---------------------------------------- */
    if (PostProcessTestBitmapParams.Data1_p != NULL)
    {
        /* Calculate pixel address */
        PixelAddress = (void *)((U32)PostProcessTestBitmapParams.Data1_p + PostProcessTestBitmapParams.Offset + PostProcessTestBitmapParams.Pitch*LineIndex);

        if (PixelAddress != NULL)
        {
            /* Draw an horizontal line  */
            memset(PixelAddress, POSTPROCESSING_TEST_WHITE_COLOR, (PostProcessTestBitmapParams.Pitch/10));
        }
    }
    ...
    return;
}/* End of VID_PostprocessTestFieldCallBack function */

/*-------------------------------------------------------------------------
 * Function : VID_StartPostprocessTest
 * Input    : Start Post-processing feature test
 * Parameter:
 *
 * Output   :
 * Return   : FALSE if no error
 * ----------------------------------------------------------------------*/
static BOOL VID_StartPostprocessTest(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ST_ErrorCode_t RetErr = FALSE;
    STVID_Handle_t CurrentHandle;
    S32 LVar;
    ...
    if (!(RetErr))
    {
        /* Initialize test semaphores */
        PostProcessTestAccessControlSemaphore_p = STOS_SemaphoreCreateFifoTimeOut(NULL,1);

        /* Configure STVID_NEW_FIELD_TO_BE_DISPLAYED_EVT event */
        ConfigureParams.Enable = TRUE;
        ConfigureParams.NotificationsToSkip = 0;

        RetErr = STVID_ConfigureEvent (CurrentHandle, (STEVT_EventID_t)STVID_NEW_FIELD_TO_BE_DISPLAYED_EVT, &ConfigureParams);
        if (RetErr != ST_NO_ERROR)
        {
            /* Error: Configure Event failed */
            STTBX_ErrorPrint(( "Failed to Configure Event STVID_NEW_FIELD_TO_BE_DISPLAYED_EVT !!"));
            return(TRUE);
        }
        else
        {
            /* Subscribe to event STVID_NEW_FIELD_TO_BE_DISPLAYED_EVT */
            STEVT_SubscribeParams.NotifyCallback = (STEVT_DeviceCallbackProc_t) VID_PostprocessTestFieldCallBack;
            STEVT_SubscribeParams.SubscriberData_p = (void *)CurrentHandle;
            RetErr = STEVT_SubscribeDeviceEvent(EvtSubHandle, NULL,
                               (STEVT_EventConstant_t)STVID_NEW_FIELD_TO_BE_DISPLAYED_EVT, &STEVT_SubscribeParams);
            if (RetErr != ST_NO_ERROR)
            {
                /* Error: subscribe failed, undo initialisations done */
                STTBX_ErrorPrint(( "Failed to subscribe STVID_NEW_FIELD_TO_BE_DISPLAYED_EVT !!"));
                return(TRUE);
            }
            else
            {
                /* Subscribe to event STVID_NEW_PICTURE_TO_BE_DISPLAYED_EVT */
                STEVT_SubscribeParams.NotifyCallback = (STEVT_DeviceCallbackProc_t) VID_PostprocessTestPictureCallBack;
                STEVT_SubscribeParams.SubscriberData_p = (void *)CurrentHandle;
                RetErr = STEVT_SubscribeDeviceEvent(EvtSubHandle, NULL,
                                   (STEVT_EventConstant_t)STVID_NEW_PICTURE_TO_BE_DISPLAYED_EVT, &STEVT_SubscribeParams);
                if (RetErr != ST_NO_ERROR)
                {
                    /* Error: subscribe failed, undo initialisations done */
                    STTBX_ErrorPrint(( "Failed to subscribe STVID_NEW_PICTURE_TO_BE_DISPLAYED_EVT !!"));
                    return(TRUE);
                }
            }
        }
        ...
        sprintf(VID_Msg, "StartPostprocessTest(handle=0x%x)\n", CurrentHandle );
        STTBX_Print((VID_Msg));
    }
    ...
    STTST_AssignInteger(result_sym_p, RetErr, FALSE);
    return (API_EnableError ? RetErr : FALSE );
}/* End of VID_StartPostprocessTest function */

/*-------------------------------------------------------------------------
 * Function : VID_StopPostprocessTest
 * Input    : Stop Post-processing feature test
 * Parameter:
 *
 * Output   :
 * Return   : FALSE if no error
 * ----------------------------------------------------------------------*/
static BOOL VID_StopPostprocessTest(STTST_Parse_t *pars_p, char *result_sym_p)
{
    ST_ErrorCode_t RetErr = FALSE;
    ...
    /* Unsubscribe to event STVID_NEW_FIELD_TO_BE_DISPLAYED_EVT */
    RetErr = STEVT_UnsubscribeDeviceEvent(EvtSubHandle, NULL, (STEVT_EventConstant_t)STVID_NEW_FIELD_TO_BE_DISPLAYED_EVT);
    if (RetErr != ST_NO_ERROR)
    {
        /* Error: unsubscribe failed, undo initialisations done */
        STTBX_ErrorPrint(( "Failed to unsubscribe STVID_NEW_FIELD_TO_BE_DISPLAYED_EVT !!"));
        return(TRUE);
    }
    else
    {
        /* Unsubscribe to event STVID_NEW_PICTURE_TO_BE_DISPLAYED_EVT */
        RetErr = STEVT_UnsubscribeDeviceEvent(EvtSubHandle, NULL, (STEVT_EventConstant_t)STVID_NEW_PICTURE_TO_BE_DISPLAYED_EVT);
        if (RetErr != ST_NO_ERROR)
        {
            /* Error: unsubscribe failed, undo initialisations done */
            STTBX_ErrorPrint(( "Failed to unsubscribe STVID_NEW_PICTURE_TO_BE_DISPLAYED_EVT !!"));
            return(TRUE);
        }
    }

    /* Delete test semaphores */
    STOS_SemaphoreDelete(NULL, PostProcessTestAccessControlSemaphore_p);

    sprintf(VID_Msg, "Postprocess Test Stopped\n");
    STTBX_Print((VID_Msg));

    /* Return the standard result. */
    STTST_AssignInteger(result_sym_p, RetErr, FALSE);
    return (API_EnableError ? RetErr : FALSE );
}/* End of VID_StopPostprocessTest function */
...
BOOL VID_RegisterCmd2 (void)
{
    ...
    RetErr = FALSE;
    RetErr |= STTST_RegisterCommand("VID_GETLINE",    VID_GetLine,   "[<Handle>] Reads line by line a file that contain a list of stream");
    RetErr |= STTST_RegisterCommand("VID_OPENFILE",    VID_OpenFile,   "[<FileName>] Open a file");
    RetErr |= STTST_RegisterCommand("VID_CLOSEFILE",    VID_CloseFile,   "[<FileDescriptor>] Close a file");
...
#ifdef STVID_USE_FMD
    RetErr |= STTST_RegisterCommand( "VID_FMDTRACEON",     VID_FMDStartTrace, "<ViewPort Handle>");
    RetErr |= STTST_RegisterCommand( "VID_FMDTRACEOFF",    VID_FMDStopTrace, "<ViewPort Handle>");
    RetErr |= STTST_RegisterCommand( "VID_FMDSETPARAMS",  VID_FMDSetParams, "<ViewPort Handle>\n\t\t   <CFDThreshold> 0-15\n\t\t   <BlockMoveDetectionThreshold> 0-4095\n\t\t   <PixelMoveDetectionThreshold> 0-255 \n\t\t   <RepeatFieldDetectionThreshold> 0-1048575 \n\t\t   <SceneChangeDetectionThreshold> 0-1048575");
#endif /* STVID_USE_FMD */
    ...
    RetErr |= STTST_RegisterCommand( "VID_FDW",      VID_ForceDecimationAndWait, "<VideoName> <Handle> <Mode> <Factor> Forces the current decimation factor and waits until a new decimated picture is displayed");
#ifndef STVID_NO_DISPLAY
...
#if defined(USE_FILESYSTEM)
    RetErr |= STTST_RegisterCommand( "VID_LOAD",     VID_Load,      "<Stream name><Load buffer nb><Direct injection> Load a file in memory");
#else  /* #if defined(USE_FILESYSTEM) ... */
    RetErr |= STTST_RegisterCommand( "VID_LOAD",     VID_Load,      "<Stream name><Load buffer nb> Load a file in memory");
#endif /* #if defined(USE_FYLESYSTEM) ... #else ... */
	...
	/* Post-processing test related */
    RetErr |= STTST_RegisterCommand("VID_PostProcessTestStart",  VID_StartPostprocessTest,    "Start Postprocess Test");
    RetErr |= STTST_RegisterCommand("VID_PostProcessTestStop",   VID_StopPostprocessTest,     "Stop Postprocess Test");
