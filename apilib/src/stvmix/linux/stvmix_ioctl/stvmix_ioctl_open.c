...
/*=============================================================================
  ...
  ===========================================================================*/
int stvmix_ioctl_open(struct inode *node, struct file *filp)
{
 ...
 /* int major = imajor(node);         *//* The major number */
    ...
    if (NULL == filp->private_data) {
        ...
        /* stvmix_ioctl_NewNode() may deshedule - call must be reentrant */
        filp->private_data = stvmix_ioctl_NewNode(minor);
...
/*=============================================================================
   ...
   stvmix_ioctl_release   (close)

   This function can deschedule and MUST be reentrant.

  ===========================================================================*/
int stvmix_ioctl_release(struct inode *node, struct file *filp)  /* close */
{
...
static stvmix_ioctl_dev_t *table[16];


/*=============================================================================

   stvmix_ioctl_NewNode
  ...
  ===========================================================================*/
stvmix_ioctl_dev_t *stvmix_ioctl_NewNode(unsigned int id)
{
    /* Check valid id */
    ...
    down(&(stvmix_ioctl_lock)); /* We can deshedule here !!! */
    ...
    if (NULL == table[id]) {
        ...
        table[id] = (stvmix_ioctl_dev_t*)kmalloc(sizeof(stvmix_ioctl_dev_t), GFP_KERNEL);
    ...
    up(&(stvmix_ioctl_lock));  /* Release the global lock */
...
/*=============================================================================
   ...
   stvmix_ioctl_GetNode

   Get an existing data structure for this 'device' (minor number)

  ===========================================================================*/
stvmix_ioctl_dev_t *stvmix_ioctl_GetNode(unsigned int id)
{
    /* Check valid id */
