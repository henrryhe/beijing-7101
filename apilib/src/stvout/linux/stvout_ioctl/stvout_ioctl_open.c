...
/*=============================================================================
  ...
  ===========================================================================*/
int stvout_ioctl_open(struct inode *node, struct file *filp)
{
 ...
 /* int major = imajor(node);         *//* The major number */
    ...
    if (NULL == filp->private_data) {
        ...
        /* stvout_ioctl_NewNode() may deshedule - call must be reentrant */
        filp->private_data = stvout_ioctl_NewNode(minor);
...
/*=============================================================================
   ...
   stvout_ioctl_release   (close)

   This function can deschedule and MUST be reentrant.

  ===========================================================================*/
int stvout_ioctl_release(struct inode *node, struct file *filp)  /* close */
{
...
static stvout_ioctl_dev_t *table[16];


/*=============================================================================

   stvout_ioctl_NewNode
  ...
  ===========================================================================*/
stvout_ioctl_dev_t *stvout_ioctl_NewNode(unsigned int id)
{
    /* Check valid id */
    ...
    down(&(stvout_ioctl_lock)); /* We can deshedule here !!! */
    ...
    if (NULL == table[id]) {
        ...
        table[id] = (stvout_ioctl_dev_t*)kmalloc(sizeof(stvout_ioctl_dev_t), GFP_KERNEL);
    ...
    up(&(stvout_ioctl_lock));  /* Release the global lock */
...
/*=============================================================================
   ...
   stvout_ioctl_GetNode

   Get an existing data structure for this 'device' (minor number)

  ===========================================================================*/
stvout_ioctl_dev_t *stvout_ioctl_GetNode(unsigned int id)
{
    /* Check valid id */
