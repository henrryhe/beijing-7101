...
#if 1 /*yxl 2007-03-22 modify below*/
  DELAY_US(4);
#else
  task_delay(6*ST_GetClocksPerSecond());
#endif
...
/*	STTBX_Print(("LLD_UNLOCK_ADDR1=%lx %lx LLD_UNLOCK_ADDR2=%lx %lx NOR_AUTOSELECT_CMD=%lx",
		LLD_UNLOCK_ADDR1, NOR_UNLOCK_DATA1,LLD_UNLOCK_ADDR2, NOR_UNLOCK_DATA2,NOR_AUTOSELECT_CMD));
*/	
	FLASH_WR(base_addr, LLD_UNLOCK_ADDR1, NOR_UNLOCK_DATA1);
/*  while(Count--);
  Count=CountA;
