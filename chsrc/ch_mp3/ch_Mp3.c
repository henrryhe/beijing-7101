...
static int g_bIsMp3Init = false;
#ifdef USE_IPANEL
#include "..\eis\eis_api_media.h"

static int  mp3_data_len = 0;
...
void CH_GetDecodedPCMData(U16* uST_GetBuffer, U32* uST_Length )
{
	int	i_Loop;

    //printf("CH_GetDecodedPCMData\n");
#if 1 	/*sqzow20100607*/
...
void CH_MP3_Pause(void)
{
    //printf("\nCH_MP3_Pause\n");
    //mp3模块初始化以后才能对他进行操作
    if (g_bIsMp3Init == false)
        return;

    /*if (semMp3DataBuf == NULL)
        printf("semMp3DataBuf == NULL!!!\n");
    else
        printf("semaphore_value: %d\n", semaphore_value(semMp3DataBuf));*/

    //先等待信号量再suspend gs_mp3CtrlHdl.PlayTaskID这个线程
    //因为信号量是gs_mp3CtrlHdl.PlayTaskID的CH_GetDecodedPCMData释放的
	#ifdef USE_IPANEL
		if(NULL != semMp3DataBuf)
        {
		    semaphore_wait(semMp3DataBuf);
...
boolean CH_IPANEL_MP3Start(unsigned char * mp3_buf, int len)
{
	...
	if(NULL != semMp3DataBuf)
	{
		...
		#ifdef MP3_DATA
			buffer_size = MP3_DECODE_BUFFER_SIZE;
		#else
			buffer_size = PCM_INJECT_BUFFER_SIZE;
		#endif
		
		
		if((mp3_data_len + len) <= buffer_size)		
		{
		...
		else
		{
			...
			sttbx_Print("inject date in splice\n");			
		
			while(len > 0){
				injsize = len;
				if(injsize > buffer_size - mp3_data_len){
					injsize = buffer_size - mp3_data_len;
				}
				
				memcpy(mp3_data_buf + mp3_data_len, mp3_buf + offset, injsize);
				mp3_data_len += injsize;
				semaphore_signal(semMp3DataBuf);
