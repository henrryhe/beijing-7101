...
enum mad_flow receive(int fd, void **message, unsigned int *size)
{
  ...
  if (result == MAD_FLOW_CONTINUE) {
    ...
    if (*size > 0) {
      if (*message == 0) {
	*message = CH_MP3_MemoryAllocate(*size);/*malloc(*size) 20070704 change*/
	if (*message == 0)
	  return MAD_FLOW_BREAK;
      }
...
enum mad_flow check_message(struct mad_decoder *decoder)
{
  ...
  if (message)
  	CH_MP3_MemoryDeallocate(message);
   /* free(message); wz 20070704 del*/
...
int mad_decoder_run(struct mad_decoder *decoder, enum mad_decoder_mode mode)
{
  ...
  decoder->sync = CH_MP3_MemoryAllocate(sizeof(*decoder->sync));/*malloc(sizeof(*decoder->sync))*/
  if (decoder->sync == 0)
    return -1;

  result = run(decoder);

 CH_MP3_MemoryDeallocate(decoder->sync);
 /* free(decoder->sync); 20070704 del */
  decoder->sync = 0;
