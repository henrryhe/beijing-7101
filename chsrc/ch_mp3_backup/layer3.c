...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  if (stream->main_data == 0) {
    stream->main_data = CH_MP3_MemoryAllocate(MAD_BUFFER_MDLEN);/* malloc(MAD_BUFFER_MDLEN) 20070704 change */
    if (stream->main_data == 0) {
  ...
  if (frame->overlap == 0) {
    frame->overlap = CH_MP3_MemoryAllocate(2 * 32 * 18*sizeof(mad_fixed_t));
    if (frame->overlap == 0) {
      stream->error = MAD_ERROR_NOMEM;
      return -1;
    }
