...
/* declarations */
U8 CH_DurationTransform2(U16 Value);
void CH_DurationTransform(STBLAST_Symbol_t* SymbolBuf_p,
                        const U32 SymbolsAvailable, 
                        U8 * DurationArray);

U8 CH_ValidateHeader(U8* LevelArray, U8 *DurationArray, U8 DurationArraySize );
                        ...
                        U8 * DurationArray)
{
    ...
    {
        *DurationArray = CH_DurationTransform2( SymbolBuf_p->MarkPeriod );
        DurationArray++;
        *DurationArray = CH_DurationTransform2( SymbolBuf_p->SymbolPeriod - SymbolBuf_p->MarkPeriod);
        DurationArray++;
        SymbolBuf_p++;         
    }
                               ...
                               U32                  *SymbolsUsed_p)
{
 ...
 /* generate correct interpretation from inverted IR receive input */
    ...
    *NumDecoded_p = 0;
    ...
    CH_DurationTransform(SymbolBuf_p, SymbolsAvailable, DurationArray);


    /*** HEADER VALIDATION ***/  
    CustCodeDurationOneIndex = CH_ValidateHeader(LevelArray, DurationArray, 2*SymbolsAvailable);
    ...
    if (LevelArraySize != /*2*(8 + ProtocolParams_p->RC6A.NumberPayloadBits)*/64)
    {
        *NumDecoded_p = 0;
        return ST_NO_ERROR;
    } 
