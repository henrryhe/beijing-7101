...
//#define USE_BMP_SCALE /* for 更改CVBS输出模糊问题*/
...
#ifdef SUMA_SECURITY
#define CH_NCACHE_PARTITION_SIZE           /*(0x01500000)*/(0x01900000)
#else
#define CH_NCACHE_PARTITION_SIZE           (0x00300000+0x00300000+0x00400000+0x00600000) 
#endif
...
#define DecompressBuffer_SIZE (1024*512)
...
unsigned int    gAppAddr = (unsigned int)Appl_Mem;
...
#else
    InitParams.EventProcessPriority   =248;
    InitParams.InterruptProcessPriority            = 255;
#endif  //<!-- ZQiang 2010/12/23 16:12:11 --!>
...
#if 1 /*yxl 2007-01-02 cancel below,yxl 2008-05-07 reenable this*/
	ErrCode = STCLKRV_SetSTCSource(CLKRVHandle, STCLKRV_STC_SOURCE_PCR);
    if (ErrCode != ST_NO_ERROR)
    {
...
BOOL CH_VideoDecoderControl(VID_Device VIDDevice,BOOL Status)
{
	...
	if(Status==TRUE)
	{
		...
		ErrCode = STPTI_BufferSetOverflowControl(BufHTemp, TRUE);/*FALSE*/
...
/*end yxl 2006-04-12 add below section for test*/
	...
	{
		MemoryProfile.NbFrameStore = 8;
	}

#endif

	MemoryProfile.CompressionLevel = STVID_COMPRESSION_LEVEL_NONE;
	MemoryProfile.DecimationFactor = STVID_DECIMATION_FACTOR_2;/*H,2,H2ok*/
...
#if 0 /*yxl 2007-10-13 cancel for problem*/
	/*if(VIDDevice==VID_H) yxl 2007-10-11 temp cancel this line for test*/
	{
...
#endif /*end yxl 2007-10-13 temp cancel for problem*/
	...
	if(VIDDevice==VID_H)
	{
		SetupParams.SetupObject = STVID_SETUP_PICTURE_PARAMETER_BUFFERS_PARTITION;
		SetupParams.SetupSettings.AVMEMPartition = AVMEMHandle[TEST_AVM];
		ErrCode=STVID_Setup(VIDHandle[VIDDevice],&SetupParams);
		if (ErrCode != ST_NO_ERROR)
		{
...
#if 1
ErrCode = STVID_SetErrorRecoveryMode(VIDHandle[VIDDevice],/*STVID_ERROR_RECOVERY_HIGH*/STVID_ERROR_RECOVERY_FULL);
    if (ErrCode != ST_NO_ERROR)
    {
...
BOOL MBX_Setup(void) 
{
	...
	{
		Lx_Code_Address[i] = Main_Companion_71XX_Audio_Code[i]; 
	}
#endif
...
/*yxl 2007-06-12 add below section*/
...
#if 0 /*yxl 2007-07-04 modify below section*/
	InitParams.PCMReaderMode.MSBFirst   	= TRUE;
	InitParams.PCMReaderMode.Alignment     	= STAUD_DAC_DATA_ALIGNMENT_LEFT;
	InitParams.PCMReaderMode.Padding	    = TRUE;
	InitParams.PCMReaderMode.FallingSCLK	= FALSE;
	...
	InitParams.NumChPCMReader				= 1;
#else
	InitParams.PCMReaderMode.MSBFirst   	= FALSE;
	InitParams.PCMReaderMode.Alignment     	= STAUD_DAC_DATA_ALIGNMENT_LEFT;
	InitParams.PCMReaderMode.Padding	    = FALSE;
	InitParams.PCMReaderMode.FallingSCLK	= FALSE;
	InitParams.PCMReaderMode.LeftLRHigh		= FALSE;
	InitParams.PCMReaderMode.Precision		= STAUD_DAC_DATA_PRECISION_24BITS;
	InitParams.PCMReaderMode.BitsPerSubFrame = STAUD_DAC_NBITS_SUBFRAME_32;
	InitParams.PCMReaderMode.MemFormat		= STAUD_PCMR_BITS_16_0;
	InitParams.PCMReaderMode.Rounding		= STAUD_NO_ROUNDING_PCMR;
	InitParams.PCMReaderMode.Frequency		= 0; 
	InitParams.NumChPCMReader				= 0;
#endif /*end yxl 2007-07-04 modify below section*/
...
#if 0
	/*zxc 2008-12-13 add for HDMI audio not put in SanSung and Sony LCD*/
	{
		STAUD_DRInitParams_t AUD_DRInitParams;
		AUD_DRInitParams.SBRFlag=TRUE;
		ErrCode=STAUD_DRSetInitParams(AUDHandle,STAUD_OBJECT_DECODER_COMPRESSED0,&AUD_DRInitParams);
		if (ErrCode!=ST_NO_ERROR)
		{
			return(ErrCode);
		}
...
#if 1 /*yxl 2007-01-17 cancel below section for 71XXnewdriver,yxl 2007-08-09 reenable below section*/
	{
		STAUD_OutputParams_t AUD_OutputParams;
		ErrCode=STAUD_OPGetParams(AUDHandle,STAUD_OBJECT_OUTPUT_PCMP0,&AUD_OutputParams);
		if (ErrCode!=ST_NO_ERROR)
...

{
	...
	gpu8_DecompressBuffer=memory_allocate(DecompressBuffer_partition,DecompressBuffer_SIZE);
		...
		GDPParam.PicStartAdr=(unsigned int)(&(*gpu8_DecompressBuffer));
...
/*-------------------------------函数定义注释头--------------------------------------
    FuncName:CH6_GetSPDIFDigtalOutput
    Purpose: 获得spdif的实际输出格式
    Reference: 
    in:
        void >> 
    out:
...
notice : 
    sqZow 2010/07/31 18:16:10 
---------------------------------函数体定义----------------------------------------*/
 STAUD_DigitalMode_t CH6_GetSPDIFDigtalOutput(void)
{
	ST_ErrorCode_t ErrCode;
	...
	ErrCode=STAUD_GetDigitalOutput(AUDHandle,&DigitalOutput);
	if (ErrCode!=ST_NO_ERROR)
	{
		/*do_report(0,"STAUD_SetDigitalOutput()=%s\n", GetErrorText(ErrCode) );*/
		return STAUD_DIGITAL_MODE_NONCOMPRESSED;
	}    
	return DigitalOutput.DigitalMode;
}               
/************************函数说明***********************************/
...
#ifdef SUMA_SECURITY
#define            APPL2_MEM_SIZE (38*1024*1024)
#else
...
#ifdef USE_BMP_SCALE
int bmscaler(int dest_width,int dest_height,int source_width, int source_height, U16 *DesBitmap, U16 *bitmap);
#endif
/*当前的OSD 屏幕分辨率*/
...
boolean CH_CreateViewPort_HighSolution(CH_RESOLUTION_MODE_e renum_mode,CH_VideoOutputAspect rch_Ratio,CH_VideoOutputMode rch_HDMode)
{
	...
	for(i=0;i<2;i++)
	{
		...
		CH6VPOSD.LayerViewPortHandle[i] = g_VPOSD_HighSolution[i];/* sqzow20100625*/
...
#ifdef EIS_OSD_YASUO
    	{
	extern unsigned char 	*cvbs_draw_buffer;

	CH_RESOLUTION_MODE_e enum_Mode;
	enum_Mode = CH_GetReSolutionRate() ;
	if((enum_Mode == CH_1280X720_MODE)&&(NULL!=cvbs_draw_buffer))
	{
	      st_Rect.PositionY = CH_Switch_CVBS_Osd_y(ri_SourcePosY);  
	      st_Rect.Height = (ri_Height*576)/720;
		ri_DesPosY=CH_Switch_CVBS_Osd_y(ri_DesPosY);
			st_BMPT.Height = 576;
		st_BMPT.Data1_p = cvbs_draw_buffer;
	}
	}
#endif
...
boolean  CH_SetViewPortAlpha_HighSolution(int AlphaValue)
{
	...
	for(i=0;i<2;i++)
	{
		GloabalAlpha.A0 = AlphaValue/*0x80*/; 
		GloabalAlpha.A1 = 0x80;
		error = STLAYER_SetViewPortAlpha( g_VPOSD_HighSolution[i], &GloabalAlpha);
		if( error != ST_NO_ERROR )
		{
			STTBX_Print(( "\n[GRA]->>STLAYER_OpenViewPort()=%s",GetErrorText(error) ));
...
/*调用的主要函数列表：                                                                                 */
/*返回值说明：无                                                                                                  */
 boolean CH_PutBakOSD(int ri_SourcePosX, int ri_SourcePosY,int ri_Width,int ri_Height,U8* rup_pBMPData,int ri_DesPosX, int ri_DesPosY )
 {
      int i_ErrCode;
      STGXOBJ_Rectangle_t st_Rect;
      STGXOBJ_Bitmap_t    st_BMPT;
...
int bmscaler(int dest_width,int dest_height,int source_width, int source_height, U16 *DesBitmap, U16 *bitmap)
{
		...
		(destination_height > source_height))
	{
		do	/*destination fatter (x-axis) than source bitmap*/
		{
			...
			{
				color0=bitmap[source_y*source_width*4+source_x*4];
				color1=bitmap[source_y*source_width*4+source_x*4+1];
				color2=bitmap[source_y*source_width*4+source_x*4+2];
				color3=bitmap[source_y*source_width*4+source_x*4+3];
				if((screen_y*destination_width+screen_x)<destination_width*destination_height)/*zxg 030228 fix the bug*/
					{
					*(DesBitmap+screen_y*destination_width*4+screen_x*4)=color0;
					*(DesBitmap+screen_y*destination_width*4+screen_x*4+1)=color1;
					*(DesBitmap+screen_y*destination_width*4+screen_x*4+2)=color2;
					*(DesBitmap+screen_y*destination_width*4+screen_x*4+3)=color3;
					}
...
#endif
#endif
void CH_TestVIDStatitics(void)
{
			     STVID_Statistics_t Statistics;
				 int i_test = 0;
			   STVID_ResetStatistics(VIDDeviceName[0],&Statistics);
	                 task_delay(ST_GetClocksPerSecond()*10);
                        STVID_GetStatistics(VIDDeviceName[0],&Statistics);	
						i_test++;
...
void CH_STVID_GetStatistics(void)
{
	int ErrCode;
	 STVID_Statistics_t   VID_Statistics;

 	memset(&VID_Statistics,0,sizeof(STVID_Statistics_t));
	
 	ErrCode=STVID_GetStatistics(VIDDeviceName[1], &VID_Statistics);
 	if (ErrCode!=ST_NO_ERROR)
  	{
	   	do_report(0,"\n error --> Unable to get informations from the video decoder!\n");
	   	return(TRUE);
  	}

	  do_report(0,"\n----------------------------\n\n");
	  do_report(0,"\n-- Misc --\n");
	  do_report(0,"\nApiPbLiveResetWaitForFirstPictureDetected   = %u\n",VID_Statistics.ApiPbLiveResetWaitForFirstPictureDetected);
	  do_report(0,"\nApiPbLiveResetWaitForFirstPictureDecoded    = %u\n",VID_Statistics.ApiPbLiveResetWaitForFirstPictureDecoded);
	  do_report(0,"\nApiPbLiveResetWaitForNextPicture            = %u\n",VID_Statistics.ApiPbLiveResetWaitForNextPicture);
	  do_report(0,"\n-- Synchro --\n");
	  do_report(0,"\nAvsyncSkippedFields                         = %u\n",VID_Statistics.AvsyncSkippedFields);
	  do_report(0,"\nAvsyncRepeatedFields                        = %u\n",VID_Statistics.AvsyncRepeatedFields);
	  do_report(0,"\nAvsyncMaxRepeatedFields                     = %u\n",VID_Statistics.AvsyncMaxRepeatedFields);
	  do_report(0,"\nAvsyncFailedToSkipFields                    = %u\n",VID_Statistics.AvsyncFailedToSkipFields);
	  do_report(0,"\nAvsyncExtendedSTCAvailable                  = %u\n",VID_Statistics.AvsyncExtendedSTCAvailable);
	  do_report(0,"\nAvsyncPictureWithNonInterpolatedPTS         = %u\n",VID_Statistics.AvsyncPictureWithNonInterpolatedPTS);
	  do_report(0,"\nAvsyncPictureCheckedSynchronizedOk          = %u\n",VID_Statistics.AvsyncPictureCheckedSynchronizedOk);
	  do_report(0,"\nAvsyncPTSInconsistency                      = %u\n",VID_Statistics.AvsyncPTSInconsistency);
	  do_report(0,"\n-- Decode --\n");
	  do_report(0,"\nDecodeHardwareSoftReset                     = %u\n",VID_Statistics.DecodeHardwareSoftReset);
	  do_report(0,"\nDecodeStartCodeFound                        = %u\n",VID_Statistics.DecodeStartCodeFound);
	  do_report(0,"\nDecodeSequenceFound                         = %u\n",VID_Statistics.DecodeSequenceFound);
	  do_report(0,"\nDecodeUserDataFound                         = %u\n",VID_Statistics.DecodeUserDataFound);
	  do_report(0,"\nDecodePictureFound                          = %u\n",VID_Statistics.DecodePictureFound);
	  do_report(0,"\nDecodePictureFoundMPEGFrameI                = %u\n",VID_Statistics.DecodePictureFoundMPEGFrameI);
	  do_report(0,"\nDecodePictureFoundMPEGFrameP                = %u\n",VID_Statistics.DecodePictureFoundMPEGFrameP);
	  do_report(0,"\nDecodePictureFoundMPEGFrameB                = %u\n",VID_Statistics.DecodePictureFoundMPEGFrameB);
	  do_report(0,"\nDecodePictureSkippedRequested               = %u\n",VID_Statistics.DecodePictureSkippedRequested);
	  do_report(0,"\nDecodePictureSkippedNotRequested            = %u\n",VID_Statistics.DecodePictureSkippedNotRequested);
	  do_report(0,"\nDecodePictureDecodeLaunched                 = %u\n",VID_Statistics.DecodePictureDecodeLaunched);
	  do_report(0,"\nDecodeStartConditionVbvDelay                = %u\n",VID_Statistics.DecodeStartConditionVbvDelay);
	  do_report(0,"\nDecodeStartConditionPtsTimeComparison       = %u\n",VID_Statistics.DecodeStartConditionPtsTimeComparison);
	  do_report(0,"\nDecodeStartConditionVbvBufferSize           = %u\n",VID_Statistics.DecodeStartConditionVbvBufferSize);
	  do_report(0,"\nDecodeInterruptStartDecode                  = %u\n",VID_Statistics.DecodeInterruptStartDecode);
	  do_report(0,"\nDecodeInterruptPipelineIdle                 = %u\n",VID_Statistics.DecodeInterruptPipelineIdle);
	  do_report(0,"\nDecodeInterruptDecoderIdle                  = %u\n",VID_Statistics.DecodeInterruptDecoderIdle);
	  do_report(0,"\nDecodeInterruptBitBufferEmpty               = %u\n",VID_Statistics.DecodeInterruptBitBufferEmpty);
	  do_report(0,"\nDecodeInterruptBitBufferFull                = %u\n",VID_Statistics.DecodeInterruptBitBufferFull);
	  do_report(0,"\nDecodePbStartCodeFoundInvalid               = %u\n",VID_Statistics.DecodePbStartCodeFoundInvalid);
	  do_report(0,"\nDecodePbStartCodeFoundVideoPES              = %u\n",VID_Statistics.DecodePbStartCodeFoundVideoPES);
	  do_report(0,"\nDecodePbMaxNbInterruptSyntaxErrorPerPicture = %u\n",VID_Statistics.DecodePbMaxNbInterruptSyntaxErrorPerPicture);
	  do_report(0,"\nDecodePbInterruptSyntaxError                = %u\n",VID_Statistics.DecodePbInterruptSyntaxError);
	  do_report(0,"\nDecodePbInterruptDecodeOverflowError        = %u\n",VID_Statistics.DecodePbInterruptDecodeOverflowError);
	  do_report(0,"\nDecodePbInterruptDecodeUnderflowError       = %u\n",VID_Statistics.DecodePbInterruptDecodeUnderflowError);
	  do_report(0,"\nDecodePbDecodeTimeOutError                  = %u\n",VID_Statistics.DecodePbDecodeTimeOutError);
	  do_report(0,"\nDecodePbInterruptMisalignmentError          = %u\n",VID_Statistics.DecodePbInterruptMisalignmentError);
	  do_report(0,"\nDecodePbInterruptQueueOverflow              = %u\n",VID_Statistics.DecodePbInterruptQueueOverflow);
	  do_report(0,"\nDecodePbHeaderFifoEmpty                     = %u\n",VID_Statistics.DecodePbHeaderFifoEmpty);
	  do_report(0,"\nDecodePbVbvSizeGreaterThanBitBuffer         = %u\n",VID_Statistics.DecodePbVbvSizeGreaterThanBitBuffer);
	  do_report(0,"\nDecodeMinBitBufferLevelReached              = %u\n",VID_Statistics.DecodeMinBitBufferLevelReached);
	  do_report(0,"\nDecodeMaxBitBufferLevelReached              = %u\n",VID_Statistics.DecodeMaxBitBufferLevelReached);
	  do_report(0,"\nDecodePbSequenceNotInMemProfileSkipped      = %u\n",VID_Statistics.DecodePbSequenceNotInMemProfileSkipped);
	  do_report(0,"\nDecodePbParserError                         = %u\n",VID_Statistics.DecodePbParserError);
	  do_report(0,"\nDecodePbPreprocError                        = %u\n",VID_Statistics.DecodePbPreprocError);
	  do_report(0,"\nDecodePbFirmwareError                       = %u\n",VID_Statistics.DecodePbFirmwareError);
	  do_report(0,"\nDecodeGNBvd42696Error                       = %u\n",VID_Statistics.DecodeGNBvd42696Error);
	 do_report(0,"\nDecodeTimeNbPictures                        = %u\n",VID_Statistics.DecodeTimeNbPictures);
	  do_report(0,"\nDecodeTimeMinPreprocessTime (us)            = %u\n",VID_Statistics.DecodeTimeMinPreprocessTime);



	  do_report(0,"\nDecodeTimeMaxPreprocessTime (us)            = %u\n",VID_Statistics.DecodeTimeMaxPreprocessTime);
	  do_report(0,"\nDecodeTimeSumOfPreprocessTime (us)          = %u\n",VID_Statistics.DecodeTimeSumOfPreprocessTime);
	  do_report(0,"\nDecodeTimeMinBufferSearchTime (us)          = %u\n",VID_Statistics.DecodeTimeMinBufferSearchTime);
	  do_report(0,"\nDecodeTimeMaxBufferSearchTime (us)          = %u\n",VID_Statistics.DecodeTimeMaxBufferSearchTime);
	  do_report(0,"\nDecodeTimeSumOfBufferSearchTime (us)        = %u\n",VID_Statistics.DecodeTimeSumOfBufferSearchTime);
	  do_report(0,"\nDecodeTimeMinDecodeTime (us)                = %u\n",VID_Statistics.DecodeTimeMinDecodeTime);
	  do_report(0,"\nDecodeTimeMaxDecodeTime (us)                = %u\n",VID_Statistics.DecodeTimeMaxDecodeTime);
	  do_report(0,"\nDecodeTimeSumOfDecodeTime (us)              = %u\n",VID_Statistics.DecodeTimeSumOfDecodeTime);
	  do_report(0,"\nDecodeTimeMinFullDecodeTime (us)            = %u\n",VID_Statistics.DecodeTimeMinFullDecodeTime);
	  do_report(0,"\nDecodeTimeMaxFullDecodeTime (us)            = %u\n",VID_Statistics.DecodeTimeMaxFullDecodeTime);
	  do_report(0,"\nDecodeTimeSumOfFullDecodeTime (us)          = %u\n",VID_Statistics.DecodeTimeSumOfFullDecodeTime);
	  do_report(0,"\n-- Display --\n");
	  do_report(0,"\nDisplayPictureInsertedInQueue               = %u\n",VID_Statistics.DisplayPictureInsertedInQueue);
	  do_report(0,"\nDisplayPictureInsertedInQueueDecimated      = %u\n",VID_Statistics.DisplayPictureInsertedInQueueDecimated);
	  do_report(0,"\nDisplayPictureDisplayedByMain               = %u\n",VID_Statistics.DisplayPictureDisplayedByMain);
	  do_report(0,"\nDisplayPictureDisplayedByAux                = %u\n",VID_Statistics.DisplayPictureDisplayedByAux);
	  do_report(0,"\nDisplayPictureDisplayedBySec                = %u\n",VID_Statistics.DisplayPictureDisplayedBySec);
	  do_report(0,"\nDisplayPictureDisplayedDecimatedByMain      = %u\n",VID_Statistics.DisplayPictureDisplayedDecimatedByMain);
	  do_report(0,"\nDisplayPictureDisplayedDecimatedByAux       = %u\n",VID_Statistics.DisplayPictureDisplayedDecimatedByAux);
	  do_report(0,"\nDisplayPictureDisplayedDecimatedBySec       = %u\n",VID_Statistics.DisplayPictureDisplayedDecimatedBySec);
	  do_report(0,"\nDisplayPbQueueLockedByLackOfPicture         = %u\n",VID_Statistics.DisplayPbQueueLockedByLackOfPicture);
	// do_report(0,"\nDisplayPbVSyncDuringDisplayProgramming      = %u\n",VID_Statistics.DisplayPbVSyncDuringDisplayProgramming);
	  do_report(0,"\nDisplayPbQueueOverflow                      = %u\n",VID_Statistics.DisplayPbQueueOverflow);
	  do_report(0,"\n-- Queue --\n");
	  do_report(0,"\nQueuePictureInsertedInQueue                 = %u\n",VID_Statistics.QueuePictureInsertedInQueue);
	  do_report(0,"\nQueuePictureRemovedFromQueue                = %u\n",VID_Statistics.QueuePictureRemovedFromQueue);
	  do_report(0,"\nQueuePicturePushedToDisplay                 = %u\n",VID_Statistics.QueuePicturePushedToDisplay);
	  do_report(0,"\n-- Speed --\n");
	  do_report(0,"\nQueuePbPictureTooLateRejected               = %u\n",VID_Statistics.QueuePbPictureTooLateRejected);
	  do_report(0,"\nSpeedDisplayBFramesNb                       = %u\n",VID_Statistics.SpeedDisplayBFramesNb);
	  do_report(0,"\nSpeedDisplayPFramesNb                       = %u\n",VID_Statistics.SpeedDisplayPFramesNb);
	  do_report(0,"\nSpeedDisplayIFramesNb                       = %u\n",VID_Statistics.SpeedDisplayIFramesNb);
	  do_report(0,"\nMaxBitRate                                  = %u\n",VID_Statistics.MaxBitRate);
	  do_report(0,"\nMinBitRate                                  = %u\n",VID_Statistics.MinBitRate);
	  do_report(0,"\nLastBitRate                                 = %u\n",VID_Statistics.LastBitRate);
	  do_report(0,"\nNbDecodedPicturesB                          = %u\n",VID_Statistics.NbDecodedPicturesB);
	  do_report(0,"\nNbDecodedPicturesP                          = %u\n",VID_Statistics.NbDecodedPicturesP);
	  do_report(0,"\nNbDecodedPicturesI                          = %u\n",VID_Statistics.NbDecodedPicturesI);
	  do_report(0,"\nSpeedSkipReturnNone                         = %u\n",VID_Statistics.SpeedSkipReturnNone);
	  do_report(0,"\nSpeedRepeatReturnNone                       = %u\n",VID_Statistics.SpeedRepeatReturnNone);
	  do_report(0,"\n-- Inject --\n");
	  do_report(0,"\nInjectFdmaTransfers                         = %u\n",VID_Statistics.InjectFdmaTransfers);
	  do_report(0,"\nInjectDataInputReadPointer                  = 0x%08x\n",(U32)VID_Statistics.InjectDataInputReadPointer);
	  do_report(0,"\nInjectDataInputWritePointer                 = 0x%08x\n",(U32)VID_Statistics.InjectDataInputWritePointer);
	
}


/*20091216 add for MP3切换*/

void CH_AudioLink(void)
{
	STAUD_BufferParams_t AUDBufferParams;
	ST_ErrorCode_t ErrCode;
...
void CH_AudioUnLink(void)
{
	ST_ErrorCode_t ST_ErrorCode;	
#if 0
	ST_ErrorCode = STAUD_DeleteDataInputInterface( AUD_Handle[rui_AudInst] );
	if (ST_ErrorCode != ST_NO_ERROR)
	{
	return(ST_ErrorCode);
	}
	...
	ST_ErrorCode = STPTI_BufferFlush(AUDBufHandleTemp);
	if (ST_ErrorCode != ST_NO_ERROR)
	{
		return(ST_ErrorCode);
	}
	{
		ST_ErrorCode = STPTI_SlotUnLink(AudioSlot);
		if (ST_ErrorCode != ST_NO_ERROR)
		{
			return(ST_ErrorCode);
		}
	...
	ST_ErrorCode = STPTI_BufferDeallocate(AUDBufHandleTemp);
	if (ST_ErrorCode != ST_NO_ERROR)
	{
		return(ST_ErrorCode);
	}
