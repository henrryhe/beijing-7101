...
BOOL VID_InjectFromFile(U8 *FileName)
{
	BOOL result=TRUE;
	U32 FSizeTemp=0;
	U8* pFBufTemp=NULL;
	STVID_StartParams_t VIDStartParams;
	Partition_t PartitionTemp=NcachePartition;
	...
	pFBufTemp=VID_LoadFileToMemory(FileName,&FSizeTemp,NcachePartition);
	if(pFBufTemp==NULL)
	{
	...
	result=VID_InjectStart();
	if(result==TRUE)
	{
		VID_InjectParamRelease();
		STTBX_Print(("\nYxlInfo:VID_InjectStart() isn't successful"));
		goto Exit1;
		return TRUE;
	}
...
BOOL VID_InjectWaitFinish(void)
{
	...
	{
		STTBX_Print(("\nYxlInfo:VID_InjectWaitFinish() isn't successful"));
		return TRUE;
	}

	if(VIDi_InjectContext[DeviceId]->State!=VID_INJECT_START)
	{
		STTBX_Print(("\nYxlInfo:Inject isn't in start state"));
		return TRUE;;
	}
...
/* ========================================================================
   Name:        VID_LoadFile
   Description: Load file into memory
   ======================================================================== */
U8* VID_LoadFileToMemory(U8 *FileName,U32* pFileSize,Partition_t UsePartition)
{
	void                         *FILE_Handle;
	...
	*pFileSize=0;
	...
	FILE_Buffer         = Memory_Allocate(UsePartition/*NcachePartition*/,FILE_Size+256);
...
BOOL VID_InjectParamInit(U32 FileSize,U8* pFileBuffer,STVID_StartParams_t VIDStartParams)
{

	U8* pFILE_Buffer;
	U8* pFILE_Buffer_Aligned;
	int DeviceId=0;
/*	STVID_StartParams_t VIDStartParamsTemp;

	memcpy((void*)&VIDStartParamsTemp,(const void*)&VIDStartParams,sizeof(STVID_StartParams_t)));
*/
	STEVT_OpenParams_t            EVT_OpenParams;
	STEVT_DeviceSubscribeParams_t EVT_SubcribeParams; 
	ST_ErrorCode_t                ErrCode=ST_NO_ERROR;
...
#if 0
	VIDi_InjectContext[DeviceId]->VIDStartParams     = *VIDStartParams;
#else
	...
	VIDi_InjectContext[DeviceId]->NbLoops             = 10;
	...
	*/
	...
	if (VIDi_InjectContext[DeviceId]->FdmaNode==NULL)
	{
		STTBX_Print(("nYxlInfo:Unable allocate memory\"));
		goto Exit1;
	}
	...
	/* ---------------------- */
	VIDi_InjectContext[DeviceId]->Task=Task_Create(VIDi_InjectTask,VIDi_InjectContext[DeviceId],2048,MAX_USER_PRIORITY/2,"VID_InjectorTask",
...
BOOL VID_InjectStart()
{
	...
	Task_resume(VIDi_InjectContext[DeviceId]->Task);
#endif 
...
ST_ErrorCode_t VID_InjectStart(U32 DeviceId,U8 *FileName,STVID_StartParams_t *VIDStartParams,U32 NbLoops)
{
	void                         *FILE_Handle;
	U32                           FILE_Size;
	U8                           *FILE_Buffer;
	...
	*/
	...
	ErrCode|=STVID_Start(VIDHandle[DeviceId],VIDStartParams);
	if (ErrCode!=ST_NO_ERROR)
	{
...
{       
	...
	VIDStartParams.BrdCstProfile = STVID_BROADCAST_DVB;
	if(ID)
		VIDStartParams.StreamType    = (STVID_StreamType_t)STVID_STREAM_TYPE_PES;
	else
		VIDStartParams.StreamType    = (STVID_StreamType_t)STVID_STREAM_TYPE_PES;
	VIDStartParams.StreamID      = STVID_IGNORE_ID;
	VIDStartParams.DecodeOnce    = FALSE;
	...
	if(ID)
		...
		VID_InjectStart(ID ,(U8 *)".\\files\\teksoccer_pes.mp4",&VIDStartParams, 10);
	
	else
		VID_InjectStart(ID ,(U8 *)".\\files\\mpeg2_hd_pes.mp2",&VIDStartParams, 10);
#else
	ID=0;
		VIDStartParams.StreamType    = (STVID_StreamType_t)STVID_STREAM_TYPE_ES;
/*	VID_InjectStart(ID ,(U8 *)"trumpet.pes",&VIDStartParams, 10);*/
/*	VID_InjectStart(ID ,(U8 *)"susie.m1v",&VIDStartParams, 10);ok*/
	VID_InjectStart(ID ,(U8 *)"BK06.MPG",&VIDStartParams, 10);/*ok*/
	...
	BOOL res;
	int ID=0;
	res=VID_InjectFromFile((U8 *)"BK06.MPG");
