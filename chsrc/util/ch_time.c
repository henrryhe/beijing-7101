...
2003/11/30       create
2004/7/11        change adapt to 5516
* ----     --------	------------
*/
/*{{{ Includes */
...
#if 1
/*使用静态查找表*/
static const unsigned long  ulTable_CRC32[256] =
{
...
*                                                                            *
*****************************************************************************/
unsigned long  CRC_Code32(U8* pvStartAddress,  int ulSize_in_bytes)
{
...
#define XOROT 0xffffffffL
	...
	ULONG      ulCRC;
	U8  *pbData;          /* %% SRP changed to HuGe 15/02 */
...
/*得到当前日期*/ 
void GetCurrentDate(TDTDATE *MyDate)
{	
...
/*得到当前时间*/   
void GetCurrentTime(TDTTIME *MyTime)
{
#ifdef CH_IPANEL_MGCA
...
/*计算剩余时间*/
/*
clock_t start_time,  计算的开始时间
int total_Second， 总共时间      秒
输出：得到剩余的时间             秒
*/
int CH_GetLeaveTime(clock_t start_time,int total_Second)
{
	unsigned long int time_diff;
	int x_diff_second;
  	time_diff=time_minus(time_now(),start_time);   
    x_diff_second=time_diff/(ST_GetClocksPerSecond());	
    return (total_Second-x_diff_second);
}



/*得到当前STB状态*/
STBStaus CH_GetCurSTBStaus(void)
{

	STBStaus TempStatus;
	if(CH_TUNER_IsLocked())
		TempStatus=SIGNAL_LOCK;
	else TempStatus=NO_SIGNAL;	
	return TempStatus;
