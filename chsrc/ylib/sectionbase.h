...
#define SECTION_CIRCULAR_BUFFER_SIZE      ( ONE_KILO_SECTION_MAX_LENGTH * 8 )/*4*/
...
typedef enum
{
	...
	STOCK_APP
#ifdef MHEG5_FUNCTION /*yxl 2008-02-27 add below section*/
	,MHEG5_APP
#endif /*end yxl 2008-02-27 add below section*/
...
/*yxl 2005-07-26 add this function*/
/*

*/
extern BOOL CH_SetPTIHandle(STPTI_Handle_t Handle/*,U8 Type*/);


extern BOOL CH_GetPTIHandle(STPTI_Handle_t* pHandle);

/*yxl 2005-08-07 add this function for Html section request*/ 

/*Function description:
	Confirm whether a pid is being captured  
Parameters description: 
	message_queue_t *ReturnQueueId:指定SECTION数据收到时要发送的消息队列，
		若为NULL，则SECTION数据收到时不发送消息
	sf_filter_mode_t  FilterMode：指定请求SECTION的1K或4K模式，
		ONE_KILO_SECTION代表1K模式，FOUR_KILO_SECTION代表4K模式
	STPTI_Pid_t ReqPid：指定请求SECTION的PID
	int iNotEqualIndex：指定请求SECTION为相等模式木或不等模式,
		0代表相等模式值，3代表不等模式
	boolean MulSection：指定请求SECTION为单SECTION或多SECTION
		FALSE代表单SECTION,TRUE代表多SECTION
	STPTI_Signal_t SignalHandle：指定SECTION数据收到时要通知的信号量
	app_t app_type:指定该SECTION的应用类型
	U8 FilterDataCount：指定请求SECTION的filter数据的个数
	U8*FilterData：指定请求SECTION的filter数据
	U8 FilterMaskCount：指定请求SECTION的filter MASK数据的个数
	U8*FilterMask：指定请求SECTION的filter MASK数据
	U8 FilterPaternCount：指定请求SECTION的filter PATERN数据的个数
	U8*FilterModePatern:指定请求SECTION的filter PATERN数据

return value:
	-1:代表SECTION请求不成功
	other value:代表SECTION请求成功，返回值为该SECTION占用的FILTER索引
	
*/

extern SLOT_ID	CH6_SectionReq_ByData( message_queue_t *ReturnQueueId,sf_filter_mode_t  FilterMode,
...
extern BOOLEAN CH6_SectionFilterInit ( int MaxSlotNumber,int MaxFilterNumber  );
